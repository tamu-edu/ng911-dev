...
def get_command_parser():
    """
        ...
        --json_file path/report_file.json \
        --filename new_report \
        --report_type json \
        --detailed \
    ...
    jws_generator.add_argument(
        "payload_file_path", type=str, help="Path to payload file"
    )
    jws_generator.add_argument(
    ...
    jws_generator.add_argument(
        "--disable_eddsa_checks",
        action="store_true",
        help="Disables verification if used certificate and key use EdDSA algorithm required for the JWS (optional)",
    )
    ...
    # Command for `run_tests`
    pcap_test_runner = subparsers.add_parser(
        "test_ag_pcap", help="Run NG911 conformance tests from pcap + lab_config"
    )
    pcap_test_runner.add_argument(
        "--tests",
        type=str,
        nargs="+",
        default=None,
        help="Specific test_ids to run (default: auto-discover all ready tests)",
    )
    pcap_test_runner.add_argument(
        "--report_file",
        type=str,
        default=None,
        help="Path to write JSON report file (optional)",
    )
