...
* Test System CHFE (Downstream ESRP)
...
* Device is provisioned with `Test System CHFE` as downstream ESRP
...
* Configure Wireshark to decode SIP over TLS packets from Test System and ESRP as well[^3]
* Using Wireshark on 'Test System' start packet tracing on IF_BCF_ESRP interface - run following filter:
  > ip.addr == IF_CHFE_ESRP_IP_ADDRESS and (tls or sip)
...
- CHFE `sudo sipp -t l1 -sf SIP_INVITE_RECEIVE.xml -i IF_CHFE_ESRP_IP_ADDRESS:5061`
...
- CHFE `sudo sipp -t t1 -sf SIP_INVITE_RECEIVE.xml -i IF_CHFE_ESRP_IP_ADDRESS:5060`
...
- CHFE `sudo sipp -t u1 -sf SIP_INVITE_RECEIVE.xml -i IF_CHFE_ESRP_IP_ADDRESS:5060`
4. Upstream interface TCP/TLS fallback to UDP (use incorrect certificate files cacert.pem and cakey.pem on BCF):
- BCF `sudo sipp -t l1 -sf SIP_INVITE_from_OSP.xml -i IF_BCF_ESRP_IP_ADDRESS:5061 IF_ESRP_BCF_IP_ADDRESS:5061`
- CHFE `sudo sipp -t u1 -sf SIP_INVITE_RECEIVE.xml -i IF_CHFE_ESRP_IP_ADDRESS:5061`
5. Downstream interface TCP/TLS fallback to UDP (accept only UDP transport on CHFE):
- BCF `sudo sipp -t l1 -sf SIP_INVITE_from_OSP.xml -i IF_BCF_ESRP_IP_ADDRESS:5061 IF_ESRP_BCF_IP_ADDRESS:5061`
- CHFE `sudo sipp -t u1 -sf SIP_INVITE_RECEIVE.xml -i IF_CHFE_ESRP_IP_ADDRESS:5061`

#### Stimulus
Run receiving SIPp scenario on CHFE and sending one on BCF. Use commands set for tested variation
...
Variation 5
* verify if SIP INVITE messages have been sent properly
* verify if SIP INVITE from ESRP to CHFE has been sent using UDP transport after failure of TCP/TLS
