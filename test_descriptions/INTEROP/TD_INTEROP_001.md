...
- INTEROP_2.4_TMS_2: The ESInet BCF receives the INVITE.
- INTEROP_2.4_TMS_4: The ESInet BCF sends a 100 Trying provisional response to the OSP.
- INTEROP_2.4_TMS_6: The OSP receives the 100 Trying provisional response sent by the BCF.
- INTEROP_2.4_TMS_7: The BCF scrubs the INVITE to remove inappropriate Resource-Priority and “verstat” values
- INTEROP_2.4_TMS_8: The BCF sends the INVITE to an ESRP (Originating ESRP).
- INTEROP_2.4_TMS_10: The ESRP receives the INVITE from the BCF.
- INTEROP_2.4_TMS_12: The ESRP sends a 100 Trying provisional response to the BCF.
- INTEROP_2.4_TMS_14: The BCF receives the 100 Trying provisional response from the ESRP.
- INTEROP_2.4_TMS_53: The ESRP sends the call to the SIP URI specified in the RouteAction. (PoC: should be sent to the CHFE, ESRP has RouteAction manually preconfigured)
- INTEROP_2.4_TMS_69: The PSAP’s CHFE may send a 180 Ringing or other non-100 provisional response to the ESRP or BCF that sent it the INVITE. If so:
    - The ESRP receives this provisional response.
    - The ESRP sends that response to the BCF
    - BCF received this provisional response.
    - The BCF sends that response to the OSP
- INTEROP_2.4_TMS_70: The PSAP’s CHFE sends a 200 OK final response to the ESRP
- INTEROP_2.4_TMS_72: The ESRP receives the 200 OK final response.
- INTEROP_2.4_TMS_74: The ESRP sends the 200 OK to the BCF from which it received the INVITE.
- INTEROP_2.4_TMS_75: The BCF sends 200 OK to the OSP.
- INTEROP_2.4_TMS_76: The OSP receives the 200 OK sent by the Originating BCF.
- INTEROP_2.4_TMS_77: The OSP sends an ACK to the Originating BCF.
- INTEROP_2.4_TMS_78: The BCF receives the ACK sent by the OSP.
- INTEROP_2.4_TMS_80: The BCF sends the ACK to the Originating ESRP.
- INTEROP_2.4_TMS_81: The Originating ESRP receives the ACK from the BCF.
- INTEROP_2.4_TMS_84: That ESRP sends the ACK to the CHFE
- INTEROP_2.4_TMS_89: The call taker answers the call; interactive media is established between caller and call taker OSP<->CHFE (media is anchored at each call-anchoring point - BCF).

##### Message checks

- INTEROP_2.4_CTES_1: Verify that at each step in the Test Message Sequence the input and output messages contain correct content.
...
- INTEROP_2.4_IETES_6: Originating BCFs anchor media (the RTP media goes through the BCF)
- INTEROP_2.4_IETES_7: <!-- DISABLED for PoC: Encrypted --> media stream(s) successfully established between OSP and Originating BCF <!-- when SDP from OSP requested encrypted media -->
- INTEROP_2.4_IETES_8: <!-- DISABLED for PoC: Encrypted --> media stream(s) successfully established between PSAP CHFE and OSP <!-- when SDP from OSP requested encrypted media, and between PSAP CHFE and Originating BCF when SDP from OSP did not request encrypted media. -->
