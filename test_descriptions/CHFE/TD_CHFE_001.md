...
* Test System SIP
  * IF_TSSIP_CHFE - connected to IF_CHFE_TSSIP
* CHFE
  * IF_CHFE_TSSIP - connected to IF_TSSIP_CHFE
  * IF_CHFE_TSLIS - connected to IF_TSLIS_CHFE
* Test System LIS
  * IF_TSLIS_CHFE - connected to IF_CHFE_TSLIS


### Test System Interfaces
<!-- Identify each of the test system interfaces and whether it will be in active or monitor mode -->
* Test System SIP
  * IF_TSSIP_CHFE - Active
* CHFE
  * IF_CHFE_TSSIP - Active
  * IF_CHFE_TSLIS - Active
* Test System LIS
  * IF_TSLIS_CHFE - Active
...
* Using Wireshark on 'Test System SIP' start packet tracing on IF_TSSIP_CHFE interface - run following filter:
   * (TLS)
     > ip.addr == IF_TSSIP_CHFE_IP_ADDRESS and tls
   * (TCP)
     > ip.addr == IF_TSSIP_CHFE_IP_ADDRESS and http
...
* Using Wireshark on 'Test System LIS' start packet tracing on IF_TSLIS_CHFE interface - run following filter:
   * (TLS)
     > ip.addr == IF_TSLIS_CHFE_IP_ADDRESS and tls
   * (TCP)
     > ip.addr == IF_TSLIS_CHFE_IP_ADDRESS and (http or sip)
...
* run following SIPp scenario to handle SIP Presence Event Package:
    * (TCP transport)
      ```
      sudo sipp -t t1 -sf SIP_SUBSCRIBE_LIS.xml IF_TSLIS_CHFE_IPv4:5060
      ```
    * (TLS transport)
      ```
      sudo sipp -t l1 -tls_cert test_system.crt -tls_key test_system.key -sf SIP_SUBSCRIBE_LIS.xml IF_TSLIS_CHFE_IPv4:5061
      ```
...
* (TCP transport)
  ```
  sudo sipp -t t1 -sf SIPP_SCENARIO IF_TSSIP_CHFE_IPv4:5060
  ```
* (TLS transport)
  ```
  sudo sipp -t l1 -tls_cert test_system.crt -tls_key test_system.key -sf SIPP_SCENARIO IF_TSSIP_CHFE_IPv4:5061
  ```
