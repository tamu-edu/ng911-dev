...
### References
* Requirements : RQ_CHFE_003, RQ_CHFE_004, RQ_CHFE_005
* Test Case    : TC_CHFE_004
...
* Test System
  * IF_TS_CHFE - connected to IF_CHFE_TS
* CHFE
  * IF_CHFE_TS - connected to IF_TS_CHFE 
...
![image](../_assets/CHFE/TD_CHFE_004_Connectivity_Diagram.png)
...
* Using Wireshark on 'Test System' start packet tracing on IF_TS_CHFE interface - run following filter:
   * (TLS transport)
     > ip.addr == IF_TS_CHFE_IP_ADDRESS and tls
   * (TCP transport)
     > ip.addr == IF_TS_CHFE_IP_ADDRESS and sip
* Generate call for callback test. Send SIP INVITE to CHFE - run following SIPp command on Test System, example:
    * (TLS transport)
      > sudo sipp -t l1 -sf SIP_INVITE_SDP_with_g711alaw_audio.xml  -tls_cert test_system.crt -tls_key test_system.key IF_CHFE_TS_IP_ADDRESS:5061
    * (TCP transport)
      > sudo sipp -t t1 -sf SIP_INVITE_SDP_with_g711alaw_audio.xml IF_CHFE_TS_IP_ADDRESS:5060
* Prepare Test System to receive SIP INVITE - run following SIPp command on Test System, example:
    * (TLS transport)
      > sudo sipp -t l1 -sf SIP_INVITE_RECEIVE.xml -tls_cert test_system.crt -tls_key test_system.key -i IF_TS_CHFE_IP_ADDRESS -p 5061
    * (TCP transport)
      > sudo sipp -t t1 -sf SIP_INVITE_RECEIVE.xml -i IF_TS_CHFE_IP_ADDRESS -p 5060
...
1. Validate SIP INVITE sent by CHFE for callbacks
2. Validate SIP INVITE sent by CHFE for outbound calls

#### Stimulus
* Variation 1
  
  On CHFE trigger callback to incoming call sent earlier - follow steps described in IXIT config file
  
* Variation 2
  
  On CHFE trigger outbound call to to Test System - follow steps described in IXIT config file

#### Response
* Variation 1
  - CHFE should send SIP INVITE to Test System
  - SIP INVITE should contain:
    ...
    - 'From' header field containing 'sip:TEL_NUMBER@CHFE_FQDN;user=phone' where TEL_NUMBER is 10-digit NANP number or E.164 number (f.e. '+5511912345678') and CHFE_FQDN shall be correct domain name
    - 'Via' header field with SIP protocol version, transport type and CHFE ip address + port, f.e. `Via: SIP/2.0/TCP 192.0.2.1:5060;branch=z9hG4bK23423`
    - 'Call-ID' with unique string
    - 'CSeq' with string value f.e. `1 INVITE`
    - 'Contact' with SIP URI of CHFE
    ...
    - Second 'P-Asserted-Identity' containing identity of an Agent logged in to CHFE (f.e. `P-Asserted-Identity: sip:<tester@psap.example.com>`
...
* Variation 2
  ...
  - SIP INVITE should contain:
    ...
    - 'From' header field containing one of following:
      - 'sip:TEL_NUMBER@CHFE_FQDN;user=phone' where TEL_NUMBER is 10-digit NANP number or E.164 number (f.e. '+5511912345678') and CHFE_FQDN shall be correct domain name
    ...
    - 'P-Asserted-Identity' containing 'sip:TEL_NUMBER@CHFE_FQDN;user=phone' where TEL_NUMBER is 10-digit int and CHFE_FQDN shall be correct domain name
    - Second 'P-Asserted-Identity' containing identity of an Agent logged in to CHFE (f.e. `P-Asserted-Identity: sip:<tester@psap.example.com>`
...
VERDICT:
* PASSED - if CHFE sends language tags as expected
...
![image](../_assets/CHFE/TD_CHFE_004_Sequence_Diagram.png)
