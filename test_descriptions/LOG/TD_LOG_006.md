...
33. Validate 4xx error response for request with incorrect 'endTime' parameter (send incorrect day in February '2015-02-30T12:58:03.01-05:00')
    ...
    ```
    LOGGING_SERVICE_FQDN_OR_IP:PORT/CallIds?endTime=2025-02-30T12%3A58%3A61.01%2D05%3A00
    ```
...
36. Validate 200 OK + JSON response for any correct request.

    Send request with additional incidentId parameter, example URL:
    ```
    IF_LOG_TS_IP_ADDRESS:PORT/CallIds?incidentId=urn:emergency:uid:incidentid:0123456789qwertyuiop:bcf.ng911.test
    ```

37. Validate JSON response for request with valid startTime.

    Send request with additional startTime parameter, example URL:
    
    ```
    IF_LOG_TS_IP_ADDRESS:PORT/CallIds?startTime=2025-02-27T12%3A58%3A01.01%2D05%3A00
    ```

38. Validate JSON response for request with valid endTime.

    Send request with additional endTime parameter, example URL:
    
    ```
    IF_LOG_TS_IP_ADDRESS:PORT/CallIds?endTime=2025-02-27T12%3A58%3A11.01%2D05%3A00
    ```

39. Validate JSON response for request with valid endTime.

    Send request with additional startTime and endTime parameters, example URL:
    
    ```
    IF_LOG_TS_IP_ADDRESS:PORT/CallIds?startTime=2025-02-27T12%3A58%3A01.01%2D05%3A00?endTime=2025-02-27T12%3A58%3A11.01%2D05%3A00
    ```
...
## Sequence Diagram
<!--
https://mermaid.live/edit#pako:eNq1kl9LwzAUxb9KuK9rR5O2y8zDQFQU_AsrMqQvob3LojaZaTqcY9_dtnNu6KOYp9zk_M49cO8GClsiCAjDMDeFNXOtRG4IqbRz1p0W3rpakLl8rTE3vajGtwZNgedaKierTrw7d9YjsSt0JMPak-m69lgF5MYqpY0iU3QrXaAgj9Jp6bU1hIZxcuCPqHAyGfzirrLsgVxeZAfih6Sjjky-iGQ2-0vGOA3jk38PyaKI3F-TAXmurYEAlNMlCO8aDKBCV8muhE3nmoNfYIU5iPZaSveSQ262LbOU5snaao8526gFiH50ATTLUvr9zL5fHZoS3ZltjAfBOO9NQGzgvS3pkI14G4yP01FEo3ESwBpESFM-TBIac95-cpaybQAffV86pOmIRbRTs3GSsJaQjbfTtSn2qbDU7Urd7pau373tJxx9x9Q
-->
