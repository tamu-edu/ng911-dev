...
* Variation 3
  ...
  * JWS payload JSON body shall contain:
     ...
     <!-- - 'incidentId' should be the same as in `Call-Info` header field of SIP message included within `"text"` parameter of Log Event -->
      - 'calIdSip' - can be added to any 'logEventType', but is required for:
         ```
           CallProcessLogEvent
           CallStartLogEvent
           CallEndLogEvent
           ...
           CallStateChangeLogEvent
         ```
      - 'calIdSip' - if exist should be a string conformant with Call-ID format (RFC 3261)
      <!-- - 'calIdSip' should be the same as in `Call-ID` header field of SIP message included within `"text"` parameter of Log Event -->
