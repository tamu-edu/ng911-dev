...
def _pretty(value, indent=0):
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            nested = _pretty(v, indent + 1)
            if "\n" in nested:
                items.append(f"{space}{k}:\n{nested}")
            else:
                items.append(f"{space}{k}: {nested}")
        return "\n".join(items)
    ...
    if isinstance(value, (list, tuple)):
        ...
        for i, v in enumerate(value):
            nested = _pretty(v, indent + 1)
            if "\n" in nested:
                items.append(f"{space}[{i}]\n{nested}")
            else:
                items.append(f"{space}[{i}] {nested}")
        return "\n".join(items)
...
def log_method_args(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        params = "\n".join(
            f"{name} = {_pretty(value)}" for name, value in bound.arguments.items()
        )
