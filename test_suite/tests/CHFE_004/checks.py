...
from services.aux_services.sip_msg_body_services import clean_up_string, extract_media_attributes
...
def validate_chfe_sip_invite_for_callback(sip_invtie_to_chfe, chfe_response, ):
    """
    ...
    initial_pai = get_header_field_value(sip_invtie_to_chfe, 'P-Asserted-Identity', )

    # Validate From header format
    from_hdr = get_header_field_value(chfe_response, 'From', )
    if not from_hdr:
    ...
    # Validate 'Priority'
    if clean_up_string("".join(get_header_field_value(chfe_response, 'Priority').strip())) != 'psap-callback':
...
def validate_chfe_sip_invite_for_outbound_calls(chfe_response):
    ...
    pai_hdr = clean_up_string(get_header_field_value(chfe_response, 'P-Asserted-Identity'))
    if not pai_hdr:
...
def validate_chfe_callbacks_and_outbound(sip_invite_to_chfe, chfe_response, variation_method):
    """
    ...
    content_length = clean_up_string(get_header_field_value(chfe_response, 'Content-Length'))
    ...
    if variation_method == 'send':
        ...
        if (result := validate_chfe_sip_invite_for_callback(sip_invite_to_chfe, chfe_response)) != 'PASSED':
            return result

    else:
        if (result := validate_chfe_sip_invite_for_outbound_calls(chfe_response)) != 'PASSED':
            return result
