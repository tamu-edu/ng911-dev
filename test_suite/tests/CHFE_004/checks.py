...
def is_valid_sip_uri(uri: str) -> bool:
    ...
    """
    pattern = re.compile(
        r"^sips?:"  # SIP schememe # before r'^sip:'
...
def validate_chfe_sip_invite_for_callback(
    sip_invtie_to_chfe,
    chfe_response,
):
    """
    Method to validate SIP INVITE sent by CHFE for callbacks
    """
    # Extract From and P-Asserted-Identity from initial SIP INVITE
    initial_from = get_header_field_value(sip_invtie_to_chfe, "From")
    initial_pai = get_header_field_value(
        sip_invtie_to_chfe,
        "P-Asserted-Identity",
    )

    # Validate From header format
    from_hdr = get_header_field_value(
        chfe_response,
        "From",
    )
    if not from_hdr:
    ...
    if request_uri not in (initial_from or "") and request_uri not in (
        initial_pai or ""
    ):
        return (
            "FAILED -> Request-URI does not match initial From or P-Asserted-Identity"
        )

    # Validate 'To' header
    to_hdr = get_header_field_value(chfe_response, "To")
    ...
    if (
        clean_up_string(
            "".join(get_header_field_value(chfe_response, "Priority").strip())
        )
        != "psap-callback"
    ):
    ...
    if "esnet.0" not in get_header_field_value(chfe_response, "Resource-Priority"):
        return "FAILED -> 'Resource-Priority' header must be 'esnet.0'"

    # Validate P-Asserted-Identity matches From
    pai_hdr = get_header_field_value(chfe_response, "P-Asserted-Identity")
    if pai_hdr != from_hdr:
...
def validate_chfe_sip_invite_for_outbound_calls(chfe_response):
    """
    Method to validate SIP INVITE sent by CHFE for outbound calls
    """
    # Validate Request-URI
    request_uri_match = re.match(
        r"^INVITE\s+"
    ...
    # Validate 'To' header
    to_hdr = clean_up_string(get_header_field_value(chfe_response, "To"))
    if not to_hdr:
        return "FAILED -> Missing 'To' header"
    else:
        uri_match = re.search(r"(sip:[^;>]+|tel:[^;>]+)", to_hdr)
        if not uri_match or not (
            is_valid_tel_uri(uri_match.group(1)) or is_valid_sip_uri(uri_match.group(1))
        ):
            return "FAILED -> 'To' header must contain a valid tel: or sip: URI"

    # Validate 'From' header
    from_hdr = clean_up_string(get_header_field_value(chfe_response, "From"))
    if not from_hdr:
        return "FAILED -> Missing 'From' header"
    else:
        anon_pattern = r"^sip:anonymous@anonymous\.invalid"
        phone_pattern = r"^sip:(\+?\d{10,15})@([a-zA-Z0-9.-]+);user=phone$"
        if not re.match(anon_pattern, from_hdr) and not re.match(
            phone_pattern, from_hdr
        ):
            return (
                "FAILED -> 'From' header must be either 'sip:anonymous@anonymous.invalid' "
            ...
            if not re.search(FQDN_PATTERN, fqdn):
                return "FAILED -> CHFE FQDN in 'From' is not valid FQDN"

    # Validate 'Resource-Priority'
    rpri = clean_up_string(get_header_field_value(chfe_response, "Resource-Priority"))
    ...
    pai_hdr = clean_up_string(
        get_header_field_value(chfe_response, "P-Asserted-Identity")
    )
    if not pai_hdr:
        ...
        if not match:
            ...
            if not re.search(FQDN_PATTERN, fqdn):
                return "FAILED -> CHFE FQDN in 'P-Asserted-Identity' is not valid FQDN"

    # Validate 'Privacy' only when From is anonymous
    if "sip:anonymous@anonymous.invalid" in from_hdr:
        privacy = clean_up_string(get_header_field_value(chfe_response, "Privacy"))
        if privacy != "user":
...
def validate_chfe_callbacks_and_outbound(
    sip_invite_to_chfe, chfe_response, variation_method
):
    """
    Method that parses incoming test data: SIP initial INVITE,
                                           CHFE INVITE response,
                                           Variation method
    """
    if not chfe_response:
        return "FAILED -> CHFE response not found"

    # Check 'Content-Type'
    content_type_list = get_content_type_data(chfe_response)
    for content_type in content_type_list:
        if "application/sdp" not in clean_up_string(content_type):
            return "FAILED -> 'Content-Type' must be 'application/sdp'"

    # Check 'Via'
    via = clean_up_string(get_header_field_value(chfe_response, "Via"))
    if not via:
    ...
    # Check 'Contact'
    contact = clean_up_string(get_header_field_value(chfe_response, "Contact"))
    if not contact or not re.search(
        r"\b(?:sip|sips|tel):[^@<>\s]+@[^;:<>\s]+(?::\d+)?(?:;[^<>\s]*)?", contact
    ):
        return "FAILED ->Invalid or missing Contact header"

    # Check 'Cseq'
    cseq = clean_up_string(get_header_field_value(chfe_response, "Cseq"))
    if not cseq or not re.match(r"^\d+ INVITE$", cseq):
        return "FAILED -> 'CSeq' must be in format '<number> INVITE'"

    # Check 'Call-ID'
    call_id = clean_up_string(get_header_field_value(chfe_response, "Call-ID"))
    # TODO: Potential place where "INCONCLUSIVE" result may be applied since there might be a problem with regex
    if not call_id or not re.match(
        r"[a-zA-Z0-9\-_.!%*+`'~]+(?:@[a-zA-Z0-9\-._]+)?", call_id
    ):
        return "FAILED -> 'Call-ID' is missing or invalid format"

    # Check 'Content-Length'
    content_length = clean_up_string(
        get_header_field_value(chfe_response, "Content-Length")
    )
    if not content_length or not content_length.isdigit():
        return "FAILED -> 'Content-Length' is missing or not an integer"

    # Check 'Route'
    route_hdr = clean_up_string(get_header_field_value(chfe_response, "Route"))
    if not route_hdr:
    ...
    if not sdp:
        ...
        for hlang in ["hlang-send", "hlang-recv"]:
            ...
            if not hlang_sdp or not all(
                [lang in bcp47.tags for lang in hlang_sdp.split(" ")]
            ):
    ...
    if variation_method == "send":
        if not sip_invite_to_chfe:
            return "FAILED -> SIP INVITE to CHFE not found"
        if (
            result := validate_chfe_sip_invite_for_callback(
                sip_invite_to_chfe, chfe_response
            )
        ) != "PASSED":
            return result
        ...
        if (
            result := validate_chfe_sip_invite_for_outbound_calls(chfe_response)
        ) != "PASSED":
            return result
