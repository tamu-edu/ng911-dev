...
def get_test_parameters(
    ...
    xml_tag_as_of = None
    response_data: set | None | list | str = None
    response_code = None

    xml_request = extract_xml_body_string_from_file(
        variation_scenario.removeprefix("file.")
    )
    if xml_request:
        xml_tag_as_of_result = extract_all_values_for_xml_tag_name(xml_request, "asOf")
        if xml_tag_as_of_result:
            xml_tag_as_of = (
                xml_tag_as_of_result[0]
                if isinstance(xml_tag_as_of_result, list)
                else xml_tag_as_of_result
            )

    # Variation 3/4
    if variation_url == "/PlannedChangePoll":
        ecrf_lvf_requests = get_messages(
            pcap_service,
            FilterConfig(
        ...
        if stimulus_messages:
            ...
            ok_and_json_response_list = [
                message
                for message in ecrf_lvf_responses
            ...
            json_responses = [
                extract_json_data_from_http(message)
                for message in ok_and_json_response_list
            ]
...
def get_test_names() -> list:
    return [
        ...
        "Validate ECRF-LVF response for any correct request",
    ]
