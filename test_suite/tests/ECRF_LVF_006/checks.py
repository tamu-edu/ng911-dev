...
from services.aux_services.message_services import is_valid_catype_schema, is_equal_or_after
from services.aux_services.xml_services import is_valid_xml


def validate_ecrf_response_data(response_data, expected_response_code, response_code, variation_url, xml_tag_as_of):
    """
    ...
    try:
        ...
        assert response_data, \
            "FAILED-> No output message found"
        ...
        if (response_data and not isinstance(response_data, dict)
            ...
            assert is_valid_xml(response_data), \
                "FAILED-> Malformed XML data"

            assert is_equal_or_after(xml_tag_as_of, response_data), \
                "FAILED-> 'findServiceResponse' timestamp should be equal or after requested."
        else:
            assert is_valid_json(response_data), \
                "FAILED-> Malformed JSON data"
            # Variation #3/4
            if 'PlannedChangePoll'.lower() in variation_url.lower():
                # Variation #4
                if isinstance(response_data, set):
                    assert len(response_data) > 1, \
                        f"FAILED-> ECRF-LVF response should be a data array with at least 2 responses."
                else:
                    # Variation #3
                    assert isinstance(response_data, dict), \
                        f"FAILED-> ECRF-LVF response should be a data array."

                    assert 'PlannedChangeIdList' in response_data.keys(), \
                        "FAILED-> No 'PlannedChangeIdList' in JSON response"

                    planned_changed_id_list = response_data.get('PlannedChangeIdList', None)

                    assert (result := is_type(planned_changed_id_list,
                                              'PlannedChangeIdList',
                                              (list, dict))) == 'PASSED', \
                        f"FAILED-> {result}"
                    assert all([isinstance(item, str) for item in planned_changed_id_list]), \
                        "FAILED-> All items inside of 'PlannedChangeIdList' should be strings"
            else:
                # VARIATION #5
                assert 'ChangeSet' in response_data, \
                    "FAILED->  'ChangeSet' key missing from response_data."

                change_set = response_data['ChangeSet']

                assert (result := is_type(change_set,
                                          'ChangeSet',
                                          (list, dict))) == 'PASSED', \
                    f"FAILED-> {result}"

                assert 'changeSetId' in change_set, \
                    "FAILED-> 'changeSetID' key missing from ChangeSet."

                assert (result := is_type(change_set['changeSetId'],
                                          'changeSetId',
                                          str)) == 'PASSED', \
                    f"FAILED-> {result}"

                assert 'changeSetEffective' in change_set, \
                    "FAILED->  'changeSetEffective' key missing from ChangeSet."

                assert (result := is_type(change_set['changeSetEffective'],
                                          'changeSetEffective',
                                          str)) == 'PASSED', \
                    f"FAILED-> {result}"
                ...
                assert 'partialLocationList' in change_set, \
                    "FAILED-> 'partialLocationList' key missing from ChangeSet."

                assert (result := is_type(change_set['partialLocationList'],
                                          'partialLocationList',
                                          (list, dict))) == 'PASSED', \
                    f"FAILED-> {result}"
                ...
                for record in partial_location_list:
                    ...
                    assert 'caType' in record, \
                        "FAILED-> 'caType' missing in 'partialLocationList item'."

                    ca_type = record.get('caType', None)
                    namespace = record.get('namespace', None)
                    assert is_valid_catype_schema(ca_type, namespace), \
                        f"FAILED-> Invalid 'caType' or 'namespace'."

                    # Check for 'value'
                    assert 'value' in record, \
                        f"FAILED-> 'value' missing in 'partialLocationList item'."

                    assert (result := is_type(record['value'],
                                              'value',
                                              str)) == 'PASSED', \
                        f"FAILED-> {result}"
