...
def validate_support_of_discrepancy_report(
    sip_invite_str, out_message_json, emergency_call_info_header
):
    ...
    try:
        ...
        if packet_header:
            assert (
                result := is_type(packet_header, "packetHeader", str)
            ) == "PASSED", f"INCONCLUSIVE -> {result}"
