...
from .constants import OSP, O_BCF, SBC
from .checks import (
...
def get_test_parameters(pcap_service: PcapCaptureService):
    ...
    o_bcf_sbc_upd_messages = pcap_service.get_messages_by_config(
        FilterConfig(src_ip=O_BCF, dst_ip=SBC, packet_type=PacketTypeEnum.UDP)
    )

    o_bcf_sbc_upd_audio_first_message, o_bcf_sbc_upd_video_first_message = (
        get_first_audio_video_first_messages(o_bcf_sbc_upd_messages)
    )
    osp_o_bcf_udp_messages = pcap_service.get_messages_by_config(
        FilterConfig(src_ip=SBC, dst_ip=O_BCF, packet_type=PacketTypeEnum.UDP)
    )

    osp_o_bcf_upd_audio_first_message, osp_o_bcf_upd_video_first_message = (
        get_first_audio_video_first_messages(osp_o_bcf_udp_messages)
    )
