...
from tests.LOG_010.constants import INCIDENT_ID
...
def get_test_parameters(
    ...
    ):  # if it /IncidentIds + some additional parameters like incidentId/startTime/endTime
...
def get_test_names() -> list:
    return [
        ...
        "Validate 200 OK + JSON response for any correct request",
        "Validate 200 OK + JSON response for /IncidentIds with additional parameters",
    ]
...
def get_test_list(
    ...
    # Verification of 200 OK response
    else:
        # Verification of 200 OK response for /IncidentIds + additional parameters (incidentId/startTime/endTime)
        if is_additional_parameters:
            return [
                TestCheck(
                    test_name="Validate 200 OK + JSON response for /IncidentIds with additional parameters",
                    test_method=validate_response_json_body_for_http_get_on_log_event_ids_entry_point,
                    test_params={
                        "response": test_response,
                        "additional_element": {"incidentIds": (list, dict, tuple)},
                        "array_name": "IncidentIdArray",
                        "id_name": "incidentIds",
