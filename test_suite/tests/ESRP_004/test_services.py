...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, expected_response_code, method = (
        get_filter_parameters(lab_config, filtering_options, variation)
    )
    message_method = (
        [
            SIPMethodEnum.OPTIONS,
        ]
        if method == "OPTIONS"
        else [
...
def get_test_names() -> list:
    return [
        "Validate processing of SIP OPTIONS and SIP CANCEL",
    ]
...
def get_test_list(
    ...
    exp_response_code, call_id, esrp_resp = get_test_parameters(
