...
def get_filter_parameters(lab_config: LabConfig, filtering_options: list[MessageFilter], variation) \
    ...
    for message in filtering_options:
        if message.message_type == FilterMessageType.STIMULUS:
            stimulus = message
            expected_response_code = message.response_status_code if hasattr(message, 'response_status_code') \
    ...
    if stimulus:
        ...
        if stimulus_src_ip is None or stimulus_dst_ip is None or variation_file_path is None:
            ...
            return (stimulus_src_ip, stimulus_dst_ip, out_scr_ip, out_dst_ip, out_dst_fqdn,
                    expected_response_code, variation_file_path, key_filepath)
    else:
...
def get_test_parameters(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    (stimulus_src_ip, stimulus_dst_ip, out_scr_ip, out_dst_ip, out_dst_fqdn,
     expected_response_code, variation_file_path, key_filepath) = \
    ...
    logger_out_message = get_http_response_containing_string_in_http_body_for_message_matching_filter(
        pcap_service,
        FilterConfig(
    ...
    if logger_out_message:
        response_code = logger_out_message.http.response_code \
            if hasattr(logger_out_message.http, 'response_code') else None
    ...
    if request_x5u and out_dst_ip and ((out_dst_ip in request_x5u) or (out_dst_fqdn in request_x5u)):
        logger_to_lis_request = get_first_message_matching_filter(
            ...
            FilterConfig(
                ...
                after_timestamp=float(logger_out_message.sniff_timestamp) if logger_out_message else None
            )
        )
        if logger_to_lis_request and hasattr(logger_to_lis_request.http, 'request_full_uri', ):
            x5u_from_response = logger_to_lis_request.http.request_full_uri

        is_x5u_the_same = True if (request_x5u and x5u_from_response) and (request_x5u == x5u_from_response) else False
        return response_code, expected_response_code, request_x5u, logger_to_lis_request, is_x5u_the_same
...
def get_test_names() -> list:
    return [f"Verify Logging Service response.", ]
...
def get_test_list(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    response_code, expected_response_code, request_x5u, logger_to_lis_request, is_x5u_the_same \
