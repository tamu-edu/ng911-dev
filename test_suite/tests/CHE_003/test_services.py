...
def get_test_parameters(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    for message in sip_invite_messages_responses:
        if request_method == 'send':
            # Filter out SIP 200 OK INVITE message fom all CHE response messages
            ...
            # Filter out SIP INVITE message fom all CHE response messages to any other IP not from Test System
            if hasattr(message.sip, 'cseq_method'):
                if message.sip.cseq_method == 'INVITE' and message.ip.dst != stimulus_src_ip:
                    sip_invite_messages_response = message
                    continue
