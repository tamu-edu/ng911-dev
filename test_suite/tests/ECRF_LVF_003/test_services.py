...
def get_test_names() -> list:
    return [
         f"(request=1x fully covered service boundary + 1x partially) returning service fully covered",
         f"(request=2x fully covered service boundaries) returning one of service boundaries",
    ]
...
def get_test_list(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    if variation_test_name == "findService_polygon_covering_fully_one_and_partially_another_boundary":
        return [
            TestCheck(
                ...
                test_params={
                    ...
                    "expected_sip_uri_list": [ECRF_LVF_SERVICE_BOUNDARIES[0]["Boundary1"]["SIP_URI"]]
                }
            )
            ]
    else:
