from checks.http.lost_checks.checks import (
    test_http_lost_find_service_response_with_sip_uri_check,
)
...
from services.aux_services.xml_services import (
    extract_xml_body_string_from_file,
    extract_all_xml_bodies_from_message,
)
from .constants import ECRF_LVF_SERVICE_BOUNDARIES
    ...
    for param in variation.params:
        if param == "messages":
            for message in variation.params["messages"]:
                sipp_scenario_file_path = message.get("body", None).removeprefix(
                    "file."
                )
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, sipp_scenario, variation_name = (
...
def get_test_names() -> list:
    return [
        "(request=1x fully covered service boundary + 1x partially) returning service fully covered",
        "(request=2x fully covered service boundaries) returning one of service boundaries",
    ]
...
def get_test_list(
    ...
    xml_request, xml_response, variation_test_name = get_test_parameters(
            ...
            TestCheck(
                ...
                test_params={
                    ...
                    "expected_sip_uri_list": [
                        ECRF_LVF_SERVICE_BOUNDARIES[0]["Boundary1"]["SIP_URI"]
                    ],
                },
            )
        ]
        ...
        return [
            TestCheck(
                ...
                test_params={
                    ...
                    "expected_sip_uri_list": [
                        ECRF_LVF_SERVICE_BOUNDARIES[0]["Boundary1"]["SIP_URI"],
                        ECRF_LVF_SERVICE_BOUNDARIES[1]["Boundary2"]["SIP_URI"],
                    ],
                },
            )
        ]
