...
from tests.ESRP_018.checks import (
...
def get_filter_parameters(
    ...
    esrp_fqdn = iut_entity.get_first_available_fqdn()
...
def get_test_parameters(
    ...
    esrp_to_bcf_initial_notify_json = None
    esrp_to_logger_http_post_message = None
    esrp_to_logger_post_data = None
    ...
    esrp_to_bcf_initial_notify_state_message = get_first_message_matching_filter(
        pcap_service,
        FilterConfig(
    ...
    if esrp_to_bcf_initial_notify_state_message:
        message_content = extract_all_contents_from_message_body(
            esrp_to_bcf_initial_notify_state_message
        )
        if message_content:
            esrp_to_bcf_initial_notify_json = message_content[0].get("body", "")

    esrp_to_logger_http_post_messages = get_messages(
        pcap_service,
        FilterConfig(
            src_ip=out_scr_ip,
            dst_ip=out_dst_ip,
    ...
    if esrp_to_logger_http_post_messages:
        jws, message = get_message_and_jws_by_event_type(
            esrp_to_logger_http_post_messages,
            "ServiceStateChangeLogEvent",
            key_filepath,
        )
        esrp_to_logger_post_data = jws
        esrp_to_logger_http_post_message = message
    ...
    test_data.esrp_to_bcf_initial_notify_json = esrp_to_bcf_initial_notify_json
    test_data.esrp_to_logger_http_post_message = esrp_to_logger_http_post_message
    test_data.esrp_to_logger_post_data = esrp_to_logger_post_data
...
def get_test_names() -> list:
    return [
        "Validation of ESRP ServiceStateChangeLogEvent members after 'Security Posture' change.",
        "Validation of ESRP ServiceStateChangeLogEvent members after 'ServiceState' change.",
        "Validation of ESRP ServiceStateChangeLogEvent 'Security Posture' change.",
        "Validation of ESRP ServiceStateChangeLogEvent 'ServiceState' change.",
    ]
...
def get_test_list(
    ...
    variant_one_name = (
        "Logging of ServiceStateChangeLogEvent on ESRP Security Posture changes"
    )
    ...
    if variation.name == variant_one_name:
        ...
        return [
            TestCheck(
                test_name="Validation of ESRP ServiceStateChangeLogEvent members after 'Security Posture' change.",
            ...
            TestCheck(
                test_name="Validation of ESRP ServiceStateChangeLogEvent 'Security Posture' change.",
        ...
        return [
            TestCheck(
                test_name="Validation of ESRP ServiceStateChangeLogEvent members after 'ServiceState' change.",
                test_method=validate_event_members,
                test_params={
                    "test_data": test_data,
                },
            ...
            TestCheck(
                test_name="Validation of ESRP ServiceStateChangeLogEvent 'ServiceState' change.",
