...
from tests.LIS_005.checks import validate_lis_certs_acceptance
    ...
    for param in variation.params:
        if param == "messages":
            try:
                ...
                if len(variation.params[param]) == 0:
                    continue

                if len(variation.params[param][0]["prep_steps"]) == 0:
                    continue

                method_name = variation.params[param][0]["prep_steps"][0].get(
                    "method_name"
                )
            except KeyError:
                continue
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, method_name = get_filter_parameters(
    ...
    if tcp_responses:
        has_app_data = (
            ...
            if [
                record
                for record in tcp_responses
                if hasattr(record, "tls")
                and hasattr(record.tls, "app_data")
                and record.tls.app_data is not None
            ]
            else False
        )
        has_alert_message = (
            ...
            if [
                ...
                and record.tls.alert_message is not None
            ]
            else False
        )
...
def get_test_names() -> list:
    return [
        "Verify LIS accepts only PCA traceable certificates",
    ]
...
def get_test_list(
    ...
    method_name, has_app_data, has_alert_message = get_test_parameters(
        pcap_service, lab_config, filtering_options, variation
    )
    return [
        TestCheck(
            test_name="Verify LIS accepts only PCA traceable certificates",
            test_method=validate_lis_certs_acceptance,
            test_params={
