...
from logger.method_decorators import log_method_args, log_method_result
...
from checks.sip.resource_priority_header_field_checks.constants import (
    RESOURCE_PRIORITY_PATTERN,
)
from checks.sip.resource_priority_header_field_checks.checks import (
    test_resource_priority,
)
from checks.sip.stimulus_output_by_call_id_check.checks import (
    test_compare_stimulus_and_output_messages,
)
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, out_scr_ip, out_dst_ip = get_filter_parameters(
    ...
    if bcf_output_message is not None and hasattr(bcf_output_message, "sip"):
        bcf_o_headers = split_sip_header_by_pattern(
            bcf_output_message.sip.get("msg_hdr")
        )
...
def get_test_list(
    ...
    return [
        TestCheck(
            ...
            test_params={"stimulus": stimulus_message, "output": bcf_output_message},
        ),
        TestCheck(
