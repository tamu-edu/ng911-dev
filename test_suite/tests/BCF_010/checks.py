NOT_FOUND_ERROR = "FAILED -> {} hello not found"
NOT_RUN_ERROR = "NOT RUN -> Client Hello cipher suite has not been found"
...
def is_both_client_and_server_hello_present(client_hello, server_hello):
    ...
    try:
        assert client_hello is not None, NOT_RUN_ERROR
        assert server_hello is not None, NOT_FOUND_ERROR.format("Server")
        assert client_hello != [], NOT_RUN_ERROR
        assert server_hello != [], NOT_FOUND_ERROR.format("Server")
        return "PASSED"
