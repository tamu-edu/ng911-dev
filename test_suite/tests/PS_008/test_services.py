...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, http_url = get_filter_parameters(
    ...
    if http_responses:
        ...
        http_response = [
            http_resp
            for http_resp in http_responses
            if http_resp.http.get("response_code", None) == "200"
        ][-1]
...
def get_test_names() -> list:
    return [
        "Verify if JWS objects are unaltered after fetching from Policy Store",
    ]
...
def get_test_list(
    ...
    jws_from_request, http_response = get_test_parameters(
