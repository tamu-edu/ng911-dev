from services.aux_services.message_services import (
    get_bidirectional_media,
    get_payload_type_from_media,
)
    ...
    if is_variation1:
        ...
        if chfe_refer_request:
            ...
            media_transfer_esrp_to_t_chfe = get_bidirectional_media(
                "IF_ESRP_TS-CHFE", "IF_TS-CHFE_ESRP", pcap_service, interfaces_dict
            )
            media_transfer_esrp_bcf = get_bidirectional_media(
                "IF_BCF_ESRP", "IF_ESRP_BCF", pcap_service, interfaces_dict
            )
            media_transfer_osp_bcf = get_bidirectional_media(
                "IF_OSP_BCF", "IF_BCF_OSP", pcap_service, interfaces_dict
            )
