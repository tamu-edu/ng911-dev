from services.aux_services.sip_msg_body_services import get_conference_id, extract_media_attributes
from services.config.errors.wrong_configuration_error import WrongConfigurationError
from services.config.types.lab_config import LabConfig
from services.config.types.run_config import MessageFilter, RunVariation
from services.pcap_service import PcapCaptureService, FilterConfig
from services.aux_services.aux_services import get_first_message_matching_filter, get_messages, get_dns_list, \
    get_dns_packets_from_pcap
from enums import PacketTypeEnum, SIPMethodEnum, TransportProtocolEnum
from services.test_services.test_assessment_service import TestCheck
from tests.CHFE_006.checks import verify_routing_via_conference_aware, verify_routing_w_or_wo_b2bua


def get_filter_parameters(lab_config: LabConfig, filtering_options: list[MessageFilter], variation,
                          pcap_service: PcapCaptureService) -> (dict, bool):
    """
...
def get_test_parameters(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    interfaces_dict, is_variation1 = (
        get_filter_parameters(lab_config, filtering_options, variation, pcap_service))
    ...
    if is_variation1:
        ...
        esrp_initial_conference_id = chfe_invite_response_code = \
            chfe_subscribe_request = chfe_conference_id = chfe_request_uri_refer = \
            chfe_refer_to = sip_bye = media_transfer_esrp_to_t_chfe = media_transfer_esrp_bcf = media_transfer_osp_bcf = \
            bridge_call = esrp_to_t_chfe_payload_type = esrp_to_bcf_payload_type = osp_to_bcf_payload_type = \
            bcf_osp_media_type = esrp_to_bcf_media_type = None
        ...
        if esrp_invite_request:
            ...
            esrp_initial_conference_id = get_conference_id(esrp_invite_request.sip.get('contact'))
            ...
            if chfe_invite_response:
                try:
                    chfe_invite_response_code = chfe_invite_response.sip.get('status_code')
            ...
            if chfe_subscribe_request:
                chfe_conference_id = get_conference_id(chfe_subscribe_request.sip.get('to'))
        ...
        if chfe_refer_request:
            ...
            media_transfer_esrp_to_t_chfe = get_bidirectional_media('IF_ESRP_TS-CHFE', 'IF_TS-CHFE_ESRP')
            media_transfer_esrp_bcf = get_bidirectional_media('IF_BCF_ESRP', 'IF_ESRP_BCF')
            ...
            if sum(len(lst) for lst in media_transfer_esrp_to_t_chfe) > 2:
                esrp_to_t_chfe_payload_type = get_payload_type_from_media(media_transfer_esrp_to_t_chfe[0][0])
            if sum(len(lst) for lst in media_transfer_esrp_bcf) > 2:
                esrp_to_bcf_payload_type = get_payload_type_from_media(media_transfer_esrp_bcf[0][0])
            if sum(len(lst) for lst in media_transfer_osp_bcf) > 2:
                osp_to_bcf_payload_type = get_payload_type_from_media(media_transfer_osp_bcf[0][0])
        ...
        if esrp_invite_request_to_t_chfe:
            t_chfe_responses = get_messages(
                ...
                FilterConfig(
                    ...
                    packet_type=PacketTypeEnum.SIP,
                    after_timestamp=float(esrp_invite_request_to_t_chfe.sniff_timestamp)
                )
            )
            ...
            if (interfaces_dict['IF_CHFE_BRIDGE']['ip'] and interfaces_dict['IF_BRIDGE_CHFE']['ip']) or \
                    (interfaces_dict['IF_CHFE_BRIDGE']['ip_list'] and interfaces_dict['IF_BRIDGE_CHFE']['ip_list']):
    ...
    # Variation 2/3 workflow
        ...
        osp_call_id = osp_to = osp_from = chfe_invite_response_code = conf_response = esrp_conference_id_init = \
            chfe_conference_id_bridge = chfe_conference_id_subscribe = chfe_request_uri = chfe_refer_to = chfe_call_id = \
            chfe_to_tag = chfe_from_tag = chfe_cof_id_refer2 = media_transfer_data = media_attr_request_from_chfe_dict = \
            payload_type = None
        ...
        if 'IF_CHFE_OSP' in interfaces_dict.keys():  # Variation number 2
            ...
            if init_sip_chfe_response:
                try:
                    chfe_invite_response_code = init_sip_chfe_response.sip.get('status_code')
                except AttributeError:
                    pass
        ...
        if chfe_invite_request_ts:
            ...
            if conf_response:
                ...
                media_attr_request_from_chfe_dict = extract_media_attributes(conf_response)
        ...
        if chfe_invite_request_bridge:
            ...
            if chfe_invite_request_ok:
                chfe_conference_id_bridge = chfe_invite_request_ok.sip.get('contact')  # TODO tag for conference_id

            # Get media transfers by calling the helper function for each pair of interfaces
            if variation_name == 'Variation2':
                media_transfer_data = get_bidirectional_media('IF_OSP_TS-CHFE', 'IF_TS-CHFE_OSP')
            else:
                media_transfer_data = get_bidirectional_media('IF_BCF_TS-CHFE', 'IF_TS-CHFE_BCF')

            if media_transfer_data:
                if sum(len(lst) for lst in media_transfer_data) > 2:
                    payload_type = get_payload_type_from_media(media_transfer_data[0][0])
...
def get_test_names() -> list:
    return [
        f"Verify routing all calls via a Conference-aware UA",
        f"Verify Ad Hoc without B2BUA in the Ingress Call Path/Ad Hoc with B2BUA in the Ingress Call Path"
            ]
...
def get_test_list(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    variation_name, test_data = get_test_parameters(pcap_service, lab_config, filtering_options, variation)

    if variation_name == 'Variation1':
        (osp_invite_request, chfe_invite_response_code, esrp_initial_conference_id,
         chfe_subscribe_request, chfe_conference_id, chfe_refer_request, chfe_request_uri_refer, chfe_refer_to, sip_bye,
         esrp_notify_msgs, chfe_ok_msgs, media_transfer_esrp_to_t_chfe, media_transfer_esrp_bcf, media_transfer_osp_bcf,
         bcf_osp_media_type, esrp_to_bcf_media_type, media_attr_response_t_chfe, esrp_to_t_chfe_payload_type,
         esrp_to_bcf_payload_type, osp_to_bcf_payload_type, bridge_call) = test_data
        ...
        (osp_invite_request, osp_call_id, osp_to, osp_from, init_sip_chfe_response, chfe_invite_response_code,
         chfe_invite_request_ts, conf_response, esrp_conference_id_init, chfe_invite_request_bridge,
         chfe_conference_id_bridge, chfe_subscribe_request, chfe_conference_id_subscribe, chfe_refer_request,
         chfe_bye_after_refer, chfe_request_uri, chfe_refer_to, chfe_call_id, chfe_to_tag, chfe_from_tag, chfe_sip_refer_2,
         chfe_cof_id_refer2, sip_bye, esrp_notify_msgs, chfe_ok_msgs, media_transfer_data,
         media_attr_request_from_chfe_dict, payload_type) = test_data
