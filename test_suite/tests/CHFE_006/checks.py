def validate_sip_notify_ok_responses_from_chfe(esrp_notify_msgs, chfe_ok_msgs):
    # TODO write method
    chfe_ok_filtered = [
        message for message in chfe_ok_msgs if "NOTIFY" in message.sip.cseq
    ]
...
# Variation 1 check
def verify_routing_via_conference_aware(
    osp_invite_request,
    ...
    try:
        ...
        assert (
            chfe_invite_response_code == "200"
        ), "FAILED -> No 200 OK response code from CHFE found."

        assert chfe_subscribe_request, "FAILED -> CHFE SUBSCRIBE request not found."

        assert (
            esrp_initial_conference_id
        ), "FAILED -> Conference_id from ESRP not found."

        assert (
            chfe_conference_id
        ), "FAILED -> Conference_id from CHFE SUBSCRIBE request not found."
        ...
        assert (
            chfe_request_uri_refer
        ), "FAILED -> CHFE 'Request URI' attribute from REFER request not found."

        assert (
            esrp_initial_conference_id in chfe_request_uri_refer
        ), "FAILED -> 'Conference_id' not in CHFE Request URI."

        assert (
            chfe_refer_to
        ), "FAILED -> Request 'Refer-to' attribute from REFER request not found."

        assert (
            sum(len(lst) for lst in media_transfer_esrp_to_t_chfe) > 2
        ), "FAILED -> No media data exchange between ESRP and TS-CHFE."

        assert (
            sum(len(lst) for lst in media_transfer_esrp_bcf) > 2
        ), "FAILED -> No media data exchange between ESRP and BCF."

        assert (
            sum(len(lst) for lst in media_transfer_osp_bcf) > 2
        ), "FAILED -> No media data exchange between OSP and BCF."

        assert any(
            item in esrp_to_t_chfe_payload_type for item in media_attr_response_t_chfe
        ), "FAILED -> Media payload types doesn't match between ESRP and TS-CHFE."

        assert any(
            item in esrp_to_bcf_payload_type for item in esrp_to_bcf_media_type
        ), "FAILED -> Media payload types doesn't match between ESRP and BCF."

        assert any(
            item in osp_to_bcf_payload_type for item in bcf_osp_media_type
        ...
        assert (
            not bridge_call
        ), "FAILED -> CHFE shouldn't send any requests to the BRIDGE."

        assert validate_sip_notify_ok_responses_from_chfe(
            esrp_notify_msgs, chfe_ok_msgs
...
# Variation 2/3 check
def verify_routing_w_or_wo_b2bua(
    osp_invite_request,
    osp_call_id,
    osp_to,
    osp_from,
    ...
    try:
        assert (
            init_sip_chfe_response
        ), "FAILED -> No CHFE response found for SIP INVITE."
        ...
        assert (
            chfe_invite_request_ts
        ), "FAILED -> No CHFE INVITE to Test Conference App found."

        assert (
            conf_response
        ), "INCONCLUSIVE -> Test Conference App doesn't respond to CHFE."

        assert esrp_conference_id_init, "INCONCLUSIVE -> 'Conference_Id' not found."

        assert (
            chfe_invite_request_bridge
        ), "FAILED -> CHFE INVITE request to BRIDGE not found."

        assert (
            chfe_conference_id_bridge
        ), "FAILED -> 'Conference_Id' not found in SIP INVITE from CHFE to BRIDGE."
        ...
        assert (
            chfe_conference_id_subscribe
        ), "FAILED -> 'Conference_Id' not found in SIP SUBSCRIBE from CHFE to BRIDGE."
        ...
        assert (
            chfe_refer_request
        ), "FAILED -> SIP REFER request from the CHFE not found."
        ...
        assert (
            sum(len(lst) for lst in media_transfer_data) > 2
        ), "FAILED -> No media data exchange found."

        assert any(
            item in media_attr_request_from_chfe_dict for item in payload_type
        ), "FAILED -> Media payload types doesn't match between ESRP and BCF."
