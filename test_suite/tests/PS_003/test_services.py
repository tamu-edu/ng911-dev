...
from tests.PS_003.checks import (
    validate_version_int_values,
    validate_version_service_info,
    validate_version_vendor,
)
...
def get_test_names() -> list:
    return [
        "Verify if 'fingerprint' has string value",
        "Verify if 'version' is an array",
        "Verify if all 'versions' contain 'major' and 'minor' integer values",
        "Verify if all 'versions' contain 'serviceInfo' with 'requiredAlgorithms' array of string values",
        "Verify 'vendor' is present in version and is a string",
    ]
...
def get_test_list(
    ...
    http_response = get_test_parameters(
    ...
    return [
        ...
        TestCheck(
            ...
            test_params={"response": http_response},
        ),
        TestCheck(
