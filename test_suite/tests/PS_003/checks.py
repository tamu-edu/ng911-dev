...
def validate_version_int_values(response):
    ...
    for version in response["versions"]:
        if (
            result := is_type(version.get("major", None), "major", int)
        ) != "PASSED" and not isinstance(version.get("major", None), int):
            return result
        if (
            result := is_type(version.get("minor", None), "minor", int)
        ) != "PASSED" and not isinstance(version.get("major", None), int):
            return result
        return "PASSED"
...
def validate_version_service_info(response):
    ...
    for version in response["versions"]:
        ...
        if (result := is_type(service_info, "serviceInfo", (list, dict))) != "PASSED":
            return result

        if (
            result := is_type(
                service_info.get("requiredAlgorithms", None),
                "requiredAlgorithms",
                (list, dict),
            )
        ) != "PASSED":
        ...
        for algorithm in service_info["requiredAlgorithms"]:
            if (result := is_type(algorithm, "algorithm", str)) != "PASSED":
                return result
    return "PASSED"
