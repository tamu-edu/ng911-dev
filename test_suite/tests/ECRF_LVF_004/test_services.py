...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, payload_path = get_filter_parameters(
    ...
    by_location = (
        True
        if extract_all_values_for_xml_tag_name(payload, "listServicesByLocation")
        else False
    )
    if by_location:
        service_urn = [
            record
            for record in extract_all_values_for_xml_tag_name(
                payload, "listServicesByLocation"
            )
    ...
    try:
        return (
            payload,
            extract_all_xml_bodies_from_message(message)[0],
            service_urn,
            by_location,
        )
    except IndexError:
...
def get_test_names() -> list:
    return [
        "Validate http response for listServices",
    ]
...
def get_test_list(
    ...
    xml_request, xml_response, service_urn, by_location = get_test_parameters(
