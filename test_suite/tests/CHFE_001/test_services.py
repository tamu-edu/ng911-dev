...
from services.aux_services.message_services import (
    get_sip_response_by_attribute_and_attr_value,
)
...
from tests.CHFE_001.checks import verify_http_held, verify_sip_subscribe
...
def get_test_parameters(
    ...
    chfe_responses: list = []
    if not out_response:
        return chfe_responses, geolocation, request_method
...
def get_test_names() -> list:
    return [
        "Validate HTTP POST sent to LIS after receiving SIP INVITE (HELD)",
        "Validate SIP SUBSCRIBE sent to LIS after receiving SIP INVITE (SIP Presence Event Package)",
    ]
