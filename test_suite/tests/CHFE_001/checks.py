...
def verify_http_held(response, geolocation):
    ...
    try:
        if (
            response.http.get("request_full_uri")
            .removeprefix("http://")
            .removeprefix("https://")
            not in geolocation
        ):
    ...
    if response.http.get("request_method") == "POST":
        msg_bodies = [
            c.get("body")
            for c in extract_all_contents_from_message_body(response)
...
def verify_sip_subscribe(**response_data):
    ...
    try:
        ...
        if status_code != "200":
            return "FAILED-> CHFE does not respond for SIP NOTIFY with status code 200"
    except AttributeError:
        return "FAILED-> No 200 OK status code found."
    try:
        subscribe_resp_to_header = str(subscribe_resp.sip.get("to"))
        if (
            response_data["geolocation"].removeprefix("<cid:").removesuffix(">")
            not in subscribe_resp_to_header
        ):
