...
from test_suite.services.aux_services.message_services import extract_json_data_from_http
...
def validate_response_json_for_http_get_to_logevents_entrypoint(container=None, **result_data: dict[str: str]):
    ...
    if not is_unsigned_jws(jws) and not is_signed_by_tracable_pca(jws, trusted_pca_cert):
...
def validate_json_body_for_log_events(**result_data):
    ...
    for element in extract_json_data_from_http(result_data['response'])['logEventContainers']:
