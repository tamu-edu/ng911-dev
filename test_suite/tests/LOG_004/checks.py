...
from services.aux_services.json_services import (
    ...
    is_signed_by_tracable_pca,
)
from services.aux_services.sip_msg_body_services import is_valid_sip_call_id
from tests.LOG_004.constants import (
    LOG_EVENT_TYPES,
    REQUIRED_CALL_ID,
    REQUIRED_INCIDENT_ID,
)

from test_suite.services.aux_services.json_services import is_unsigned_jws
...
def validate_required_types(record_type, log_event, payload_data):
    list_of_required_types = {
        "callId": REQUIRED_CALL_ID,
...
def validate_response_json_for_http_get_to_logevents_entrypoint(
    container=None, **result_data: dict[str:str]
):
    if (
        not result_data["response"]
        or result_data["response"].http.response_code != "200"
    ):
        return "FAILED-> Response code is not 200"
    if container:
        jws = container
    else:
        jws = extract_json_data_from_http(result_data["response"]).replace("\n", "")
    ...
    if not is_unsigned_jws(jws) and not is_signed_by_tracable_pca(
        jws, trusted_pca_cert
    ):
    ...
    if client_assigned_identifier:
        if (
            result := is_type(
                client_assigned_identifier, "clientAssignedIdentifier", str
            )
        ) != "PASSED":
    ...
    for log_record in ("callId", "incidentId", "callIdSIP"):
        if payload_data.get(log_record, None):
            if (
                result := validate_required_types(
                    payload_data["logEventType"], log_record, payload_data
                )
            ) != "PASSED":
                return result
            if log_record != "callIdSIP":
                if (
                    result := validate_identifier(payload_data[log_record], log_record)
                ) != "PASSED":
                    # Test step result FAILED in case of invalid format
                    return result
    ...
    if not re.search(FQDN_PATTERN, ip_address_port) and not validate_ip_port_combo(
        ip_address_port
    ):
    ...
    if payload_data["logEventType"] == "CallSignalingMessageLogEvent":
        ...
        if ip_address_port not in contact:
            return (
                "FAILED-> 'ipAddressPort' doesn't match to value in 'text' "
...
def validate_json_body_for_log_events(**result_data):
    ...
    for key, value in FIELD_TYPES.items():
        try:
            elem = extract_json_data_from_http(result_data["response"])[key]
    ...
    for element in extract_json_data_from_http(result_data["response"])[
        "logEventContainers"
    ]:
        ...
        if (result := validate_identifier(log_event_id, "logid")) != "PASSED":
            # Test step result FAILED in case of invalid format
            return result
        ...
        if (
            result := validate_response_json_for_http_get_to_logevents_entrypoint(
                container=jws, **result_data
            )
        ) != "PASSED":
            # Test step result FAILED in case of invalid format
            return result
