...
def get_filter_parameters(lab_config: LabConfig, filtering_options: list[MessageFilter], variation):
    ...
    host_ip = lab_config.test_suite_host_ip if hasattr(lab_config, 'test_suite_host_ip') else ''
    ...
    if stimulus and request_url_data:
        ...
        if stimulus_src_ip is None or stimulus_dst_ip is None:
            ...
            return (host_ip, stimulus_src_ip, stimulus_dst_ip, request_url_data, pca_file_path, pca_key_path, resp_code,
                    entities_keys)
    else:
...
def get_test_parameters(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    (host_ip, stimulus_src_ip, stimulus_dst_ip, variation_data, pca_file_path, pca_file_cert, exp_resp_code,
    ...
    if 'var.' in variation_data:
        variation_uri = initial_request.http.request_uri if (hasattr(initial_request, 'http')
                                                             and hasattr(initial_request.http, 'request_uri')) else ''
    else:
    ...
    response_code = out_message.http.response_code if (hasattr(out_message, 'http')
                                                       and hasattr(out_message.http, 'response_code')) \
        else ''
...
def get_test_list(pcap_service: PcapCaptureService,
    ...
    (
        http_response_codes_or_response_data, pca_key, cert_file, entity_data, exp_resp_code, variation_data
