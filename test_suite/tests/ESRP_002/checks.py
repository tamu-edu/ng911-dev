def verify_location_and_dereferencing(http_lost_request, dereferenced_geolocation_data):
    ...
    if not hasattr(http_lost_request, "xml") or not http_lost_request.xml.get(
        "cdata", None
    ):
    ...
    if dereferenced_geolocation_data != http_lost_request.xml.get("cdata", None):
        return (
            "FAILED -> Location data doesn't match. HTTP LoST from ESRP does not contain geolocation "
