...
def get_test_parameters(
    ...
    stimulus_message_body_list = extract_all_contents_from_message_body(
        stimulus_message
    )
    output_message_body_list = extract_all_contents_from_message_body(output_message)
    stimulus_geolocation_header_fields = (
        extract_all_header_fields_matching_name_from_sip_message(
            stimulus_message, "Geolocation"
        )
    )
    ...
    output_geolocation_header_fields = (
        extract_all_header_fields_matching_name_from_sip_message(
            output_message, "Geolocation"
        )
    )
    ...
    output_pidf_lo_body_list = [
        body for body in output_message_body_list if is_valid_pidf_lo(body["body"])
    ]
    stimulus_message_xml_json_body_list = [
        body
        for body in stimulus_message_body_list
        if "xml" or "json" in body["Content-Type"]
    ]
    output_message_xml_json_body_list = [
        body
        for body in output_message_body_list
        if "xml" or "json" in body["Content-Type"]
    ]
    return [
        stimulus_message,
...
def get_test_list(
    ...
    return [
        TestCheck(
            test_name="Adding default PIDF-LO.",
        ...
        TestCheck(
            test_name="Adding 'Geolocation' pointing to default PIDF-LO.",
        ...
        TestCheck(
            test_name="Adding 'Geolocation' as top-most entry in 'Geolocation' section.",
            test_method=test_adding_header_field_on_top_of_its_section,
            test_params={
                "stimulus": stimulus_message,
                "output": output_message,
                "header_field_name": "Geolocation",
            },
        ),
        TestCheck(
            test_name="Keeping original 'Geolocation' header field.",
        ...
        TestCheck(
            test_name="Keeping original message bodies.",
