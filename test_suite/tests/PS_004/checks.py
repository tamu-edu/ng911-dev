...
def validate_policy_store_response_body(**response_data: dict[str:str]):
    """
    Validation of policyStore response body for 200 response code.
    ...
    try:
        policy_store_data = extract_json_data_from_http(response_data["response"])
    except JSONDecodeError:
        return "FAILED ->'Invalid JSON response"

    # Validate that 'PolicyArray' is an array
    if (
        result := is_type(policy_store_data["PolicyArray"], "PolicyArray", (list, dict))
    ) != "PASSED":
    ...
    for policy_element in policy_store_data["PolicyArray"]:
        try:
            ...
            if (
                result := is_type(policy_element.get("count", None), "count", int)
            ) != "PASSED":
                return result
            assert policy_element["count"] == len(
                policy_element["policies"]
            ), "FAILED ->'PolicyArray' has different number of policies"

            # Validate 'totalCount' is int
            if (
                result := is_type(
                    policy_element.get("totalCount", None), "totalCount", int
                )
            ) != "PASSED":
                return result
            ...
            if (
                result := is_type(
                    policy_element.get("policies", None), "policies", (list, dict)
                )
            ) != "PASSED":
                return result
            for policy in policy_element["policies"]:
                assert is_jws(policy), "FAILED ->'Invalid JWS token"
                return "PASSED"
