...
def get_test_names() -> list:
    return [
        "Validate 4xx error response code is sent.",
...
def get_test_list(
    ...
    ok_response_variation, test_response, variation_name, expected_response_code = (
        get_test_parameters(pcap_service, lab_config, filtering_options, variation)
    )
    if not ok_response_variation:
        return [
            TestCheck(
                test_name="Validate 4xx error response code is sent.",
        ...
        return [
            TestCheck(
                test_name="Validate response message.",
                test_method=validate_policy_store_response_body,
                test_params={"response": test_response},
            )
        ]
