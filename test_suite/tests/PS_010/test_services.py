...
from tests.PS_010.checks import validate_policy_created_successfully, validate_expired_policy_response
...
def get_filter_parameters(lab_config: LabConfig, filtering_options: list[MessageFilter], variation) \
    ...
    for param in variation.params:
        if param == 'messages':
            for message in variation.params[param]:
                variation_file_path = message['prep_steps'][-1]['kwargs']['output_file'].replace('file.', '') \
                    if message.get('prep_steps', None) and message['prep_steps'][-1].get('kwargs', None) \
                       and message['prep_steps'][-1]['kwargs'].get('output_file', None) else None

    for message in filtering_options:
        if message.message_type == FilterMessageType.STIMULUS:
            stimulus = message
            exp_resp_code_expired = message.response_status_code if hasattr(message, 'response_status_code') \
                else exp_resp_code_expired
        elif message.message_type == FilterMessageType.OTHER:
            output = message
            exp_resp_code_policy_created = message.response_status_code if hasattr(message, 'response_status_code') \
    ...
    if stimulus and output:
        ...
        if not any([stimulus_src_ip, stimulus_dst_ip, out_scr_ip, out_dst_ip, variation_file_path, key_filepath]):
            ...
            return (stimulus_src_ip, stimulus_dst_ip, out_scr_ip, out_dst_ip, exp_resp_code_policy_created,
                    exp_resp_code_expired, variation_file_path, key_filepath)
    else:
...
def get_test_parameters(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    (stimulus_src_ip, stimulus_dst_ip, out_scr_ip, out_dst_ip, exp_resp_code_policy_created, exp_resp_code_expired,
     variation_file_path, key_filepath) = get_filter_parameters(lab_config, filtering_options, variation)
    ...
    if payload_data:
        timestamp_from_payload = iso_to_timestamp(payload_data.get('policyExpirationTime', ''))
    ...
    if send_policy:
        ...
        for message in ps_responses:
            response_code = message.http.response_code \
    ...
    if expired_get_request:
        ...
        is_get_request_time_correct = True \
            if timestamp_from_payload and expired_get_request > timestamp_from_payload else False
    ...
    return (is_get_request_time_correct, send_policy, policy_created_status_code, exp_resp_code_policy_created,
            expired_get_request, policy_expired_status_code, exp_resp_code_expired)


def get_test_names() -> list:
    return [
        f"Verify policy successfully created by Policy Store",
        f"Verify that Policy Store does not provide expired policies",
    ]
