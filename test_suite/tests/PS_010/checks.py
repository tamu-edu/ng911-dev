...
def validate_policy_created_successfully(
    send_policy, policy_created_status_code, exp_resp_code_policy_created
):
    try:
        ...
        ), "INCONCLUSIVE - No '201 Policy Successfully Created' response code not found"
        assert policy_created_status_code == exp_resp_code_policy_created, (
            f"INCONCLUSIVE - PS response code:{policy_created_status_code} doesnt match expected "
            f"response code:{exp_resp_code_policy_created}"
        )
        return "PASSED"
...
def validate_expired_policy_response(
    ...
    try:
        ...
        assert (
            policy_expired_status_code
        ), "FAILED - No '4XX' response code from Policy Store not found"
        assert (
            result := validate_response_code_class(
                exp_resp_code_expired, policy_expired_status_code
            )
        ) == "PASSED", result
        return "PASSED"
    except (AssertionError, ValueError) as e:
