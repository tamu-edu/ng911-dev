...
from tests.CHFE_005.checks import validate_chfe_response_code, validate_chfe_response
...
def get_test_names() -> list:
    return [
        "Validate CHFE response for SIP INVITE",
        "Validate CHFE response code for SIP INVITE",
    ]
...
def get_test_list(
    ...
    response_code, expected_media_attrs, actual_media_attrs = get_test_parameters(
        pcap_service, lab_config, filtering_options, variation
    )

    return [
        TestCheck(
            test_name="Validate CHFE response for SIP INVITE",
            test_method=validate_chfe_response,
            test_params={
        ...
        TestCheck(
            test_name="Validate CHFE response code for SIP INVITE",
            test_method=validate_chfe_response_code,
            test_params={"response_code": response_code},
        ),
    ]
