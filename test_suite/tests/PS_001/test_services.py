...
from services.aux.aux_services import check_value_for_file_var_mode
    ...
    for param in variation.params:
        if param == 'messages':
            for message in variation.params['messages']:
                ...
                variation_file = message.get('body', None).strip('file.')
...
def get_test_list(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    return [
        TestCheck(
            test_name=f"Verify error code 4xx for: {variation_name}",
