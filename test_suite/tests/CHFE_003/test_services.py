...
def get_test_parameters(pcap_service: PcapCaptureService, lab_config: LabConfig,
    ...
    for message in sip_invite_messages_responses:
        if request_method == 'send':
            ...
            if hasattr(message.sip, 'status_code') and hasattr(message.sip, 'cseq_method'):
                if message.sip.status_code == '200' and message.sip.cseq_method == 'INVITE':
    ...
    try:
        lang_attrs = sip_invite_messages_response.sip.get('sdp_media_attribute_value', None)
        if lang_attrs:
...
def get_test_names() -> list:
    return [
        f"Validate SDP answer from CHFE.",
        f"Validate SDP offer from CHFE for outgoing calls",
    ]
