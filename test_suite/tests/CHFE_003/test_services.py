...
from tests.CHFE_003.checks import validate_language_response
...
def get_filter_parameters(
    ...
    if stimulus:
        ...
        raise WrongConfigurationError(
            "It seems that the RunConfig does not contain required"
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, expected_response_code, request_method = (
    ...
    sip_invite_messages_responses = get_messages(
        pcap_service,
        FilterConfig(
    ...
    sip_invite_messages_response = None

    for message in sip_invite_messages_responses:
        if request_method == "send":
            # Filter out SIP 200 OK INVITE message fom all CHFE response messages
            if hasattr(message.sip, "status_code") and hasattr(
                message.sip, "cseq_method"
            ):
                if (
                    message.sip.status_code == "200"
                    and message.sip.cseq_method == "INVITE"
                ):
                    sip_invite_messages_response = message
                    continue
        else:
            # Filter out SIP INVITE message fom all CHFE response messages to any other IP not from Test System
            if hasattr(message.sip, "cseq_method"):
                if message.sip.cseq_method == "INVITE":
                    sip_invite_messages_response = message
                    continue
    ...
    try:
        lang_attrs = sip_invite_messages_response.sip.get(
            "sdp_media_attribute_value", None
        )
        if lang_attrs:
            return (
                all([lang in bcp47.tags for lang in lang_attrs.split(" ")]),
                request_method,
            )
...
def get_test_names() -> list:
    return [
        "Validate SDP answer from CHFE.",
        "Validate SDP offer from CHFE for outgoing calls",
    ]
...
def get_test_list(
    ...
    is_languages_included, request_method = get_test_parameters(
        pcap_service, lab_config, filtering_options, variation
    )

    if request_method == "send":
        test_name = "Validate SDP answer from CHFE."
    else:
        test_name = "Validate SDP offer from CHFE for outgoing calls"
