        ...
        if param == "messages":
            for message in variation.params["messages"]:
                if message.get("action", None) == "send":
                    ...
                    request_payload_path = message.get("body", None).removeprefix(
                        "file."
                    )
...
def get_test_parameters(
    ...
    stimulus_message_xml = "".join(
        get_payload_data_from_file(request_payload_path, "plain").split()
    )

    stimulus_response_message = (
        get_http_response_containing_string_in_xml_body_for_message_matching_filter(
            pcap_service,
            FilterConfig(
    ...
    if output_src_ip and output_dst_ip and not expected_response_code:
        ...
        def get_first_xml(message):
            xmls = extract_all_xml_bodies_from_message(message) or []
            if not xmls:
                return None
            return "".join(xmls[0].split)
            # try:
            #     return extract_all_xml_bodies_from_message(message)[0]
            # except IndexError:
            #     return None

        output_message_xml = get_first_xml(output_message)
        output_message_response_xml = get_first_xml(output_message_response)
        stimulus_message_response_xml = get_first_xml(stimulus_response_message)

        return (
            expected_response_code,
            stimulus_message_xml,
            stimulus_message_response_xml,
            None,
            None,
            output_message_xml,
            output_message_response_xml,
        )
    else:
        stimulus_message_response_status_code = None
        stimulus_message_response_location_url = None
        if stimulus_response_message:
            stimulus_message_response_status_code = (
                stimulus_response_message.http.response_code
            )
            stimulus_message_response_location_url = stimulus_response_message.http.get(
                "location"
            )
        return (
            expected_response_code,
            ...
            None,
            stimulus_message_response_status_code,
            stimulus_message_response_location_url,
            None,
            None,
        )
...
def get_test_names() -> list:
    return [
        "Validate sending HTTP LoST request to another ECRF",
        "Validate sending back HTTP LoST response from another ECRF",
        "Validate if LoST response contain URL",
    ]
