    ...
    for param in variation.params:
        if param == "messages":
            for message in variation.params["messages"]:
                request_payload_path = message.get("body", "").removeprefix("file.")
                request_url = message.get("http_url", "")
    if stimulus:
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, uri, payload_path, exp_resp_code = (
    ...
    response_code = (
        ...
        if out_message and hasattr(out_message, "http")
        else out_message
    )
...
def get_test_names() -> list:
    return [
        "Validate 4xx error response for invalid request",
...
def get_test_list(
    ...
    response_code, expected_response_code = get_test_parameters(
