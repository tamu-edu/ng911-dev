...
def get_test_parameters(
    ...
    if osp_invite_request:
        emergency_call_data_list = extract_sip_header_values(
            osp_invite_request, "Call-Info", "purpose=EmergencyCallData"
        )
    ...
    if esrp_get_policies_request:
        ...
        if ps_response_mgs:
            for msg in ps_response_mgs:
                if (
                    hasattr(msg, "http")
                    and hasattr(msg.http, "response_code")
                    and msg.http.response_code == "200"
                ):
                    if hasattr(msg, "xml"):
    ...
    if esrp_to_chfe_request:
        esrp_emergency_call_data_list = extract_sip_header_values(
            esrp_to_chfe_request, "Call-Info", "purpose=EmergencyCallData"
        )

    return (
        emergency_call_data_list,
        bcf_invite_request,
        esrp_get_policies_request,
        is_jws_in_response_valid,
        esrp_get_adr_request,
        esrp_emergency_call_data_list,
    )
...
def get_test_names() -> list:
    return [
        "Validate ESRP for retrieving Additional Data.",
    ]
