...
def validate_timestamp(actual_timestamp, expected_timestamp, **kwargs) -> str:
    ...
    try:
        ...
        assert round(abs(ts_float - expected_timestamp), 2) <= TIMESTAMP_THRESHOLD, (
...
def validate_optional_log_event_fields(raw_event, optional_fields) -> str:
    """Validates optional log event string fields and ipAddressPort if present."""
    try:
        assert raw_event, "FAILED -> No log event data found."
        for field_name, field_type in optional_fields.items():
            ...
            if field_value is not None:
                assert isinstance(field_value, field_type), (
                    f"FAILED -> '{field_name}' must be a {field_type.__name__} when present, "
        ...
        if cause is not None:
            assert isinstance(
                cause, str
            ), "FAILED -> 'cause' must be a string when present"
            assert is_valid_route_cause(
                cause
            ), "FAILED -> 'cause' must be a string in the RouteCause Registry (NENA-STA-010.3, Section 10.20)"
        return "PASSED"
    except AssertionError as e:
        return str(e)
