...
from checks.log_events.log_event_services import validate_log_event_timestamp
...
def get_test_list(
    ...
    return [
        ...
        TestCheck(
            ...
            test_method=is_data_present,
            test_params={
                "test_data": lost_response_data.direction in ["incoming", "outgoing"],
                "error": f"FAILED -> 'direction' attribute should be 'incomming' or 'outgoing'."
                f"Actual: {lost_response_data.direction}",
            },
        ),
        TestCheck(
