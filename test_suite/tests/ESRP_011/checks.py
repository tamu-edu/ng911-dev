...
def validate_mime_integrity(stimulus_message, output_message) -> str:
    ...
    :param stimulus_message: Messages from BCF to ESRP
    :param output_message: Messages from ESRP to CHFE
    :return: "PASSED" if validation passes, error message otherwise
    """
    try:
        assert (
            stimulus_message
        ), "NOT RUN -> No stimulus messages from BCF to ESRP found"
        assert output_message, "FAILED -> No output messages from ESRP to CHFE found"
