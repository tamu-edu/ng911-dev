...
def validate_supported_media_type(response_code, expected_response_code, expected_media_attrs, actual_media_attrs):
    try:
        ...
        assert str(response_code) == str(expected_response_code), "FAILED -> Wrong CHFE response code."

        assert actual_media_attrs, "FAILED -> No media information present in CHFE response."
