...
def validate_supported_media_type(
    ...
    try:
        ...
        assert (
            actual_media_attrs
        ), "FAILED -> No media information is found in CHFE response."

        assert str(response_code) == str(
            expected_response_code
        ), "FAILED -> Wrong CHFE response code."

        assert (
            actual_media_attrs
        ), "FAILED -> No media information present in CHFE response."
        ...
        for media_attr in expected_media_attrs:
            ...
            if media_attr in actual_media_attrs:
                ...
                assert is_having_similar_attrs(
                    media_attr, actual_media_attrs
