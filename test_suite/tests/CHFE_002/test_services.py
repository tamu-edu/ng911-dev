...
from tests.CHFE_002.checks import validate_supported_media_type
...
def get_test_parameters(
    ...
    if sip_invite_message:
        try:
            ...
            return None, int(expected_response_code), None, None
        ...
        if not messages_after_timestamp:
            return None, int(expected_response_code), invite_media_attrs, None
...
def get_test_names() -> list:
    return [
        "Validate SIP 200 OK response for SIP INVITE",
    ]
...
def get_test_list(
    ...
    (
        ...
        expected_media_attrs,
        actual_media_attrs,
    ) = get_test_parameters(pcap_service, lab_config, filtering_options, variation)

    return [
        TestCheck(
            test_name="Validate SIP 200 OK response for SIP INVITE",
            test_method=validate_supported_media_type,
            test_params={
