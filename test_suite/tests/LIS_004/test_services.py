...
from tests.LIS_004.checks import (
    validate_lis_response_time_between_messages,
...
def get_filter_parameters(
    ...
    for message in variation.params.get("messages"):
        ...
        scenario_file_path = (
            sipp_scenario.get("scenario_file_path").removeprefix("file.")
            if sipp_scenario
            else None
        )
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, expected_resp_code, scen_filepath = (
    ...
    if expected_resp_code and expected_resp_code == "4xx" and sip_subscribe_message:
        ...
        if (
            response
            and hasattr(response, "sip")
            and hasattr(response.sip, "status_code")
        ):
            lis_response_code = response.sip.status_code

    return (
        sip_subscribe_message,
        sip_notify_messages,
        expected_resp_code,
        lis_response_code,
        variation_name,
...
def get_test_names() -> list:
    return [
        "Validate LIS response with 4XX error code",
        "Validate LIS response time between SIP NOTIFY messages",
    ]
