...
from tests.BRIDGE_004.checks import (
...
from test_suite.services.aux_services.sip_message_services import (
...
def get_test_parameters(
    ...
    is_direction_incoming_wrong_value = False
    is_text = True
                ...
                if payload_data:
                    ...
                    if log_event_type == "CallStartLogEvent":
                        ...
                        def get_request_line_and_call_id(raw_message_text: str):
                            if raw_message_text is None or not isinstance(
                        ...
                        def matches_incoming_signaling(
                            payload_request_line,
                            payload_call_id,
                            reference_message_text,
                        ):
                            ref_request_line, ref_call_id = (
                                get_request_line_and_call_id(reference_message_text)
                            )
                            return (
                                ref_request_line
                                and ref_call_id
                                and clean_up_string(payload_request_line)
                                == clean_up_string(ref_request_line)
                                and clean_up_string(payload_call_id)
                                == clean_up_string(ref_call_id)
                            )
                        ...
                        if (
                            ...
                            payload_request_line, payload_call_id = (
                                get_request_line_and_call_id(
                                    payload_data.get("text", None)
                                )
                            )
                            if payload_request_line:
                                if "invite" in payload_request_line:

                                    if matches_incoming_signaling(
                                        payload_request_line,
                                        payload_call_id,
                                        invite_message_text,
                                    ):
                                        call_signalling_message_1 = payload_data
                                elif "bye" in payload_request_line:
                                    if matches_incoming_signaling(
                                        payload_request_line,
                                        payload_call_id,
                                        bye_message_text,
                            ...
                            is_direction_incoming_wrong_value = True
    ...
    return (
        ...
        bye_message_text,
        bye_message_call_id,
        is_direction_incoming_wrong_value,
        is_text,
    )
    ...
    return [
        TestCheck(
            ...
            test_params={
                ...
                "is_direction_incoming_wrong_value": is_direction_incoming_wrong_value,
            },
        ),
        TestCheck(
