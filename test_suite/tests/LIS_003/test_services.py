...
from tests.LIS_003.checks import (
    validate_lis_responses_distance_filter,
    validate_lis_responses_speed_filter,
    validate_lis_responses_element_val_change_filter,
    validate_lis_responses_entering_area_change_filter,
...
def get_variation(message):
    ...
    if "dyn:speed" in message:
        return "speed_filter"
    if "ca:country" in message:
        return "element_value_filter"
    if "enterOrExit" in message:
        return "entering_area_filter"
    if "locationType" in message:
...
def get_filter_parameters(
    ...
    if stimulus:
        ...
        if stimulus_src_ip is None or stimulus_dst_ip is None:
            ...
            return stimulus_src_ip, stimulus_dst_ip, variation_file_path
...
def get_test_parameters(
    ...
    stimulus_src_ip, stimulus_dst_ip, variation_file_path = get_filter_parameters(
    ...
    try:
        with open(str(variation_file_path or ""), "r", encoding="utf-8") as f:
            variation_file_text = f.read()
    except (FileNotFoundError, PermissionError, UnicodeDecodeError):
        variation_file_text = "dyn:speed"
    variation_name = get_variation(variation_file_text)
...
def get_test_names() -> list:
    return [
        "Validate LIS location filters by distance",
        "Validate LIS speed filters by distance",
        "Validate LIS elements value change filters by distance",
        "Validate LIS entering area filters by distance",
        "Validate LIS location type filters by distance",
    ]
...
def get_test_list(
    ...
    sip_subscribe, sip_notify_messages, variation_name = get_test_parameters(
    ...
    if variation_name == "location_type_filter":
        return [
            TestCheck(
                ...
                test_params={"responses": sip_notify_messages},
            )
        ]
    return []
