...
from services.aux_services.locations_services import (
    is_valid_distance_between_points,
    is_valid_speed_between_messages,
    extract_shape_data,
    is_point_in_shape,
)
...
def validate_lis_responses_element_val_change_filter(**response_data):
    ...
    if not subscribe_msg:
        return "FAILED -> No SIP SUBSCRIBE message found."
    ...
    elem_values_list = []
    for message in messages:
        elem_values_list.append(
            extract_tag_content(extract_message_data(message), "country")
        )
