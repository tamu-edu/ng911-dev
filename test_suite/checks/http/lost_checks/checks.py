from services.aux_services.message_services import extract_sip_uri_from_text
from services.aux_services.xml_services import (
    is_valid_xml,
    extract_all_values_for_xml_tag_name,
    extract_location_id_from_text,
    is_http_lost_expired,
)


def test_http_lost_list_services_response(
    stimulus_xml: str, output_xml: str, service_urn: str, by_location=True
):
    """
    ...
    try:
        ...
        for service in extract_all_values_for_xml_tag_name(output_xml, "serviceList"):
            assert (
                service_urn in service
            ), f"FAILED -> service {service} does not match {service_urn}"
        if by_location:
            assert extract_location_id_from_text(
                stimulus_xml
            ) == extract_location_id_from_text(
                output_xml
            ), "FAILED -> locationUsed ID in response does not match location ID from request"
        return "PASSED"
...
def test_http_lost_find_service_response(stimulus_xml: str, output_xml: str):
    ...
    try:
        ...
        assert (
            "<findServiceResponse" in output_xml
        ), "FAILED -> findServiceResponse not found"
        assert (
            is_http_lost_expired(output_xml) is False
        ), "FAILED -> HTTP LoST message is expired"
        assert extract_all_values_for_xml_tag_name(
            stimulus_xml, "service"
        ) == extract_all_values_for_xml_tag_name(
            output_xml, "service"
        ), "FAILED -> 'service' from findServiceResponse does not match findService request"
        assert extract_all_values_for_xml_tag_name(
            output_xml, "serviceBoundary"
        ...
        service_number = extract_all_values_for_xml_tag_name(
            output_xml, "serviceNumber"
        )
        if service_number:
...
def test_http_lost_find_service_response_with_sip_uri_check(
    stimulus_xml: str, output_xml: str, expected_sip_uri_list: list
):
    """
    Test to validate findServiceResponse XML body from output message which is a response for stimulus_xml
    ...
    """
    try:
        test_find_service_response_result = test_http_lost_find_service_response(
            stimulus_xml, output_xml
        )
...
def test_if_geolocation_included_in_find_service(
    parameter_value: str, expected_value: str
):
    """
    Test validating if findService request contains geolocation from received SIP INVITE
    ...
    try:
        assert (
            parameter_value
        ), "FAILED -> geolocation not found in HTTP GET message from ESRP to ECRF"
        assert (
            expected_value
        ), "FAILED -> geolocation not found in initial SIP INVITE message to ESRP"
        assert (
            parameter_value == expected_value
        ), f"FAILED -> expected geolocation value: '{expected_value}' doesn't match to {parameter_value}"
        return "PASSED"
    except AssertionError as e:
        return str(e)
