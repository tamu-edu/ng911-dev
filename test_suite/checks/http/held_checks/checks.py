...
from services.aux_services.xml_services import (
    is_valid_xml,
    extract_xml_expiration_time_as_timestamp,
...
def test_if_location_response_contains_correct_location_uri(output_xml: str):
    ...
    try:
        ...
        assert extract_sip_uri_from_text(
            str(extract_all_values_for_xml_tag_name(output_xml, "locationURI"))
        ) or is_valid_http_https_url(
            extract_all_values_for_xml_tag_name(output_xml, "locationURI")[0]
        ), "FAILED -> locationURI is missing or has incorrect URL"
        return "PASSED"
    except AssertionError as e:
        return str(e)


def test_location_response_expiration_time(
    output_xml: str, output_message_timestamp: float
):
    """
    ...
    try:
        ...
        assert xml_timestamp > (
            output_message_timestamp + (30 * 60)
        ), "FAILED -> expiration time of XML message is lower than 30min"
        assert xml_timestamp < (
            output_message_timestamp + (24 * 60 * 60)
