...
def validate_response_code_class(expected_response_code_class, response_code):
    ...
    try:
        assert expected_response_code_class, f"FAILED-> Response code class cannot be None"
        assert response_code, f"FAILED-> Response code cannot be None"
        code_class_first_digit = str(expected_response_code_class)[0]
        assert bool(re.match(pattern_template.format(prefix=code_class_first_digit), response_code)), \
