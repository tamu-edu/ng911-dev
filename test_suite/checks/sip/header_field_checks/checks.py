...
def test_keeping_original_header_field(stimulus, output, header_field_name: str):
    ...
    """
    for name, input_data in [('Stimulus header fields data', stimulus),
    ...
    try:
        assert extract_all_header_fields_matching_name_from_sip_message(stimulus, header_field_name) <= \
            extract_all_header_fields_matching_name_from_sip_message(output, header_field_name), \
...
def test_adding_header_field_on_top_of_its_section(stimulus, output, header_field_name: str):
    """
    ...
    stimulus_headers = extract_all_header_fields_matching_name_from_sip_message(stimulus,header_field_name)
    output_headers = extract_all_header_fields_matching_name_from_sip_message(output, header_field_name)

    try:
        assert all(header_field_name in header for header in output_headers), \
            f"FAILED -> {header_field_name} header fields not found in output message"
        assert stimulus_headers != output_headers, f"FAILED -> Not found any added header fields"
        assert all(header in output_headers for header in stimulus_headers), \
            f"FAILED -> Some {header_field_name} header fields were removed in output message"
        assert output_headers[0] not in stimulus_headers, f"FAILED -> Top-most {header_field_name} is not added item"
        return "PASSED"
    except AssertionError as e:
        return str(e)
