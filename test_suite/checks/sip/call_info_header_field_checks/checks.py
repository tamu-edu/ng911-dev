...
def test_emergency_call_id_header(emergency_call_id_header):
    ...
    try:
        ...
        ), "FAILED to find Emergency Call Identifier header"
        return "PASSED"
...
def test_emergency_call_id_urn(emergency_call_id_header) -> str:
    ...
    try:
        ...
        if isinstance(emergency_call_id_header, str):
            assert re.search(
                EMERGENCY_IDENTIFIER_URN_PATTERN, emergency_call_id_header
            ), "FAILED - Emergency Call Identifier URN wrong pattern"
        if isinstance(emergency_call_id_header, list):
            assert any(
                [
                    True
                    for record in emergency_call_id_header
...
def test_emergency_call_id_string_id(emergency_call_id_header):
    ...
    try:
        ...
        if isinstance(emergency_call_id_header, str):
            string_id = extract_header_value_by_separator(
                emergency_call_id_header, ":", -2
            )
            assert re.search(
                STRING_ID_PATTERN, string_id
            ), "FAILED - Emergency Call Identifier URN wrong pattern"
...
def test_emergency_call_id_fqdn(emergency_call_id_header):
    ...
    try:
        ...
        if isinstance(emergency_call_id_header, str):
            ...
            assert re.search(
                FQDN_PATTERN, fqdn
            ), "FAILED - Emergency Call Identifier FQDN"
        ...
        if isinstance(emergency_call_id_header, list):
            assert any(
                [
                    True
                    for record in emergency_call_id_header
                    if re.search(FQDN_PATTERN, record)
                ]
            ), "FAILED - Emergency Call Identifier FQDN"
...
def test_incident_tracking_id_urn(incident_tracking_id_header):
    ...
    try:
        ...
        ), "FAILED to find Incident Tracking Identifier header"
        if isinstance(incident_tracking_id_header, str):
            assert re.search(
                INCIDENT_TRACKING_IDENTIFIER_URN_PATTERN, incident_tracking_id_header
            ), "FAILED - Incident Tracking Identifier URN wrong pattern"

        if isinstance(incident_tracking_id_header, list):
            assert any(
                [
                    True
                    for record in incident_tracking_id_header
                    if re.search(FQDN_PATTERN, record)
                ]
            ), "FAILED - Incident Tracking Identifier URN wrong pattern"
...
def test_incident_tracking_id_string_id(incident_tracking_id_header):
    ...
    try:
        ...
        if isinstance(incident_tracking_id_header, str):
            string_id = extract_header_value_by_separator(
                incident_tracking_id_header, ":", -2
            )
            assert re.search(
                STRING_ID_PATTERN, string_id
            ), "FAILED - Incident Tracking Identifier String ID"
...
def test_incident_tracking_id_fqdn(incident_tracking_id_header):
    ...
    try:
        ...
        if isinstance(incident_tracking_id_header, str):
            fqdn = extract_header_value_by_separator(
                incident_tracking_id_header, ":", -1
            )
            assert re.search(
                FQDN_PATTERN, fqdn
            ), "FAILED - Incident Tracking Identifier FQDN"
...
def test_call_info_header_contains_correct_purpose(
    parameter_value: list, expected_purpose: str
):
    """
    ...
    try:
        ...
        assert any(
            [
                True
                for purpose in all_matches
                if expected_purpose.lower()
                == purpose.replace("\\r", "").replace("\\n", "").lower()
            ]
        ), f"FAILED -> Call-Info header field does not contain '{expected_purpose}' purpose"
