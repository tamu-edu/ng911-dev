...
def test_keeping_original_message_bodies(
    stimulus_message_body_list, output_message_body_list
):
    """
    ...
    """
    try:
        assert stimulus_message_body_list, "FAILED -> No stimulus message found"

        assert output_message_body_list, "FAILED -> No output messages found"

        assert [
            body
            for body in stimulus_message_body_list
            if body in output_message_body_list
...
def test_adding_default_pidf_lo(
    stimulus_message_body_list: list, output_message_body_list: list
):
    """
    ...
    try:
        ...
        ), "FAILED -> message bodies not found in outgoing message"
        new_message_bodies = [
            body
            for body in output_message_body_list
            if body not in stimulus_message_body_list
        ]
        assert new_message_bodies, "FAILED -> not found added message bodies"
        assert any(
            is_valid_pidf_lo(body["body"]) for body in new_message_bodies
...
def test_if_xml_body_list_contains_pidf_lo_location(message_xml_body_list: list):
    ...
    """
    try:
        assert message_xml_body_list, "FAILED -> No messages have been found"

        assert message_xml_body_list, "FAILED -> failed to find any XML body in message"
        assert any(
            is_valid_pidf_lo(body["body"]) for body in message_xml_body_list
