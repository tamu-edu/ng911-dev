...
def is_data_absent(test_data, error="FAILED -> Unexpected data found"):
    """
    Asserts that the provided data is falsy - an inverse of 'is_data_present'.
    :param test_data: data expected to be absent
    :param error: error message if data is present
    :return: "PASSED" or an error message
    """
    try:
        assert not test_data, f"{error}\n{test_data}"
        return "PASSED"
    except AssertionError as e:
        return str(e)
...
def verify_str_test_data_the_same_with_normalization_fallback(
    expected_data, actual_data, error=None
):
    """
    Asserts equality, trying an exact match first and falling back to a
    whitespace-normalized comparison (via normalize_string_for_comparison)
    ...
    normalized_expected = normalize_string_for_comparison(expected_data)
    normalized_actual = normalize_string_for_comparison(actual_data)

    return is_test_data_the_same(normalized_expected, normalized_actual, error=error)
...
def verify_value_present_in_collection(actual_data, allowed_values, error=None) -> str:
    """
    Asserts that actual_data is present in the given collection of
    allowed values.

    :param actual_data: The value to check for membership.
    :param allowed_values: Iterable collection of allowed values.
    :param error: Optional custom error message prefix on mismatch.
    :raises AssertionError: If actual_data is falsy, allowed_values is
        not iterable, or actual_data is not found in allowed_values.
    """
    try:
        assert actual_data is not None, "FAILED -> actual data cannot be None."
        ...
        try:
            ...
            raise AssertionError(
                f"FAILED -> 'allowed_values' is not iterable, got {type(allowed_values).__name__}."
            )

        if not error:
            error = (
                f"FAILED -> Value not in the list.\n"
                f"***Actual***:\n{actual_data}\n"
                f"***Allowed***:\n{allowed_values}"
            )
        assert actual_data in allowed_values, error
