...
from api_utils import (
    ...
    logger_setup,
    stop_logger,
    generate_forward_conduit_config,
    check_forward_conduits_presence,
)
from logger.error_parser_service import ErrorParserService
...
def decode_jws(
    ...
    try:
        ...
        parsed_error = ErrorParserService(e, note="Method: decode_jws")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
        raise
...
def validate_configs(
    ...
    try:
        ...
        parsed_error = ErrorParserService(e, note="Method: validate_configs")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
...
def generate_report(
    ...
    try:
        ...
        parsed_error = ErrorParserService(e, note="Method: generate_report")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
...
def _gen_rc(
    ...
    try:
        _format = os.path.splitext(gen_rc)[1].lstrip(".").lower()
        if not _format or os.path.isdir(gen_rc):
            print(f"❌ RUN CONFIG generation failed: missing filename in '{gen_rc}'")
            return
        run_config_service = RunConfigService.from_launch_config(
            launch_config, test, lab_config
        )
        run_config_service.generate_run_config_file(file_format=_format, path=gen_rc)
        print("✅ RUN CONFIG generated successfully.")
    except Exception as e:
        parsed_error = ErrorParserService(e, note="Method: _gen_rc")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
...
def _use_rc(
    ...
    try:
        ...
        is_all_data_present, comment = (
            run_config_service.verify_interface_data_presence(lab_config_interfaces)
        )
        ...
        parsed_error = ErrorParserService(e, note="Method: _use_rc")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
        raise
...
def create_rc(launch_config: LaunchConfig, test: LaunchTest, lab_config: LabConfig):
    try:
        ...
        parsed_error = ErrorParserService(e, note="Method: create_rc")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
        raise
...
def run(launch_config_file: str, use_rc: str | None = None, gen_rc: str | None = None):
    ...
    try:
        if launch_config_file is None:
            print(
                "❌ RUN CONFIG generation failed: launch_config is empty — use --launch_config <path>"
            )
            return
        ...
        parsed_error = ErrorParserService(e, note="Method: run (setup)")
        parsed_error.pretty_print()
        parsed_error.send_to_log()
        return
    ...
    for test in launch_config.tests:
        try:
            ...
            if launch_config.global_config.type == "interoperability":
                forward_conduit_config = generate_forward_conduit_config(lab_config)
                lab_config.generate_prx_ifs(
                    forward_conduit_config=forward_conduit_config
                )
                check_forward_conduits_presence(lab_config, forward_conduit_config)
            ...
            parsed_error = ErrorParserService(e, note=f"Method: run (test {test_id})")
            parsed_error.pretty_print()
            parsed_error.send_to_log()
            continue
