...
from services.config.forward_conduit_service import ForwardConduitService
...
def generate_forward_conduit_config(lab_config: LabConfig):
    print("Generating Forward Conduits Configuration")
    _fcc = None
    try:
        _fcc_service = ForwardConduitService(lab_config=lab_config)
        _fcc = _fcc_service.build()
    except Exception as e:
        print(f"❌ Forward Conduits Configuration generation failed. -> {e}")
        raise
    print("✅ Forward Conduits Configuration successfully generated.")
    return _fcc


def check_forward_conduits_presence(lab_config: LabConfig, fcc: ForwardConduit):
    print("Check Forward Conduits Configuration representation in Lab Config")
    try:
        _fcc_service = ForwardConduitService(lab_config=lab_config)
        _errors = _fcc_service.check(fcc)
        if len(_errors) > 0:
            raise WrongConfigurationError(f"LabConfig missing ifs {_errors}.")
    except Exception as e:
        print(
            f"❌ Forward Conduits Configuration representation in Lab Config is wrong. -> {e}"
        )
        raise
    print(
        "✅ Check Forward Conduits Configuration representation in Lab Config has been conducted successfully."
    )
