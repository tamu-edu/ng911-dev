...
def get_header_field_value(message, header_name: str) -> str:
    ...
    if hasattr(message, PacketTypeEnum.HTTP):
        ...
        from services.aux_services.sip_message_services import (
            extract_raw_sip_message_string,
        )

        return extract_header_field_value_from_raw_string_body(
            header_name, extract_raw_sip_message_string(message)
        )
    return ""


def get_header_field_values_list(message, header_name: str) -> list:
    """
    Returns value of header field for HTTP or SIP message type.
    SIP header fields are extracted from raw string as pyShark does not
    support custom ones
    ...
    if hasattr(message, PacketTypeEnum.HTTP):
        ...
        raw_msg_body = extract_raw_sip_message_string(message)
        header_name = header_name.replace(":", "")
        for line in raw_msg_body.splitlines():
            if line.lower().startswith(header_name.lower() + ":"):
                result.append(line.split(":", 1)[1].strip())
    return result


def extract_header_field_value_from_raw_string_body(
    header_name: str, raw_message_body_string: str
) -> str:
    """
...
def extract_all_contents_from_message_body(
    message, ignore_content_type: bool = False
) -> list:
    """
    Extracts contents from message body as a list of dict
    :param message: The full SIP/HTTP message
    :param ignore_content_type: If True, processes body even if Content-Type header is missing
    ...
    if not content_type and ignore_content_type:
        message_body = "\n".join([line for line in message_body.splitlines() if line])
        content_dict["Content-Type"] = None
    ...
    if content_type:
        if "boundary=" in content_type:
            ...
            content_dict["Content-ID"] = content_id
            message_body = "\n".join(
                [line for line in message_body.splitlines() if line]
            )
            content_dict["body"] = message_body
            content_list.append(content_dict)
            return content_list
        ...
        for content in contents:
            ...
            try:
                content_dict["Content-Type"] = (
                    extract_header_field_value_from_raw_string_body(
                        "Content-Type", content_parts[0]
                    )
                )
                content_dict["Content-ID"] = (
                    extract_header_field_value_from_raw_string_body(
                        "Content-ID", content_parts[0]
                    )
                )
...
def get_message_and_jws_by_event_type(
    ...
    """
    try:
        if not http_post_messages:
            print("No HTTP POST messages provided")
            return None, None

        for message in http_post_messages:
            try:
                if not hasattr(message, "http") or not hasattr(
                    message.http, "file_data"
                ):
                    continue
                ...
                payload = jws if not is_unsigned_jws(jws) else jws.get("payload", {})

                if payload.get("logEventType") == log_event_type:
                    return jws, message
        ...
        print(f"Error in get_message_and_jws_by_event_type: {e}")
...
def get_http_response_containing_string_in_http_body_for_message_matching_filter(
    ...
    for message in messages:
        if (
            uri is not None
            and string_in_message is None
            and uri.lower() not in str(message.http.request_uri).lower()
        ) or (
            uri is not None
            and string_in_message is not None
            and uri.lower() not in str(message.http.request_uri).lower()
        ):
            continue
        elif (uri is None and string_in_message is not None) or (
            uri is not None
            and string_in_message is not None
...
def get_header_field_multiple_values(
    message, header_name: str, coma_separated=True
) -> set:
    """
    ...
    if hasattr(message, PacketTypeEnum.HTTP):
        ...
        msgs_headers_lst = [
            msg_str
            for msg_str in message_str.splitlines()
            if msg_str.startswith(f"{header_name}: ")
        ]
        if msgs_headers_lst:
            for mgs_header in msgs_headers_lst:
                if coma_separated:
                    ...
                    field_value = extract_header_field_value_from_raw_string_body(
                        header_name, mgs_header
                    )
                    if field_value:
...
def is_valid_catype_schema(catype: int, namespace: str) -> bool:
    ...
    VALID_TYPE_40_SCHEMAS = {NAMESPACE_PIDF_EXT, NAMESPACE_NENA}
...
def is_equal_or_after(
    timestamp1: Union[str, datetime], timestamp2: Union[str, datetime]
) -> bool:
    """
...
def get_logevent_list_by_type(messages, logevent_type, key_filepath, call_id=None):
    """
    Extracts and decodes JWS-signed log event payloads of a specific type from HTTP messages.

    :param messages: Iterable of pcap messages to search through
    :param logevent_type: The logEventType value to filter by
    :param key_filepath: Path to the key file used to decode JWS payloads
    :param call_id: Optional callIdSip value to further filter results
    :return: List of decoded payload dicts matching the given logevent_type (and call_id if provided)
    """
    result_list = []
    if messages:
        for message in messages:
...
def normalize_string_for_comparison(s: str) -> str:
    ...
    Strips leading/trailing whitespace from each line, removes empty
    lines, and collapses any remaining runs of whitespace into a single
    space.

    :param s: string to normalize
    :return: normalized string, or the original value unchanged if it
             is not a string
    """
...
def get_handshake_protocol_type(message):
    """
    Returns the TLS handshake protocol type from a packet message.

    :param message: Pcap message with a TLS layer
    :return: Handshake protocol type string (e.g. "Client Hello"), or empty string if not present
    """
    if hasattr(message.tls, "handshake"):
        return message.tls.handshake.split(": ")[-1].strip()
    else:
        return ""


def get_cipher_suite_values_list(message):
    """
    Returns a list of cipher suite display names from a TLS handshake message.

    :param message: Pcap message with a TLS layer containing handshake cipher suite fields
    :return: List of cipher suite name strings, or empty list if none are present
    """
    result = []
    if not hasattr(message.tls, "handshake_ciphersuite"):
        return result
    for ciper_suite in message.tls.handshake_ciphersuite.all_fields:
        result.append(ciper_suite.showname_value)
    return result
...
def get_geolocation_from_message(message):
    # Local import to prevent circular import with xml_services
    from services.aux_services.xml_services import extract_location_from_text

    hex_data = None

    if hasattr(message, "http") and hasattr(message.http, "file_data"):
        hex_data = message.http.file_data.replace(":", "")
    elif hasattr(message, "sip") and hasattr(message.sip, "msg_body"):
        hex_data = message.sip.msg_body.replace(":", "")
    if hex_data:
        byte_data = bytes.fromhex(hex_data)
        message_body = byte_data.decode("ascii", errors="ignore")
        return extract_location_from_text(message_body)
    else:
        return None
...
def get_bidirectional_media(ip1: str, ip2: str, pcap_service, interfaces_dict) -> list:
    ...
    return [
        get_messages(
            ...
            FilterConfig(
                ...
                src_ip_list=interfaces_dict[src].get("ip_list"),
                dst_ip_list=interfaces_dict[dst].get("ip_list"),
                packet_type=[TransportProtocolEnum.RTP, TransportProtocolEnum.UDP],
            ),
        )
...
def is_tcp_packet(packet) -> bool:
    """Return True if the packet uses TCP as its transport protocol."""
    return getattr(packet, "transport_layer", None) == "TCP"


def is_udp_packet(packet) -> bool:
    """Return True if the packet uses UDP as its transport protocol."""
    return getattr(packet, "transport_layer", None) == "UDP"
