    ...
    for pkt in packets:
        ...
        try:
            ...
            if dns_pkt.qry_name == fqdn and dns_pkt.a:
                ...
                return [re.sub(r'\s+', '', line).split('Address:')[-1]  # remove space, \t, \r, \n
                        for line in str(dns_pkt).split('\n')  # iterate over raw dns message
                        if 'Address: ' in line]  # choose only messages that contains "Address: "
        except AttributeError:
...
def get_dns_packets_from_pcap(pcap_service: PcapCaptureService):
    return [pkt for pkt in pcap_service.capture if getattr(pkt, PacketTypeEnum.DNS, None)]
