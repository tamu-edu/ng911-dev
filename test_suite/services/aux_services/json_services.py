...
def generate_jws(
    ...
    if not json_payload:
        print(
            f"❌ Error while loading json from given source {str(json_source)}. Returning None"
        )
        if output_file:
    ...
    if cert_path and key_path:
        ...
        private_key = serialization.load_pem_private_key(
            key_file_content, password=key_password
        )
        if not isinstance(private_key, ed448.Ed448PrivateKey):
            raise UnsupportedAlgorithm(
                f'Private key "{key_path}" does not use ECDSA with Curve448 (EdDSA)'
            )
        ...
        for cert in cert_data.split(b"-----END CERTIFICATE-----"):
            ...
            if cert:
                ...
                if (
                    ...
                    raise UnsupportedAlgorithm(
                        f'Certificate "{cert_path}" does not use ECDSA with Curve448 (EdDSA)'
                    )
        ...
        header["alg"] = "none"
        jws = jwt.encode(
            payload=flatten(json_payload), key=None, algorithm="none", headers=header
        )
        try:
    ...
    if jws_object.get("protected", "") and jws_object.get("signature", ""):
        try:
            if key_path:
                ...
                raise FileExistsError(f"Key file: {key_path} seems to be non-existing")
        ...
        if not is_unsigned_jws(jws_object):
            print(
                "Error while decoding unsigned JWS. Could not find 'alg': 'none' in headers"
            )
            return []
        ...
        return [
            alg_info_source,
            get_json(decode_base64url(jws_object.get("payload", None))),
        ]
    else:
