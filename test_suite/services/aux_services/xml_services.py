...
def extract_location_from_text(text: str) -> str:
    """
    Extracts value of location using suffixes "gs:pos"/"pos" and returns as a string or None if not found.
    :param text: XML data represented as text string
    :return: value of "gs:pos" or "pos" or None if not found
    """
    pattern = r"<gs:pos>(.*?)</gs:pos>"
    match = re.search(pattern, text)
    if not match:
        pattern = r"<pos>(.*?)</pos>"
        match = re.search(pattern, text)
