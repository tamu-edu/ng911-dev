...
def extract_media_attributes(message, return_full_attr: bool = False) -> dict:
    ...
    Args:
        message: SIP message object (pyshark or raw container)
    ...
    # return_full_attr=False (codec only)
    {
        "rtpmap": ["t140"],
        "cps": ["20"]
    }

    # return_full_attr=True (full SDP attribute)
    {
        "rtpmap": "98 t140/1000",
        "sendrecv": ""
    }
    ...
    for line in strip_ansi(message_data).splitlines():
        ...
        # RAW SDP
        if is_raw and line.startswith(("a=", "m=")):
            content = line[2:]
        ...
        # PYSHARK SDP
        elif not is_raw and ("Media Attribute (a)" in line or "Media Line (m)" in line):
            clean_line = line.split(":", 1)[-1].strip()

            if ":" in clean_line:
                k, v = clean_line.split(":", 1)
                media_dict[k.replace("\\r", "").replace("\\n", "").strip()] = (
                    clean_up_string(v).strip()
                )
            else:
                media_dict[clean_line.replace("\\r", "").replace("\\n", "").strip()] = (
                    ""
                )
