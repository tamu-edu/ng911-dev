...
def optimize_test_list_for_variation(test_list: list) -> list:
    ...
    return [
        {
            "test_id": test_id,
            "subtests": [] if test_id in use_all_subtests else sorted(subtests),
        }
...
def find_path_value(variation_param_message_list):
    """
    Finds the first value for the 'path' key within a nested 'kwargs' dict.
    :param: data_list: A list of dictionaries.
    :return: The value of the first 'path' key found, or None if not found.
    """
    if variation_param_message_list:
        for item in variation_param_message_list:
            kwargs = item.get("kwargs")

            if isinstance(kwargs, dict) and "path" in kwargs:
                return kwargs["path"]
    return None
