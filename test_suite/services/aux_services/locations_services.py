...
def haversine_distance(
    ...
    # TODO CHECK TYPES LPYLYP -> we cannot unpack strs
    # lat1, lon1 = coord1
    # lat2, lon2 = coord2
    #
...
def validate_coordinate(coord_str: str):
    """
...
def extract_shape_data(xml_file: str) -> dict:
    ...
    if "<!DOCTYPE" in xml_file or "<!ENTITY" in xml_file:
...
def is_point_in_shape(shape: dict, point_str: str) -> dict:
    ...
    if shape_type == "Circle":
        ...
        points = [
            tuple(map(float, coord.split())) for coord in coordinates.strip().split(";")
        ]
        polygon = Polygon(
