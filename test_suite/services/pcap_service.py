...
class PcapCaptureService(metaclass=LoggingMeta):
    ...
    def get_messages_by_config(self, config: FilterConfig):
        def match_packet(packet, active_config: FilterConfig):
            ...
            # SIP responses (e.g. "200 OK") have neither `method` (SIP request)
            if active_config.message_method and packet_layer:
                ...
                if method is None:
                    status_line = getattr(packet_layer, "status_line", None)
                    if status_line:
                        status_line_parts = str(status_line).split(" ", 2)
                        if len(status_line_parts) == 3:
                            method = status_line_parts[2]
            ...
            # Check timestamp condition (cursor: packet must be strictly after this time)
            ...
            # Check before_timestamp condition (cursor: packet must be strictly before this time)
            if active_config.before_timestamp and (
                not hasattr(packet, "sniff_timestamp")
                or float(packet.sniff_timestamp)
                >= float(active_config.before_timestamp)
            ):
                return False
