    ...
    def get_messages_by_config(self, config: FilterConfig):
        def match_packet(packet):
            ...
            if config.src_ip and (not hasattr(packet, "ip") or packet.ip.src != config.src_ip):
                return False
            if config.src_ip_list and (not hasattr(packet, "ip") or packet.ip.src not in config.src_ip_list):
                return False
            if config.dst_ip and (not hasattr(packet, "ip") or packet.ip.dst != config.dst_ip):
                return False
            if config.dst_ip_list and (not hasattr(packet, "ip") or packet.ip.dst not in config.dst_ip_list):
                return False
        ...
        return [pkt for pkt in self.capture if match_packet(pkt)] if self.capture is not None else []
