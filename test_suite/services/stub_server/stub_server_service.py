...
class StubServerService:
    def __init__(
            ...
            port: PortMapping, docker_service: DockerService = None,
    ...
    def _add_ip_alias_safely(self, ip, mask, interface):
        ...
        if f"{ip}/{mask}" not in existing:
            os.system(f"{sudo} ip addr add {ip}/{mask} dev {interface}")
            print(f"Alias created -> {ip}/{mask} dev {interface}")
        else:
            print(f"⚠️ IP alias {ip}/{mask} already exists on {interface}")
    ...
    def _launch_sip_stub_server(self):
        ...
        if self.role == StubServerRole.RECEIVER:
            ...
            cmd = cmd + f" -aa -default_behaviors -all,bye -m 1 -bg {target_ip_port}"
    ...
    def _launch_http_stub_server(self):
        ...
        print(f"[HTTPServerService] Launching with: "
              f"python3", "test_suite/services/stub_server/http/http_entry.py ",
              f"--ip={self.ip_alias} ",
              f"--port={self.if_port.port} ",
              f"--role={self.role.value} ",
              f"--method={self.request_data.get("method")} ",
