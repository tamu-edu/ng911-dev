...
class StubServerService:
    ...
    def __init__(
            ...
            port: PortMapping, ss_role: StubServerRole, docker_service: DockerService = None,
            scenario_file: str = None, sipp_kwargs: dict = None,
            request_data: dict = None, run_in_background: bool = True,
            ssl_keylog_file: str = None, shared_ip_aliases = None
    ):
    ...
    def _get_target_api_prefix(self) -> str:
        """
            ...
            if entity.api_http_url_prefix:
                for interface in entity.interfaces:
                    if interface.name == target_if_name:
                        if entity.api_http_url_prefix:
                            return entity.api_http_url_prefix.removeprefix('/').removesuffix('/')
                    ...
                    for port in interface.port_mapping:
                        if port.protocol == self.protocol.value.upper():
                            if self.protocol.value.upper() == "SIP":
                                return (f"{self.protocol.value.lower()}:{interface.fqdn or entity.fqdn or interface.ip}"
                                        f":{port.port}")
                            elif self.protocol.value.upper() == "HTTP" or self.protocol.value.upper() == "HTTPS":
                                return (f"{self.protocol.value.lower()}://"
    ...
    def _should_use_python_sip(self) -> bool:
        ...
        Rule:
        - scenarios under .../SIPp_scenarios/sip_service/... -> Python SIP service
        ...
        return "/SIPp_scenarios/sip_service/" in normalized
    ...
    def _add_ip_alias_safely(self, ip, mask, interface):
        ...
        existing = os.popen(f"ip addr show dev {interface}").read()
        needle = f"{ip}/{prefix}"
        if needle not in existing:
            os.system(f"{sudo} ip addr add {ip}/{prefix} dev {interface}")
            print(f"Alias created -> {ip}/{prefix} dev {interface}")
            self._shared_ip_aliases.append((ip, prefix, interface))
            try:
                os.system(f"{sudo} arping -c 1 -I {interface} {ip} >/dev/null 2>&1")
            except Exception:
                pass
        else:
            self._shared_ip_aliases.append((ip, prefix, interface))
    ...
    def _launch_sip_stub_server(self):
        ...
        if self.sipp_kwargs:
            ...
            for key, value in self.sipp_kwargs.items():
    ...
    def _get_tls_cli_args_for_py_sip(self) -> list[str]:
        ...
        if proto == "TLS":
            ...
            if self.role == StubServerRole.RECEIVER:
                if self.current_entity.certificate_file and self.current_entity.certificate_key:
                    args += ["--tls-cert", f"{self.current_entity.certificate_file}",
                             "--tls-key", f"{self.current_entity.certificate_key}"]

                # mTLS for RECEIVER (require client cert)
                tp = (self.if_port.transport_protocol or "").upper()
                if "MTLS" in tp and self._lab_config.pca_certificate_file:
                    args += ["--tls-ca", f"{self._lab_config.pca_certificate_file}",
                             "--require-client-cert"]
    ...
    def _launch_sip_python_stub_server(self):
        ...
        cmd = [
            "python3", sip_entry_path,
            "--bind-ip", f"{self.ip_alias}",
            "--bind-port", f"{self.if_port.port}",
            "--protocol", self._map_transport_for_py_sip(),
            "--scenario", f"{self.scenario_file}",
            "--set", "IF_OSP_BCF", "192.168.0.151",
        ]
        ...
        if self.role == StubServerRole.SENDER:
            if not self.target_uri:
                raise ValueError("Sender role requires target_uri (e.g., sip:<ip>:<port>)")
        ...
        rtp_bind_port = (self.request_data or {}).get("rtp_bind_port") or self.if_port.port
        ...
        if rtp_bind_ip and rtp_bind_port:
            cmd += ["--rtp-bind-ip", f"{rtp_bind_ip}", "--rtp-bind-port", f"{rtp_bind_port}"]
        if rtp_remote_ip and rtp_remote_port:
            cmd += ["--rtp-remote-ip", f"{rtp_remote_ip}", "--rtp-remote-port", f"{rtp_remote_port}"]
        ...
        self.process = run_process(cmd.split(), name="SIP:PY_Stub_Server")
    ...
    def _get_ssl_args(self):
        # TLS versioning
        args = [f"--tls_min=1.2", f"--tls_max=1.3"]
        ...
        if self.role == StubServerRole.RECEIVER:
            ...
            if self.if_port.transport_protocol in [
                TransportProtocolEnum.mTLSV1_2.value, TransportProtocolEnum.mTLSV1_3.value
            ]:
                if self.current_entity.certificate_file:
                    args.append(f"--ca={self._lab_config.pca_certificate_file}")
                else:
                    print(f"For mTLS - PCA is required for certificate validation.")
                args.append(f"--require_client_cert")
    ...
    def _launch_https_stub_server(self):
        if self.if_port.protocol.upper() == StubServerProtocol.HTTPS.value:
            ...
            for port in self.interface.port_mapping:
    ...
    def _launch_http_stub_server(self, ssl_args: list | None = None) -> None:
        ...
        if self.request_data.get('content_type'):
            cmd.append(f"--content_type={self.request_data.get('content_type')}")
