...
from ..aux_services.json_services import is_valid_fqdn
...
class StubServerService:
    ...
    def __init__(
        ...
        rtp_bind_port: int | None = None,
        rtp_bind_ip: str | None = None,
        request_data: dict | None = None,
        run_in_background: bool = True,
        ssl_keylog_file: str | None = None,
        ssl_config_file: str | None = None,
        openssl_config_file: str | None = None,
        conn=None,
    ):
        ...
        self._fsh = self._lab_config.get_forward_server_entity()
        self._io_mode = self._lab_config.get_forward_server_entity() is not None
        if self._io_mode:
            self._rt_table_id = self._make_rt_table_id()
            self._route_record = None

    def _make_rt_table_id(self) -> int:
        raw = f"{self.ip_alias}:{self.if_port.port}:{self.name}".encode()
        return 10000 + (zlib.crc32(raw) % 50000)
    ...
    def _rule_exists_exact(self, source_ip: str) -> bool:
        result = subprocess.run(["ip", "rule", "show"], capture_output=True, text=True)

        for line in result.stdout.splitlines():
            if f"from {source_ip}" in line and f"lookup {self._rt_table_id}" in line:
                return True

        return False

    def _configure_routing_via_fsh(self):
        if not self._io_mode:
            return

        if self.role not in [
            StubServerRole.SENDER,
            StubServerRole.OTHER,
            StubServerRole.RECEIVER,
        ]:
            return
        ...
        target_ip, _ = self._parse_target_uri()
        source_ip = self.ip_alias
        gateway = self._fsh.interfaces[0].ip

        sudo = get_sudo()

        if is_valid_fqdn(target_ip):
        ...
        cmds = [
            [
                ...
                gateway,
                "table",
                str(self._rt_table_id),
            ],
        ]

        if not self._rule_exists_exact(source_ip):
            cmds.append(
                [
                    "ip",
                    "rule",
            ...
            print(f"[Routing] Rule exists for {source_ip}, skipping")

        for cmd in cmds:
            if sudo:
                cmd.insert(0, "sudo")

            print(f"[Routing] {' '.join(cmd)}")
            subprocess.run(cmd, check=False)

        self._route_record = (source_ip, target_ip, self._rt_table_id)

        if self._conn:
            self._conn.send(
                {
                    "type": "ROUTE_ADD",
                    "data": self._route_record,
                }
            )

    def _remove_routing_via_fsh(self):
        if not self._io_mode:
            return
        ...
        try:
            ...
            if self._conn and self._route_record:
                self._conn.send(
                    {
                        "type": "ROUTE_REMOVE",
                        "data": self._route_record,
            ...
            print(f"[Routing] Failed cleanup: {e}")
    ...
    def _get_server_api_prefix(self) -> str:
        """
        Returns the server api prefix
        :return:
        """
        for entity in self._lab_config.entities:
            if entity.api_http_url_prefix:
                for interface in entity.interfaces or []:
                    if interface.name == self.interface.name:
                        if entity.api_http_url_prefix:
                            return entity.api_http_url_prefix.removeprefix(
                                "/"
                            ).removesuffix("/")
            ...
            for interface in entity.interfaces or []:
                if interface.name == target_if_name:
                    for port in interface.port_mapping or []:
                        if (
                            port.protocol == self.protocol.value.upper()
                            and port.transport_protocol
                            == self.if_port.transport_protocol
                        ):
    ...
    def _parse_target_uri(self):
        ...
        if self.target_uri.startswith("sip:"):
            _, rest = self.target_uri.split("sip:", 1)
            ip, port = rest.split(":")
            return ip, int(port)

        parsed = urlparse(self.target_uri)
        if not parsed.hostname or not parsed.port:
            raise ValueError(f"Invalid target_uri: {self.target_uri}")
    ...
    def _add_ip_alias_safely(self, ip, mask, interface):
        ...
        if needle not in existing:
            ...
            self._conn.send({"type": "IP_ALIAS_ADD", "data": (ip, prefix, interface)})
        else:
    ...
    def _launch_sip_stub_server(self):
        ...
        self._configure_routing_via_fsh()
        ...
        if self.role == StubServerRole.RECEIVER:
            cmd = (
                ...
                f" -i {self.ip_alias} -p {self.if_port.port} -bind_local --trace_err --trace_msg"
            )
            ...
            cmd = (
                f"{sudo} sipp {self._get_transfer_protocol_flag_value()}"
                f" -sf {self.scenario_file} -i {self.ip_alias} -p {self.if_port.port} -bind_local"
            )
    ...
    def _remove_ip_alias(self, ip, mask, iface="enp0s8"):
        try:
            if not self.ip_alias_exist:
                ...
                self._conn.send({"type": "IP_ALIAS_REMOVE", "data": (ip, mask, iface)})
            else:
                print(
    ...
    def _get_tls_cli_args_for_py_sip(self) -> list[str]:
        ...
        if proto == "TLS":
            ...
            if self.ssl_config_file:
                args += [
                    "--ssl-config-file",
                    f"{self.ssl_config_file}",
                ]

            if self.openssl_config_file:
                args += [
                    "--openssl-config-file",
                    f"{self.openssl_config_file}",
                ]
    ...
    def _launch_sip_python_stub_server(self):
        ...
        rtp_bind_ip = (
            self.rtp_bind_ip
            or (self.request_data or {}).get("rtp_bind_ip")
            or self.ip_alias
        )
        rtp_bind_port = self.rtp_bind_port or self.if_port.port
        ...
        if self.openssl_config_file:
            ...
            _env["OPENSSL_CONF"] = self.openssl_config_file

            print(
                f"[SIPServerService:PY] Launching with: {' '.join(cmd)} and environment variables: {_env}"
            )
            self.process = run_process(cmd, name="SIP:PY_Stub_Server", env=_env)
        else:
    ...
    def _get_ssl_args(self):
        ...
        if self.ssl_config_file:
            args.append(f"--ssl-config-file={self.ssl_config_file}")

        if self.openssl_config_file:
            args.append(f"--openssl-config-file={self.openssl_config_file}")
    ...
    def _launch_http_stub_server(self, ssl_args: list | None = None) -> None:
        ...
        if self.role == StubServerRole.RECEIVER:
            server_prefix = self._get_server_api_prefix()
        if path and server_prefix:
            path = f"/{server_prefix.removesuffix("/")}/{path.removeprefix("/")}"
            cmd.append(f"--path={path}")
        elif server_prefix:
            cmd.append(f"--path=/{server_prefix}")
        elif path:
            cmd.append(f"--path={path}")
        ...
        if (self.request_data or {}).get("response_code"):
            response_code = (self.request_data or {}).get("response_code")
            if isinstance(response_code, (list, dict)):
                response_code = json.dumps(response_code, separators=(",", ":"))
            cmd.append(f"--response_code={response_code}")
        ...
        if self.openssl_config_file:
            ...
            print(
                f"[HTTPServerService] Launching with: {cmd} | and environment variables: {_env}"
            )
            self.process = run_process(cmd, name="HTTP_Stub_Server", env=_env)
        else:
