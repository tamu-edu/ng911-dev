...
def _pick_target_hostport(vars_dict: dict) -> str:
    ip = vars_dict.get("peer_ip") or vars_dict.get("remote_ip") or vars_dict.get("local_ip") or "127.0.0.1"
    port = vars_dict.get("peer_port") or vars_dict.get("remote_port") or vars_dict.get("local_port") or ""
...
def render_sipp_like(template_text: str, vars_dict: dict) -> bytes:
    ...
    rendered = fix_transport_tokens(rendered, vars_dict.get("transport"))
    ...
    if not re.search(r"(?im)^Call-ID\s*:", headers):
        call_id = vars_dict.get(
            "call_id") or f"{''.join(random.choices(string.ascii_letters + string.digits, k=10))}@{vars_dict.get('local_ip', 'stub')}"
