...
from secure_media import DTLSSRTPTransport
from secure_media.srtp import SRTPContext
...
class ScenarioRunner:
    ...
    def __init__(
        ...
        # Extract subscribe steps from scenario
        for st in self.steps:
            if st.get("type") == "subscribe":
        ...
        self.vars.setdefault(
            "peer_tag_param", ""
        )  # becomes ";tag=..." after 200 OK if extracted
    ...
    def _build_200_ok_from_request(self, req_text: str, vars_dict: dict) -> str:
        ...
        call_id = h.get(
            "call-id",
            vars_dict.get("call_id", "X@" + vars_dict.get("local_ip", "stub")),
        )
    ...
    def on_incoming(self, text, addr):
            ...
            if sdp:
                ...
                if port:
                    ...
                    if self.rtp is None and not getattr(self, "explicit_media", False):
                        ...
                        local_ip = (
                            self.vars.get("rtp_local_ip")
                            or self.vars.get("local_ip")
                            or self.sip.bind[0]
                        )
                        ...
                        self.rtp.recv_loop(
                            lambda data, a: None
                        )  # no-op receiver; can attach real cb if needed
        ...
        # --- SUBSCRIBE BACKGROUND MATCHING ---
        for sub in self.subscriptions:
            if self._match(text, sub.get("match", {})):
                self.log.info("SUBSCRIBE matched: %s", sub.get("match"))

                # 1️⃣ Run operate logic (like recv)
                operates = sub.get("operate") or []
                if isinstance(operates, dict):
                    operates = [operates]

                current_text = text
                for op in operates:
                    current_text = self._execute_operate(op, current_text, addr)

                # 2️⃣ Optional static response
                if sub.get("message"):
                    msg = self._render(sub["message"])
                    self._send_text(msg, addr)

                # 3️⃣ Reset socket (TCP/TLS only)
                if sub.get("reset_socket") and self.sip.protocol in ("TCP", "TLS"):
                    try:
                        self.sip.reset_listening_socket()
                    except Exception as e:
                        self.log.debug(e)

                    self.log.info("Connection reset due to subscribe reset_socket=true")
    ...
    def _execute_operate(self, operate: dict, in_text: str, src_addr):
        ...
        if src_ip and src_addr[0] != src_ip:
            self.log.info(
                "operate: source_ip mismatch (%s != %s), skipping", src_addr[0], src_ip
            )
            return
    ...
    def _find_srtp_transport(
        self,
        dst_ip: str,
        dst_port: int,
    ) -> RTPTransport | None:
        """
        Find a standalone SRTP-enabled RTP transport.

        The transport registry is keyed by its local bind address.
        For sending, prefer a transport whose processor is configured.
        """
        for transport in self.rtp_sessions.values():
            if not isinstance(
                transport,
                RTPTransport,
            ):
    ...
    def _echo_enabled(
        ...
        """
        Returns whether media echo is enabled.
        """
    ...
    def _find_dtls_transport(
        self,
        dst_ip: str,
        dst_port: int,
    ) -> DTLSSRTPTransport | None:
        for (
            _local_ip,
            _local_port,
            remote_ip,
            remote_port,
        ), transport in self.dtls_sessions.items():
            if remote_ip == dst_ip and remote_port == dst_port:
                return transport
    ...
    def run(self):
        ...
        while pc < len(self.steps):
            ...
            if t == "subscribe":
                # already registered in background
                continue
            ...
            if t == "await":
                ...
                try:
                    ...
                    if step.get("optional"):
                        self.log.info(
                            "optional await timed out, continuing: %s",
                            step.get("match"),
                        )
                        continue
                    raise
                ...
                if operates:
                    ...
                    for op in operates:
                        current_text = self._execute_operate(
                            op, current_text, matched_addr
                        )

                # NEW: reset socket after successful await
                if step.get("reset_socket") and self.sip.protocol in ("TCP", "TLS"):
                    try:
                        self.sip.reset_listening_socket()
                    except Exception as e:
                        self.log.debug(e)

                    self.log.info("Connection reset due to recv reset_socket=true")
            ...
            if t == "rtp_recv_start":
                ...
                self.rtp.recv_loop(
                    cb=lambda data, addr: self.log.debug(
                        "RTP RX %dB from %s", len(data), addr
                    )
                )
                continue
            ...
            if t == "rtp_send":
                ...
                self.rtp.send_raw(
                    bytes.fromhex(payload_hex), self._resolve_rtp_remote()
                )
                continue
            ...
            if t == "media_start":
                ...
                if media_type == "rtp":
                    ...
                    if not dst_ip or not dst_port_s:
                        self.log.error(
                            "media_start: dst_ip or dst_port is missing after render"
                        )
                        continue
                    try:
                    ...
                    if key in self.rtp_sessions:
                        self.log.info(
                            "RTP media already started on %s:%s, skipping",
                            dst_ip,
                            dst_port,
                        )
                        continue
                    ...
                    try:
                        ...
                        self.log.info(
                            "Started RTP media on %s:%s (src_ip filter=%s)",
                            dst_ip,
                            dst_port,
                            src_ip or "any",
                        )
                ...
                if media_type == "srtp":
                    bind_ip = self._render(
                        step.get("dst_ip")
                        or self.vars.get("rtp_local_ip")
                        or self.vars.get("local_ip")
                        or self.sip.bind[0]
                    )

                    bind_port_s = self._render(
                        step.get("dst_port") or self.vars.get("rtp_local_port") or "0"
                    )

                    source_ip_filter = self._render(step.get("src_ip") or "")

                    profile = self._render(
                        step.get("profile")
                        or self.vars.get("srtp_profile")
                        or "AES_CM_128_HMAC_SHA1_80"
                    )

                    tx_key = self._render(
                        step.get("tx_key") or self.vars.get("srtp_tx_key") or ""
                    )

                    rx_key = self._render(
                        step.get("rx_key") or self.vars.get("srtp_rx_key") or ""
                    )

                    if not tx_key and not rx_key:
                        self.log.error(
                            "srtp: at least one of tx_key or rx_key is required"
                        )
                        continue
                    ...
                    if requested_key in self.rtp_sessions:
                        self.log.info(
                            "SRTP media already started on %s:%s, skipping",
                            bind_ip,
                            bind_port,
                        )
                        continue
                    ...
                    try:
                        ...
                        processor = SRTPProcessor(context)

                        transport = RTPTransport(
                            bind=(bind_ip, bind_port),
                            secure_processor=processor,
                            log=self.log,
                        )

                        transport.start()

                        actual_bind = transport.sock.getsockname()
                        ...
                        def _srtp_callback(
                            data,
                            address,
                            expected_source=source_ip_filter,
                            echo_enabled=echo,
                        ):
                            if expected_source and address[0] != expected_source:
                                return
                        ...
                        transport.recv_loop(_srtp_callback)
                        ...
                        if transport is not None:
                            try:
                                ...
                                self.log.debug(
                                    "SRTP transport cleanup failed: %s",
                                    cleanup_exc,
                                )
                        ...
                        if processor is not None:
                            try:
                                ...
                                self.log.debug(
                                    "SRTP processor cleanup failed: %s",
                                    cleanup_exc,
                                )

                        self.log.error(
                            "Failed starting standalone SRTP session: %s",
                            exc,
                        )
                        continue
                    ...
                    self.rtp_sessions[actual_key] = transport
                    self.srtp_processors[actual_key] = processor

                    self.vars["rtp_local_ip"] = actual_bind[0]
                    self.vars["rtp_local_port"] = str(actual_bind[1])

                    self.log.info(
                        "Started standalone SRTP media on %s:%s "
                        "(profile=%s, source filter=%s)",
                        actual_bind[0],
                        actual_bind[1],
                        profile,
                        source_ip_filter or "any",
                    )

                    continue

                # --------- sRTP over DTLS ----------
                if media_type == "dtls_srtp":
                    bind_ip = self._render(
                        step.get("src_ip")
                        or self.vars.get("rtp_local_ip")
                        or self.vars.get("local_ip")
                    ...
                    bind_port_s = self._render(
                        step.get("src_port") or self.vars.get("rtp_local_port") or "0"
                    )

                    remote_ip = self._render(
                        step.get("dst_ip") or self.vars.get("rtp_remote_ip") or ""
                    )

                    remote_port_s = self._render(
                        step.get("dst_port") or self.vars.get("rtp_remote_port") or ""
                    )

                    if not remote_ip or not remote_port_s:
                        self.log.error(
                            "dtls_srtp: dst_ip/dst_port missing after render"
                        )
                        continue
                    ...
                    try:
                        ...
                        self.log.error("dtls_srtp: invalid src_port or dst_port")
                        continue

                    role = self._render(
                        step.get("role") or self.vars.get("dtls_role") or "client"
                    )

                    certificate_file = self._render(
                        step.get("certificate_file")
                        or self.vars.get("dtls_certificate_file")
                        or ""
                    )

                    private_key_file = self._render(
                        step.get("private_key_file")
                        or self.vars.get("dtls_private_key_file")
                        or ""
                    )

                    expected_peer_fingerprint = self._render(
                        step.get("remote_fingerprint")
                        or self.vars.get("dtls_remote_fingerprint")
                        or ""
                    )

                    fingerprint_algorithm = self._render(
                        step.get("fingerprint_algorithm")
                        or self.vars.get("dtls_fingerprint_algorithm")
                        or "sha-256"
                    )

                    cipher_list = self._render(
                        step.get("cipher_list")
                        or self.vars.get("dtls_cipher_list")
                        or "DEFAULT"
                    )

                    profiles_value = self._render(
                        step.get("srtp_profiles")
                        or self.vars.get("dtls_srtp_profiles")
                        or ("SRTP_AES128_CM_SHA1_80:" "SRTP_AES128_CM_SHA1_32")
                    )

                    srtp_profiles = tuple(
                        profile.strip()
                        for profile in profiles_value.replace(
                            ",",
                            ":",
                        ).split(":")
                        if profile.strip()
                    ...
                    handshake_timeout_s = self._render(
                        step.get("handshake_timeout")
                        or self.vars.get("dtls_handshake_timeout")
                        or "10"
                    )
                    ...
                    try:
                        ...
                        self.log.error(
                            "dtls_srtp: invalid handshake_timeout %r",
                            handshake_timeout_s,
                        )
                        continue
                    ...
                    if key in self.dtls_sessions:
                        self.log.info(
                            "DTLS-SRTP session already exists " "%s:%s → %s:%s",
                            bind_ip,
                            bind_port,
                            remote_ip,
                            remote_port,
                    ...
                    transport = DTLSSRTPTransport(
                        ...
                        src_ip_filter=self._render(step.get("src_ip_filter") or ""),
                        log=self.log,
                    )
                    ...
                    try:
                        ...
                        self.log.error(
                            "Failed starting DTLS-SRTP session: %s",
                            exc,
                        )
                        continue
                    ...
                    def _dtls_callback(
                        ...
                        self.log.debug(
                            "DTLS-SRTP %s RX %dB from %s",
                            "RTCP" if is_rtcp else "RTP",
                            len(data),
                            address,
                        )
                        ...
                        if is_rtcp:
                            transport.send_rtcp(data)
                        else:
                            transport.send_rtp(data)

                    transport.recv_loop(_dtls_callback)
                    ...
                    self.dtls_sessions[actual_key] = transport

                    self.vars["rtp_local_ip"] = actual_bind[0]
                    self.vars["rtp_local_port"] = str(actual_bind[1])

                    self.log.info(
                        "Started DTLS-SRTP session " "%s:%s → %s:%s",
                        actual_bind[0],
                        actual_bind[1],
                        remote_ip,
                        remote_port,
                ...
                if media_type == "rtp_proxy":
                    ...
                    if not proxy_ip or not dst_ip or not dst_port_s:
                        self.log.error(
                            "media_start rtp_proxy: proxy_ip/dst_ip/dst_port missing after render"
                        )
                        continue
                    try:
                        dst_port = int(dst_port_s)
                        ...
                        self.log.error(
                            "media_start rtp_proxy: invalid dst_port %r", dst_port_s
                        )
                        continue
                    ...
                    if proxy_port_s:
                        try:
                            ...
                            self.log.error(
                                "media_start rtp_proxy: invalid proxy_port %r",
                                proxy_port_s,
                            )
                            continue
                    else:
                ...
                if media_type in [
                    ...
                    if not dst_ip or not dst_port_s:
                        self.log.error(
                            "rtp_send_text: dst_ip/dst_port missing after render"
                        )
                        continue
                    ...
                    if step.get("csrc"):
                        csrc_list = [
                            int(x.strip()) for x in step["csrc"].split(",") if x.strip()
                        ]
                    ...
                    try:
                        ...
                        self.log.error(
                            "rtp_send_text: invalid port/payload_type/ts_step"
                        )
                        continue
                    ...
                    if media_type == "srtp_send_text":
                        secure_transport = self._find_srtp_transport(
                            dst_ip,
                            dst_port,
                        )
                        ...
                        if secure_transport is None:
                            self.log.error(
                                "srtp_send_text: no active standalone " "SRTP transport"
                            )
                            continue
                    ...
                    elif media_type == "dtls_srtp_send_text":
                        secure_transport = self._find_dtls_transport(
                            dst_ip,
                            dst_port,
                        )
                        ...
                        if secure_transport is None:
                            self.log.error(
                                "dtls_srtp_send_text: no active DTLS-SRTP "
                                "transport for %s:%s",
                                dst_ip,
                                dst_port,
                            )
                        ...
                        packet_sender = secure_transport.send_rtp
                ...
                if media_type in [
                    ...
                    audio_dst_ip = self._render(step["dst_ip"])

                    audio_dst_port = int(self._render(step["dst_port"]))

                    packet_sender = None

                    if media_type == "srtp_play_audio":
                        secure_transport = self._find_srtp_transport(
                            audio_dst_ip,
                            audio_dst_port,
                        )
                        ...
                        if secure_transport is None:
                            self.log.error(
                                "srtp_play_audio: no active standalone "
                                "SRTP transport"
                            )
                            continue
                    ...
                    elif media_type == "dtls_srtp_play_audio":
                        secure_transport = self._find_dtls_transport(
                            audio_dst_ip,
                            audio_dst_port,
                        )
                        ...
                        if secure_transport is None:
                            self.log.error(
                                "dtls_srtp_play_audio: no active DTLS-SRTP "
                                "transport for %s:%s",
                                audio_dst_ip,
                                audio_dst_port,
                            )
                    ...
                    audio = RTPAudioStream(
                        ...
                        dst_ip=audio_dst_ip,
                        dst_port=audio_dst_port,
                        packet_sender=packet_sender,
                        audio_file=self._render(step["audio_file"]),
                        codec=self._render(step.get("codec", "pcmu")),
                        payload_type=(
                            int(self._render(step["payload_type"]))
                            if step.get("payload_type")
                            else None
                        ),
            ...
            if t == "media_stop":
                ...
                if not dst_ip or not dst_port_s:
                    self.log.error(
                        "media_stop: dst_ip or dst_port is missing after render"
                    )
                    continue
                ...
                try:
                    ...
                    self.log.error(
                        "media_stop: invalid dst_port %r",
                        dst_port_s,
                    )
                    continue

                # Stop RTP text streams targeting the selected destination.
                for key, stream in list(self.rtp_text_streams.items()):
                    _, _, remote_ip, remote_port, _ = key
                    ...
                    if remote_ip == dst_ip and remote_port == dst_port:
                        ...
                        self.rtp_text_streams.pop(
                            key,
                            None,
                        )

                        self.log.info(
                            "Stopped RTP text stream → %s:%s",
                            dst_ip,
                            dst_port,
                        )
                ...
                # Plain RTP and standalone SRTP use the same RTP session registry.
                session_key = (
                    dst_ip,
                    dst_port,
                )

                transport = self.rtp_sessions.pop(
                    session_key,
                    None,
                )
                ...
                processor = self.srtp_processors.pop(
                    session_key,
                    None,
                )
                ...
                # DTLS-SRTP keys include both local and remote endpoints.
                for key, dtls_transport in list(self.dtls_sessions.items()):
                    (
                        local_ip,
                        local_port,
                        remote_ip,
                    ...
                    matches_local = local_ip == dst_ip and local_port == dst_port

                    matches_remote = remote_ip == dst_ip and remote_port == dst_port
                ...
                if stopped:
                    ...
                    self.log.warning(
                        "media_stop: no %s session found for %s:%s",
                        media_type,
                        dst_ip,
                        dst_port,
                    )
        ...
        for transport in list(self.rtp_sessions.values()):
            try:
                transport.stop()
            except Exception as exc:
                self.log.debug(exc)
        ...
        for processor in list(self.srtp_processors.values()):
            try:
                processor.close()
            except Exception as exc:
                self.log.debug(exc)

        self.srtp_processors.clear()

        for transport in list(self.dtls_sessions.values()):
            try:
                transport.stop()
            except Exception as exc:
                self.log.debug(exc)

        self.dtls_sessions.clear()
        ...
        while time.time() < end:
            if self.inbox:
                ...
                if self._match(msg[0], cond):
                    self.peer = msg[1]
                    self.vars["peer_ip"], self.vars["peer_port"] = self.peer[0], str(
                        self.peer[1]
                    )
                    return msg
            time.sleep(0.01)
    ...
    def _render(self, s):
        if s is None:
            return ""
        return re.sub(
            r"\$\{([a-zA-Z0-9_]+)\}", lambda m: str(self.vars.get(m.group(1), "")), s
        )
