...
class SIPTransport:
    ...
    def __init__(self, protocol, bind, ssl_context: Optional[ssl.SSLContext] = None, log=None, remote: Optional[Tuple[str, int]] = None):
        """
        ...
        # Sockets
        self.sock = None          # UDP socket OR TCP/TLS listening socket
    ...
    def send(self, data: bytes, addr: Optional[Tuple[str, int]]):
        if self.protocol == "UDP":
            ...
            if not target:
                raise RuntimeError("UDP send: no target address provided and no remote configured")
        ...
        if not used:
            ...
            if (addr is None and self.remote) or (addr and self.remote and addr == self.remote) or (self.remote and not self.conn):
                try:
                    ...
                    raise RuntimeError(f"TCP send: outbound connect/send to {self.remote} failed: {e}")
        ...
        if not used:
            raise RuntimeError("TCP/TLS send: no available connection (no peer and no remote)")
