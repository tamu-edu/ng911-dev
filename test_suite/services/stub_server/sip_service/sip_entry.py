...
def parse_args():
    ...
    p.add_argument("--ssl-config-file", default=None)
    p.add_argument("--openssl-config-file", default=None)
...
def main():
    ...
    # MUST be BEFORE any ssl context creation
    if a.openssl_config_file:
        os.environ["OPENSSL_CONF"] = a.openssl_config_file
    ...
    ssl_config = None
    if a.ssl_config_file:
        with open(a.ssl_config_file, "r") as f:
    ...
    srv = SIPStubServerService(
        ...
        rtp_bind=(
            (a.rtp_bind_ip, a.rtp_bind_port)
            if a.rtp_bind_ip and a.rtp_bind_port
            else None
        ),
        rtp_remote=(
            (a.rtp_remote_ip, a.rtp_remote_port)
            if a.rtp_remote_ip and a.rtp_remote_port
            else None
        ),
        tls_cert=a.tls_cert,
        tls_key=a.tls_key,
        ...
        openssl_config_file=a.openssl_config_file,
        require_client_cert=a.require_client_cert,
        log_file=a.log_file,
        log_level=a.log_level,
        message_timeout=a.message_timeout,
        transaction_timeout=a.transaction_timeout,
