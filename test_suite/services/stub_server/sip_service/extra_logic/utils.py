...
def add_header_field(message_text: str, header_name: str, header_value: str, parameters: dict = None) -> str:
...
def replace_header_field_value(message_text: str, header_name: str, header_value: str, parameters: dict = None,
                               header_contains: str = "") -> str:
    if not message_text:
        return ""
...
def add_or_replace_header_field(message_text: str, header_name: str, header_value: str, parameters: dict = None,
                                header_contains: str = "") -> str:
    for line in message_text.splitlines():
        if f"{header_name}: " in line and header_contains in line:
            return replace_header_field_value(message_text, header_name, header_value, parameters, header_contains)
    return add_header_field(message_text, header_name, header_value, parameters)


def add_header_field_if_not_exist(message_text: str, header_name: str, header_value: str, parameters: dict = None,
                                  header_contains: str = "") -> str:
    for line in message_text.splitlines():
...
def add_message_body_if_not_exist(message_text: str, content_type: str, content_body: str, content_id: str = "") -> str:
    if re.sub(r"\s+", "", content_body).lower() in re.sub(r"\s+", "", message_text).lower():
        return message_text
    out = message_text
    ...
    multipart_header = [l for l in message_text.splitlines() if "Content-Type: multipart/mixed" in l]
    if multipart_header:
        ...
        for index, line in enumerate(message_text.splitlines()):
            if ("Content-Type: " in line
                ...
                existing_headers_and_body += "\r\n" + "\r\n".join(message_text.splitlines()[(index+1):])
                out = "\r\n".join(message_text.splitlines()[:index])
        out = replace_header_field_value(out, "Content-Type", "multipart/mixed",
                                         {"boundary": boundary})
        out = "\r\n".join(l for l in out.splitlines() if "Content-Length: " not in l)
        out += "\r\n\r\n" + "--" + boundary + "\r\n" + existing_headers_and_body
