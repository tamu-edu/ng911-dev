...
def rewrite_invite_for_bcf(invite_text: str, ctx: dict) -> str:
    ...
    actions = [
        (utils.replace_header_field_value, out, "Request_URI", "urn:service:sos"),
        (utils.replace_header_field_value, out, "To", "urn:service:sos"),
        (utils.add_header_field, out, "Route", f"<{vars['ESRP_FQDN']}>;lr"),
        # (utils.add_header_field, out, "Via", f"<{vars['BCF_FQDN']}>;lr"),
        (
            utils.add_or_replace_header_field,
            out,
            "Contact",
            f"<bcf@{vars['BCF_FQDN']}>",
        ),
        (
            utils.add_header_field_if_not_exist,
            out,
            "Call-Info",
            utils.generate_identifier("callid", vars["BCF_FQDN"]),
        ...
        (
            ...
            out,
            "Call-Info",
            utils.generate_identifier("incidentid", vars["BCF_FQDN"]),
            {"purpose": "emergency-IncidentId"},
            ":incidentid:",
        ),
        (utils.add_or_replace_header_field, out, "Resource-Priority", "esnet.1"),
    ]
    for action in actions:
        _action_method: Callable = action[0]
        out = _action_method(out, *action[2:])
    return out
...
def rewrite_invite_for_esrp(invite_text: str, ctx: dict) -> str:
    ...
    default_location_xml_file_path = (
        repo_path.removesuffix("/")
    ...
    try:
        ...
        ctx["log"].info(
            f"[methods] Error opening a file: {default_location_xml_file_path}, "
    ...
    actions = [
        ...
        (utils.replace_header_field_value, out, "To", "urn:service:sos"),
        (
            utils.add_header_field,
            out,
            "Route",
            f"<sip:{ctx_vars['CHE_tel']}@{ctx_vars['CHE_FQDN']}>;lr",
        ),
        (utils.add_header_field, out, "Via", f"<{ctx_vars['ESRP_FQDN']}>;lr"),
        (
            utils.add_or_replace_header_field,
            out,
            "Contact",
            f"<esrp@{ctx_vars['ESRP_FQDN']}>",
        ),
        (
            utils.add_header_field_if_not_exist,
            out,
            "Call-Info",
            utils.generate_identifier("callid", ctx_vars["ESRP_FQDN"]),
            {"purpose": "emergency-CallId"},
            ":callid:",
        ),
        (
            ...
            out,
            "Call-Info",
            utils.generate_identifier("incidentid", ctx_vars["ESRP_FQDN"]),
            {"purpose": "emergency-IncidentId"},
            ":incidentid:",
        ),
        (utils.add_header_field_if_not_exist, out, "Resource-Priority", "esnet.1"),
        (
            utils.add_message_body_if_not_exist,
            out,
...
def rewrite_invite_for_esrp_add_conference_id(invite_text: str, ctx: dict) -> str:
    ...
    ctx_vars = {
        "conference_id": ctx.get("vars", {}).get(
            "conference_id", "conferenceng911test123"
        ),
        "ESRP_FQDN": ctx.get("vars", {}).get("ESRP_FQDN", "esrp.example.com"),
    }
    return utils.add_or_replace_header_field(
        out, "Contact", f"{ctx_vars['conference_id']}@{ctx_vars['ESRP_FQDN']};isfocus"
    )
...
def save_header_field(message_text: str, ctx: dict) -> str:
    ...
    if match := re.search(r"\r\n|\r|\n", message_text):
        line_ending = match.group()
    header_fields = ""
    for line in message_text.splitlines():
        if header_field_name.lower() + ":" in line.lower():
            header_fields += (line_ending + line) if header_fields else line
    if header_fields:
        ctx["vars"][key] = header_fields
    return message_text
...
def save_header_field_param_value(message_text: str, ctx: dict) -> str:
    """
    Saves value of specified param value for a header field from incoming message into ctx["vars"][<ctx_key>]
    for later use. Example:
    <operate
        method_name="save_header_field_param_value"
    ...
    header_field_name = params.get("header_field_name")
    param = (
        ";" + params.get("param_name").lower().replace(";", "").replace("=", "") + "="
    )
    for line in message_text.splitlines():
        if header_field_name.lower() in line.lower():
            if param in line.lower():
                ctx["vars"][key] = (
                    line.split(": ")[1].split(param)[1].split(";")[0].strip()
                )
    return message_text
...
def replace_string(message_text: str, ctx: dict) -> str:
    ...
    if (
        ...
        ctx["log"].info(
            "[methods] Some of string params for replacing in message are missing"
        )
        return ""
    return message_text.replace(replace_from, replace_to)


def prepare_notify_body(message_text: str, ctx: dict) -> str:
    """
    Basing on Event header field value generates appropriate SIP NOTIFY JSON body for later use
    with ctx["vars"][<ctx_key_notify_body>] also calculates Content-Length and saves to
    ctx["vars"][<ctx_key_content_length>].
    Example:
    <operate
        method_name="prepare_notify_body"
        ctx_key_notify_body="NOTIFY_JSON"
        ctx_key_content_length="NOTIFY_JSON_LENGTH"
    />
    """
    params = ctx.get("operate", {}).get("params", {})
    notify_body_var_name = params.get("ctx_key_notify_body")
    notify_content_length_var_name = params.get("ctx_key_content_length")
    event = None
    for line in message_text.splitlines():
    ...
    if not event:
        ctx["log"].info(
            "[methods][prepare_notify_body] Failed to find Event header field in received SIP message!"
        )
        return message_text
    if "emergency-ElementState" in event:
        event = "ElementState"
    elif "emergency-ServiceState" in event:
        event = "ServiceState"
    else:
        ctx["log"].info(
            f"[methods][prepare_notify_body] Event {event} is not supported!"
        )
        return message_text
    ...
    json_source_file = (
        repo_path.removesuffix("/")
        + f"/test_suite/test_files/JSON/other/{event}_example.json"
    )
    try:
        with open(json_source_file, "r", encoding="utf-8") as f:
            message_body = f.read()
            ctx["vars"][notify_body_var_name] = message_body
            ctx["vars"][notify_content_length_var_name] = len(message_body)
    except Exception:
        ctx["log"].info(
            f"[methods][prepare_notify_body] Error opening a file: {json_source_file}, "
            f"SIP NOTIFY will not contain message_body"
        )
    return message_text


def generate_tag_id(message_text: str, ctx: dict) -> str:
    """
    Generates tag ID into ctx["vars"][<ctx_key>] for later use. Example:
    <operate
        method_name="generate_tag_id"
        ctx_key="To_tag"
    />
    """
    params = ctx.get("operate", {}).get("params", {})
    ...
    ctx["vars"][key] = "".join(random.choices(chars, k=length))
    return message_text
