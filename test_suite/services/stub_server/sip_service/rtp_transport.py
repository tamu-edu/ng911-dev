            ...
            while not self._stop.is_set():
                ...
                if self._rx_cb:
                    try:
                        ...
                        # never let test run crash on user callback
                        pass

        self._rx_thread = threading.Thread(target=_loop, daemon=True)
        self._rx_thread.start()

    def send_dummy_stream(self, remote, duration_sec: float = 2.0, interval_sec: float = 0.02):
        """
...
class RTPProxyTransport:
    ...
    def __init__(self, bind_ip, bind_port, dst_ip, dst_port, src_ip_filter: str = "", log: logging.Logger | None = None):
    ...
    def _loop(self):
        while not self._stop.is_set():
            ...
            try:
                ...
                if self.log:
                    self.log.debug("RTP proxy send error", exc_info=True)
...
class RTPAudioStream:
    ...
    def __init__(
        ...
        self.payload_type = (
            payload_type if payload_type is not None else (0 if self.codec == "pcmu" else 8)
        )
