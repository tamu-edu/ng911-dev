...
def load_sipp_or_yaml(path: str, scenario_type: str, log=None):
    ...
    # --- SIPp XML ---
    steps = []
    label_index = {}   # label id -> future step index placeholder list (kept for compatibility)
    ...
    for node in root:
        ...
        if tag == "global":
            ...
            if vars_line:
                var_names = [v.strip() for v in vars_line.replace(",", " ").split() if v.strip()]
        ...
        if tag == "label":
            ...
            if not label_id:
                if log: log.warning("<label> without id ignored.")
                continue
        ...
        if tag == "send":
            ...
            for child in node:
                ...
                if ctag == "operate":
                    ...
                    method_name = child.attrib.get("method_name") or child.attrib.get("method")
                    if not method_name:
                        raise ValueError("<operate> inside <send> requires 'method_name'")
                    ...
                    operates.append(
                        {
                            ...
                            "dst_port": _norm_attr(child.attrib.get("dst_port")) or "5060",
                            # Default = False now
                            "auto_200_ok": (
        ...
        if tag == "media":
            ...
            if media_type == "rtp_send_text":
                ...
                split = (node.attrib.get("split") or "false").lower() in ("1", "true", "yes")
            ...
            if log:
                log.warning("Media type '%s' not supported, only 'rtp' and 'rtp_proxy'", media_type)
            continue
        ...
        if tag == "stop_media":
            ...
            if media_type not in ("rtp", "rtp_proxy"):
                if log:
                    log.warning("Stop media type '%s' not supported, only 'rtp' and 'rtp_proxy'", media_type)
                continue
        ...
        if tag == "goto":
            ...
            if not ref:
                if log: log.warning("<goto> without ref ignored.")
                continue
        ...
        if tag == "set":
            ...
            if not var_name:
                if log: log.warning("<Set> without 'variable' ignored.")
                continue
