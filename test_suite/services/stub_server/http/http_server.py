...
def _openssl_http_request(args, full_url, method):
    """
    Uses openssl s_client to perform HTTPS request with OPENSSL_CONF applied.
    Supports TLS1.3 cipher config via openssl.cnf.
    """
    if not args.openssl_config_file:
        raise ValueError("openssl_config_file required for OpenSSL mode")
    ...
    env = os.environ.copy()
    env["OPENSSL_CONF"] = args.openssl_config_file
    ...
    print(f"[SENDER:OPENSSL] {method} → {full_url}")
    print(f"[SENDER:OPENSSL] CMD: {' '.join(cmd)}")
    ...
    print("[SENDER:OPENSSL] RESPONSE:")
    print(out)

    if err:
        print("[SENDER:OPENSSL] STDERR:")
        print(err)


def _apply_ssl_config(ctx: ssl.SSLContext, args):
    if not getattr(args, "ssl_config_file", None):
        return

    with open(args.ssl_config_file, "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)

    ssl_cfg = cfg.get("ssl_config", {})

    cipher_string = ssl_cfg.get("cipher_string")
    if cipher_string:
        ctx.set_ciphers(cipher_string)
        print(f"[SSL] Applied cipher_string: {cipher_string}")
...
def run_http_server(args):
    if args.role == "RECEIVER":
        ...
        _apply_ssl_config(ssl_ctx, args)
        ...
        if method in ("POST", "PUT"):
            request_args["data"] = _read_body_once(args.body)

        print(f"[SENDER] {method} from {args.ip} → {full_url}")
        try:
            if args.openssl_config_file:
                _openssl_http_request(args, full_url, method)
            else:
