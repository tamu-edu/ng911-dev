...
class Comment:
    ...
    def to_dict(self) -> dict:
        return {"author": self.author, "comment": self.comment}
...
class LaunchConfig(Config):
    ...
    def from_dict(cls, config_dict: dict) -> "LaunchConfig":
        return cls(
            ...
            global_config=LaunchGlobalConfig(
                ...
                report_files=ReportFile(
                    **config_dict["launch_config"]["global_config"]["report_files"]
                ),
                log=LogConfig(
                    ...
                    output_file=cls._get_log_output_file_value(
                        ...
                        config_dict["launch_config"]["global_config"]["log"][
                            "output_file"
                        ],
                    ),
                    dev_mode=config_dict["launch_config"]["global_config"]["log"].get(
                        "dev_mode", False
                    ),
                ),
                id_summary=LaunchIdSummary(
                    eng_name=config_dict["launch_config"]["global_config"][
                        "id_summary"
                    ]["eng_name"],
                    supplier=SupplierSummary(
                        **config_dict["launch_config"]["global_config"]["id_summary"][
                            "supplier"
                        ]
                    ),
                    test_env=LaunchTestEnvSummary(
                ...
                comments=[
                    Comment(**comment)
                    for comment in config_dict["launch_config"]["global_config"][
                        "comments"
                    ]
                ],
            ),
            tests=[
                LaunchTest(
                    ...
                    forward_conduit_config=test.get("forward_conduit_config"),
                )
    ...
    @classmethod
    def _get_log_output_file_value(
        cls, output_folder: str | None = None, path: str | None = None
    ) -> str:
        if path:
            if "/" in path:
                return path
            else:
                return f"{output_folder}/{path}"
        else:
            return f"{output_folder}/log_output.log"
