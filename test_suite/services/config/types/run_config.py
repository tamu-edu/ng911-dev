...
from services.config.types.test_config import (
    VarInterfaces,
    MessageFilter,
    Preamble,
    Postamble,
...
class RunVariation:
    ...
    ssl_keys_file_path: Optional[str] = None
...
class GlobalConfig:
    ...
    def to_dict(self):
        return {
            ...
            "comments": [
                {"author": c.author, "comment": c.comment} for c in self.comments
            ],
        }
    ...
    def from_dict(cls, config_dict: dict) -> "RunConfig":
        return cls(
            ...
            global_config=GlobalConfig(
                ...
                report_files=ReportFile(
                    **config_dict["run_config"]["global"]["report_files"]
                ),
                log=LogConfig(**config_dict["run_config"]["global"]["log"]),
                id_summary=IdSummary(
                    lab=LabSummary(
                        **config_dict["run_config"]["global"]["id_summary"]["lab"]
                    ),
                    supplier=SupplierSummary(
                        **config_dict["run_config"]["global"]["id_summary"]["supplier"]
                    ),
                    iut=IUTSummary(
                        **config_dict["run_config"]["global"]["id_summary"]["iut"]
                    ),
                    test_env=TestEnvSummary(
                        **config_dict["run_config"]["global"]["id_summary"]["test_env"]
                    ),
                ),
                comments=[
                    Comment(**comment)
                    for comment in config_dict["run_config"]["global"]["comments"]
                ],
            ),
            tests=[
                RunTest(
                    name=test["name"],
                    requirements=[
                        RunRequirement(**req) for req in test["requirements"]
                    ],
                    preamble_list=[
                        Preamble(
                    ...
                    variations=[
                        RunVariation(
                            ...
                            interfaces=[
                                VarInterfaces(**v) for v in var.get("interfaces")
                            ],
                            mode=ScenarioMode(var["mode"]),
                            pcap_file=var.get("pcap_file"),
                            ssl_keys_file_path=var.get("ssl_keys_file_path"),
                ...
                "tests": [
                    {
                        "name": test.name,
                        "requirements": [
                            {"name": rq.name, "variations": [v for v in rq.variations]}
                            for rq in test.requirements or []
                        ],
                        "variations": [
                            {
                                "name": var.name,
                                ...
                                "interfaces": [
                                    ...
                                    for _if in var.interfaces or []
                                ],
                                "mode": var.mode or "online",
                                "pcap_file": var.pcap_file or "",
                                "ssl_keys_file_path": var.ssl_keys_file_path or "",
                                ...
                                "filtering_options": [
                                    {
                                        ...
                                        "http_request_method": mf.http_request_method
                                        or "",
                                        "response_status_code": mf.response_status_code
                                        or "",
                                        "body_contains": mf.body_contains or "",
                                        "header_contains": mf.header_contains or "",
                                    }
                                    for mf in var.filtering_options or []
                                ],
                            }
                            for var in test.variations or []
                        ],
                        "preamble_list": [
                            {
                                "method_name": preamble.method_name,
                                "required_for": preamble.required_for,
                                "kwargs": copy.deepcopy(preamble.kwargs),
                            }
                            for preamble in test.preamble_list or []
                        ],
                        "postamble_list": [
                            {
                                "method_name": postamble.method_name,
                                "kwargs": copy.deepcopy(postamble.kwargs),
                            }
                            for postamble in test.postamble_list or []
                        ],
                    }
