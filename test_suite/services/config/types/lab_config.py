...
if TYPE_CHECKING:
    from test_suite.services.config.types.forward_conduit_config import ForwardConduit
...
class Entity:
    ...
    def get_first_available_fqdn(self) -> Optional[str]:
        """Return the first available FQDN for the entity or None if no any is available."""
...
class LabConfig(Config):
        ...
        return cls(
            ...
            entities=[
                Entity(
                    ...
                    ip_range=entity.get("ip_range"),
                    ssh=SSH(**entity.get("ssh")) if entity.get("ssh") else None,
    ...
    def generate_prx_ifs(self, forward_conduit_config: "ForwardConduit") -> None:
        print("[Lab Config] Generating PROXY interfaces...")
        proxy_entity = self.get_proxy_server_entity()
        if proxy_entity is None:
            raise ValueError("[Lab Config] Proxy entity is not defined in LabConfig.")
        if proxy_entity.interfaces is None:
            proxy_entity.interfaces = []

        if not proxy_entity.ip_range:
            raise ValueError("[Lab Config] Proxy entity does not define ip_range.")

        network = ipaddress.ip_network(proxy_entity.ip_range, strict=False)
        available_ips = iter(network.hosts())

        used_ips = {
            interface.ip for interface in proxy_entity.interfaces or [] if interface.ip
        }

        for conduit in forward_conduit_config.forward_conduits:
            if self.get_if_by_name(conduit.proxy_if) is not None:
                continue

            to_if = self.get_if_by_name(conduit.to_if)
            if to_if is None:
                raise ValueError(
                    f"[Lab Config] Cannot generate proxy interface '{conduit.proxy_if}': "
                    f"interface '{conduit.to_if}' does not exist."
                )
            ...
            if proxy_ip is None:
                raise ValueError(
                    f"[Lab Config] No available IP addresses left in proxy ip_range "
                    f"'{proxy_entity.ip_range}'."
                )

            proxy_interface = Interface(
                name=conduit.proxy_if,
                ip=proxy_ip,
                port_mapping=[
                    PortMapping(
                        name=f"{mapping.name}_PRS",
                        protocol=mapping.protocol,
                        port=mapping.port,
                        transport_protocol=mapping.transport_protocol,
                    )
                    for mapping in to_if.port_mapping or []
                ],
            )

            proxy_entity.interfaces.append(proxy_interface)

            print(
                f"[Lab Config] Generated PROXY interface: "
                f"{proxy_interface.name} -> {proxy_interface.ip}"
            )
    ...
    def get_conformance_iut_entity(self) -> Entity | None:
        """
        Return the IUT Entity.
        In Conformance mode lab configuration contains only 1 IUT.
        """
        for entity in self.entities or []:
            if StubServerRole.IUT in entity.role:
                return entity
        return None

    def get_forward_server_entity(self) -> Entity | None:
        for entity in self.entities or []:
            if entity.function == EntityFunction.FORWARD.value:
                return entity
        return None

    def get_proxy_server_entity(self) -> Entity | None:
        for entity in self.entities or []:
            if entity.function == EntityFunction.PROXY.value:
                return entity
        return None

    def get_if_by_name(self, if_name: str) -> Interface | None:
        for entity in self.entities or []:
            for interface in entity.interfaces or []:
    ...
    def get_entity_by_if_name(self, if_name: str) -> Entity | None:
        for entity in self.entities or []:
            for interface in entity.interfaces or []:
                if interface.name == if_name:
                    return entity
    ...
    def get_port_mapp_by_name(self, port_name: str) -> PortMapping | None:
        for entity in self.entities or []:
            for interface in entity.interfaces or []:
