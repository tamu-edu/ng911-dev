...
from .config_enum import EntityFunction
...
class ConfigService:
    ...
    @classmethod
    def validate_forward_conduit_config(
        cls, config_file_path: str, output_file: str | None = None
    ) -> bool:
        forward_conduit_config = cls.__load_config_file(config_file_path)
        status, errors = cls.__validate_with_schema(
            data=forward_conduit_config,
            schema=FORWARD_CONDUIT_CONFIG_SCHEMA,
            parent_data=forward_conduit_config,
        )
        if status:
            print("Forward Conduits Configuration file Validation Successful")
        else:
            print("Forward Conduits Configuration file Validation Failed")
            if output_file:
                with open(output_file, "w") as file:
                    file.write("\n".join(errors))
                print(f"Errors successfully written to {output_file}")
    ...
    @classmethod
    def _if_required_has_exception(
        cls, field: str, data: dict, parent_data: dict | None
    ) -> bool:
        ...
        if parent_data:
            global_config = parent_data.get("global_config")
            if (
                isinstance(global_config, dict)
                and global_config.get("type") == "conformance"
                and field == "forward_conduit_config"
            ):
                return True

        if (
            parent_data
            and parent_data.get("function") != EntityFunction.FORWARD.value
            and field
            in [
                "ssh",
            ]
    ...
    def __validate_with_schema(
        ...
        for field, rules in schema.items():
            ...
            if rules.get("required") and field not in data:
                if not cls._if_required_has_exception(field, data, parent_data):
