                        ...
                        run_variations.append(
                            RunVariation(
                                ...
                                filtering_options=(
                                    ...
                                    else cls.get_filtering_options(_variation.params, lab_config)
                                )
                            )
                        )
    ...
    def verify_interface_data_presence(self, lab_config_if_list):
        ...
        for interface in required_interfaces_from_tests:
            ...
            if not lab_config_if_data:
                return False, f'There is no {interface} data specified in the lab_config.'
            if not any([lab_config_if_data.get('fqdn', None),  lab_config_if_data.get('ip', None)]):
                return False, f"Both 'fqdn' and 'ip' cannot be empty in the lab_config for {interface} interface."
        return True, ''
