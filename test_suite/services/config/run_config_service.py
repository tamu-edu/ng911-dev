...
from test_suite.services.config.config_enum import ScenarioMode, FilterMessageType
from test_suite.services.config.config_service import ConfigService
from test_suite.services.config.errors.run_config_generation_error import (
    RunConfigGenerationError,
)
...
class RunConfigService:
    ...
    def __init__(self, run_config: RunConfig, run_config_filename: str | None = None):
    ...
    def from_dict(cls, config_dict: dict) -> "RunConfigService":
        ...
        return cls(run_config=RunConfig.from_dict(config_dict))

    @classmethod
    def from_launch_config(
        cls, launch_config: LaunchConfig, test: LaunchTest, lab_config: LabConfig
    ) -> "RunConfigService":
        """Create a RunConfigService instance from provided launch_config."""

        print(
            f"Validating and parsing LAB INFO -> {launch_config.global_config.lab_info}"
        )
        if not ConfigService.validate_lab_info(launch_config.global_config.lab_info):
            print("❌ LAB INFO parsing failed.")
            raise RunConfigGenerationError(
                "Impossible to generate run_config due to LAB INFO errors"
            )
        print("✅ LAB INFO successfully parsed.")
        ...
        if not ConfigService.validate_test_info(f"{TEST_CONF_FOLDER}/test__info.yaml"):
            print("❌ TEST INFO parsing failed.")
            raise RunConfigGenerationError(
                "Impossible to generate run_config due to TEST INFO errors"
            )
        print("✅ TEST INFO successfully parsed.")
    ...
    def extract_rq_test_id(rq_schema_key: str, test_ids_list: list) -> list:
        ...
        _ti_dict = REQUIREMENTS_SCHEMA.get(rq_schema_key)
        if _ti_dict:
            _tid = _ti_dict.get("test_id")
            if _tid and _tid not in test_ids_list:
                test_ids_list.append(_tid)
        else:
            print(
                f"ℹ️ {rq_schema_key} cannot be tested yet "
                f"as we do not have implemented the tests for it."
            )
        return test_ids_list

    # TODO delete as may become deprecated
    @staticmethod
    def __get_stimulus_src_interface_name(entities, params: dict) -> str:
        send_if_names = {
            msg.get("if_name")
            for msg in params.get("messages", [])
            if msg.get("action") == "send" and msg.get("if_name")
        }

        if not send_if_names:
            return ""

        for from_entity in entities:
            if StubServerRole.SENDER in from_entity.role:
                for to_entity in entities:
                    if StubServerRole.IUT in to_entity.role:
                        _if_name = f"IF_{from_entity.name}_{to_entity.name}"
                        if _if_name in send_if_names:
                            return _if_name
        return ""

    # TODO delete as may become deprecated
    @staticmethod
    def __get_output_src_interface_name(entities, params: dict) -> str:
        receive_if_names = {
            msg.get("if_name")
            for msg in params.get("messages", [])
            if msg.get("action") == "receive" and msg.get("if_name")
        }

        if not receive_if_names:
            return ""

        for to_entity in entities:
            if StubServerRole.RECEIVER in to_entity.role:
                for from_entity in entities:
                    if StubServerRole.IUT in from_entity.role:
                        # _if_name is the DEST interface (RECEIVER ← IUT)
                        _if_name = f"IF_{to_entity.name}_{from_entity.name}"
                        if _if_name in receive_if_names:
                            # we return the SRC interface (IUT → RECEIVER)
                            return f"IF_{from_entity.name}_{to_entity.name}"
        return ""
    ...
    def get_message_filtering_options(
        cls, message_type: FilterMessageType | str, params: dict, lab_config: LabConfig
    ) -> MessageFilter:
        ...
        if message_type == FilterMessageType.STIMULUS.value:
            src_if = cls.__get_stimulus_src_interface_name(entities, params)
            ...
            for message in params["messages"]:
                if message["action"] == "send":
                    ...
                    if (
                        ...
                        _kwargs["response_status_code"] = (
                            message.get("response_code") or ""
                        )
            ...
            src_if = cls.__get_output_src_interface_name(entities, params)
            dst_if = cls.__get_dst_interface_name(src_if)
            for message in params["messages"]:
    ...
    # TODO delete as may become deprecated
    @classmethod
    def get_filtering_options(cls, params: dict, lab_config: LabConfig):
            ...
            if message["action"] == "send":
                filter_options.append(
                    cls.get_message_filtering_options(
                        FilterMessageType.STIMULUS.value, params, lab_config
                    ),
                )
        for message in params["messages"]:
            if message["action"] == "receive":
                filter_options.append(
                    cls.get_message_filtering_options(
                        FilterMessageType.OUTPUT.value, params, lab_config
                    )
                )
    ...
    def _generate_conformance_run_config(
        ...
        global_config = GlobalConfig(
            ...
            comments=[
                Comment(author=_comment.author, comment=_comment.comment)
                for _comment in launch_config.global_config.comments
            ],
            id_summary=IdSummary(
                ...
                iut=IUTSummary(
                    ...
                    cs_static_conform=test.iut.cs_static_conform,
                ),
                test_env=TestEnvSummary(
                    ixit_id=launch_config.global_config.id_summary.test_env.ixit_id,
                    test_method=launch_config.global_config.id_summary.test_env.test_method,
        ...
        test_ids_list: List[Any] = []
        rq_type_all_list: List[Any] = []
        test_configs: List[Any] = []

        for rq in test.requirements:
            _rq, _type, _num = rq.split("_")
            if test.iut.type != _type and not (
                test.iut.type == "BRIDGE" and _type == "BRG"
            ):
                raise RunConfigGenerationError(
                    f"Unexpected RQ - {rq} for the test of {test.iut.type}"
                )

            if _num == "*":
                rq_type_all_list.append(f"{_rq}_{_type}")
                for rq_schema_key in REQUIREMENTS_SCHEMA.keys():
                    if f"{_rq}_{_type}" in rq_schema_key:
                        test_ids_list = cls.extract_rq_test_id(
                            rq_schema_key, test_ids_list
                        )
        ...
        for test_id in test_ids_list:
            ...
            if not ConfigService.validate_test_config(test_config_path):
                print("❌ TEST CONFIG parsing failed.")
                raise RunConfigGenerationError(
                    "Impossible to generate run_config due to TEST CONFIG errors"
                )
            print("✅ TEST CONFIG successfully parsed.")
            test_config = TestConfig.from_dict(
                ConfigService.parse_config_file(test_config_path)
            )
            test_configs.append(test_config)

        run_requirements: List[Any] = []
        run_variations: List[Any] = []
        preamble_list: List[Any] = []
        postamble_list: List[Any] = []

        for test_config in test_configs:
            for _test in test_config.conformance.tests:
                _var_name_list: List[Any] = []
                for _requirement in _test.requirements:
                    if _requirement.name in test.requirements or (
                        rq_type in _requirement.name for rq_type in rq_type_all_list
                    ):
                        run_requirements.append(
                            RunRequirement(
                                name=_requirement.name,
                                variations=cls.__get_var_list_for_rq(
                                    _requirement.variations, _test.variations
                                ),
                            )
                        )
                ...
                for _variation in _test.variations:
                    if (
                        _variation.name.lower() in _var_name_list
                        or "all" in _var_name_list
                    ):
                            ...
                            RunVariation(
                                ...
                                filtering_options=(
                                    [
                                        ...
                                        for f in _variation.filtering_options
                                    ]
                                    if _variation.filtering_options
                                    else cls.get_filtering_options(
                                        _variation.params, lab_config
                                    )
                                ),
                            )
                        )
                for _preamble in _test.preamble_list:
                    preamble_list.append(_preamble)
                for _postamble in _test.postamble_list:
                    postamble_list.append(_postamble)
    ...
    def get_required_interfaces(self) -> set:
        result_list = set()
        for test in self.run_config.tests:
            for variation in test.variations:
                for interface in variation.interfaces:
                    result_list.add(interface.name)
        return result_list

    def verify_interface_data_presence(self, lab_config_if_list):
        required_interfaces_from_tests = self.get_required_interfaces()
        if len(required_interfaces_from_tests) < 1:
            return False, "No IUT interfaces has been found in the run_config."
        for interface in required_interfaces_from_tests:
            lab_config_if_data = lab_config_if_list.get(interface, None)
            if not lab_config_if_data:
                return (
                    False,
                    f"There is no {interface} data specified in the lab_config.",
                )
            if not any(
                [
                    lab_config_if_data.get("fqdn", None),
                    lab_config_if_data.get("ip", None),
                ]
            ):
                return (
                    False,
                    f"Both 'fqdn' and 'ip' cannot be empty in the lab_config for {interface} interface.",
                )
        return True, ""
