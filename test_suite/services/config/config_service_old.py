...
from .config_enum import ScenarioMode, TestId
...
class ConfigService(metaclass=LoggingMeta):
    ...
    @classmethod
    def __validate_config_file_exists(
        cls, config_file_path
    ) -> tuple[bool, list[str]] | tuple[bool, dict]:
        """
        Loads and validates the configuration file exists without creating an instance.
        :returns:
            (bool, list): Tuple of validity status and a list of error messages.
    ...
    def __validate_test_config(cls, config) -> tuple[bool, list[str]]:
        ...
        if "test_config" not in config:
            errors.append(
                "Missing 'test_config' value. Check your configuration structure."
            )
        else:
            ...
            if "conformance" not in test_config:
                errors.append(
                    "Missing 'conformance' value. Check your configuration structure."
                )
            else:
                g_status, g_errors = cls.__validate_conformance_block(
                    test_config.get("conformance")
                )
                if not g_status:
    ...
    @classmethod
    def __validate_conformance_block(
        cls, conformance_block: dict
    ) -> tuple[bool, list[str]]:
        """
        ...
        if "tests" not in conformance_block:
            errors.append(
                "Missing 'tests' value in 'conformance' block. Check your configuration structure."
            )
        else:
            ...
            if not isinstance(tests, list):
                ...
                for test in tests:
                    if not isinstance(test, dict):
                        errors.append(
                            "Each 'test' in 'conformance.tests' block should dictionaries."
                        )
                    else:
                        if "name" not in test:
    ...
    def __validate_run_config(cls, config) -> tuple[bool, list[str]]:
        ...
        if "run_config" not in config:
            errors.append(
                "Missing 'run_config' value. Check your configuration structure."
            )
        else:
            ...
            if "global" not in run_config:
                errors.append(
                    "Missing 'global' value. Check your configuration structure."
                )
            else:
                g_status, g_errors = cls.__validate_global_block(
                    run_config.get("global")
                )
                if not g_status:
                    errors.extend(g_errors)
            ...
            if "report_files" not in run_config:
                errors.append(
                    "Missing 'report_files' value. Check your configuration structure."
                )
            else:
                g_status, g_errors = cls.__validate_report_block(
                    run_config.get("report_files")
                )
                if not g_status:
                    errors.extend(g_errors)
            ...
            if "log" not in run_config:
                errors.append(
                    "Missing 'log' value. Check your configuration structure."
                )
            else:
        ...
        if not isinstance(report_block, list):
            ...
            for report in report_block:
                if "type" not in report:
                    errors.append(
                        f"Missing 'type' value in {r_index} element of 'report' block."
                    )
                else:
                    report_type = report.get("type")
                    if not isinstance(report_type, str):
                        errors.append(
                            f"Error - 'type' value in {r_index} element of 'report' block should be an STR"
                        )
                    else:
                ...
                if "path" not in report:
                    errors.append(
                        f"Missing 'path' value in {r_index} element of 'report' block."
                    )
                else:
                    path = report.get("path")
                    if not isinstance(path, str):
                        errors.append(
                            f"Error - 'path' value in {r_index} element of 'report' block should be an STR"
                        )
                    else:
                        if len(path) == 0:
                            errors.append(
        ...
        if "level" not in log_block:
            ...
            if not isinstance(log_level, str):
                ...
                if log_level not in LogLevel.list():
                    errors.append(
                        f"'log.level' value should be  one of {LogLevel.list()}"
                    )
    ...
    @classmethod
    def __validate_stimulus_or_output(
        cls, message: dict, prefix: str, test_index: int, sc_index: int
    ) -> list:
        errors = []

        if "src_interface" not in message:
            errors.append(
                f"Missing 'src_interface' value in {prefix} block of {test_index} element of 'test' block"
                f"of {sc_index} element of 'scenarios' block."
            )
        else:
            if not isinstance(message.get("src_interface"), str):
                errors.append(
                    f"Value 'src_interface' in {prefix} block of {test_index} element of 'test' block"
                    f"of {sc_index} element of 'scenarios' block SHOULD be str."
                )

        if "dst_interface" not in message:
            errors.append(
                f"Missing 'dst_interface' value in {prefix} block of {test_index} element of 'test' block"
                f"of {sc_index} element of 'scenarios' block."
            )
        else:
            if not isinstance(message.get("dst_interface"), str):
                errors.append(
                    f"Value 'dst_interface' in {prefix} block of {test_index} element of 'test' block"
                    f"of {sc_index} element of 'scenarios' block SHOULD be str."
                )

        if "sip_method" not in message:
            errors.append(
                f"Missing 'sip_method' value in {prefix} block of {test_index} element of 'test' block"
                f"of {sc_index} element of 'scenarios' block."
            )
        else:
            if not isinstance(message.get("sip_method"), str):
                errors.append(
                    f"Value 'sip_method' in {prefix} block of {test_index} element of 'test' block"
                    f"of {sc_index} element of 'scenarios' block SHOULD be str."
                )
            else:
        ...
        if "http_request_method" not in message:
            errors.append(
                f"Missing 'http_request_method' value in {prefix} block of {test_index} element of 'test'"
                f"block of {sc_index} element of 'scenarios' block."
            )
        else:
            if not isinstance(message.get("http_request_method"), str):
                errors.append(
                    f"Value 'http_request_method' in {prefix} block of {test_index} element of 'test' block"
                    f"of {sc_index} element of 'scenarios' block SHOULD be str."
                )
            else:
        ...
        if "response_status_code" not in message:
            errors.append(
                f"Missing 'response_status_code' value in {prefix} block of {test_index} element of 'test'"
                f"block of {sc_index} element of 'scenarios' block."
            )
        else:
            if not isinstance(message.get("response_status_code"), str):
                ...
                if len(message.get("response_status_code")) > 0:
                    if (
                        int(message.get("response_status_code"))
                        not in HTTPStatusCodeEnum.list()
                    ):
                        errors.append(
                            f"Value 'response_status_code' in {prefix} block of {test_index} element of "
    ...
    @classmethod
    def __validate_scenarios_block(
        cls, scenarios_block: dict
    ) -> tuple[bool, list[str]]:
        """
        Validates the given report block of test configuration.
        ...
        if not isinstance(scenarios_block, list):
            ...
            for scenario in scenarios_block:
                ...
                if "name" not in scenario:
                    errors.append(
                        f"Missing 'name' value in {sc_index} element of 'scenarios' block."
                    )
                else:
                    if not isinstance(scenario.get("name"), str):
                        errors.append(
                            f"Value 'name' in {sc_index} element of 'scenarios' block should be a STR."
                        )

                if "mode" not in scenario:
                    errors.append(
                        f"Missing 'mode' value in {sc_index} element of 'scenarios' block."
                    )
                else:
                    if not isinstance(scenario.get("mode"), str):
                        errors.append(
                            f"Value 'mode' in {sc_index} element of 'scenarios' block should be a STR."
                        )
                    else:
                        if scenario.get("mode") not in ScenarioMode.list():
                            errors.append(
                                f"Value 'mode' in {sc_index} element of 'scenarios' block "
                                f"should be one of {ScenarioMode.list()}"
                            )
                        if scenario.get("mode") == ScenarioMode.PCAP:
                            if "pcap_file" not in scenario:
                                errors.append(
                                    f"Missing 'pcap_file' value in {sc_index} element of 'scenarios' block."
                                )
                            else:
                                if not isinstance(scenario.get("pcap_file"), str):
                                    ...
                                    status, result = cls.__validate_file_exists(
                                        scenario.get("pcap_file")
                                    )
                                    if not status:
                                        errors.extend(result)
                            ...
                            if "stimulus_body" not in scenario:
                                errors.append(
                                    f"Missing 'stimulus_body' value in {sc_index} element of 'scenarios' block."
                                )
                            else:
                                if not isinstance(scenario.get("stimulus_body"), str):
                                    errors.append(
                                        f"Value 'stimulus_body' in {sc_index} element of 'scenarios' block "
                                        f"should be a STR."
                                    )
                                else:
                                    status, result = cls.__validate_file_exists(
                                        scenario.get("stimulus_body")
                                    )
                                    if not status:
                                        errors.extend(result)
                ...
                if "tests" not in scenario:
                    errors.append(
                        f"Missing 'tests' value in {sc_index} element of 'scenarios' block."
                    )
                else:
                    tests = scenario.get("tests")
                    if not isinstance(tests, list):
                        errors.append(
                            f"Value 'tests' in {sc_index} element of 'scenarios'"
                    ...
                    for test in tests:
                        if not isinstance(test, dict):
                            errors.append(
                                f"Tests int 'tests' list in {sc_index} element of 'scenarios' block "
                                f"should be dictionaries."
                            )
                        else:
                            if "name" not in test:
                                errors.append(
                                    f"Missing 'name' value in {test_index} element of 'test' block"
                                    f"of {sc_index} element of 'scenarios' block."
                                )
                            else:
                                if not isinstance(test.get("name"), str):
                                    errors.append(
                                        f"Value 'name' in {test_index} element of 'test' block"
                                        f"of {sc_index} element of 'scenarios' block SHOULD be str."
                                    )

                            if "test_id" not in test:
                                errors.append(
                                    f"Missing 'test_id' value in {test_index} element of 'test' block"
                                    f"of {sc_index} element of 'scenarios' block."
                                )
                            else:
                                if not isinstance(test.get("test_id"), str):
                                    errors.append(
                                        f"Value 'test_id' in {test_index} element of 'test' block"
                                        f"of {sc_index} element of 'scenarios' block SHOULD be str."
                                    )
                                else:
                                    if test.get("test_id") not in TestId.list():
                                        errors.append(
                                            f"Value 'test_id' in {sc_index} element of 'scenarios' block "
                                            f"should be an exact name of required Test module"
                                        )

                            if "ixit_file_path" not in test:
                                errors.append(
                                    f"Missing 'ixit_file_path' value in {test_index} element of 'test' block"
                                    f"of {sc_index} element of 'scenarios' block."
                                )
                            else:
                                if not isinstance(test.get("ixit_file_path"), str):
                                    errors.append(
                                        f"Value 'ixit_file_path' in {test_index} element of 'test' block"
                                        f"of {sc_index} element of 'scenarios' block SHOULD be str."
                                    )
                                if len(test.get("ixit_file_path")) > 0:
                                    status, result = cls.__validate_file_exists(
                                        test.get("ixit_file_path")
                                    )
                                    if not status:
                                        errors.extend(result)
                            ...
                            if "stimulus_message" not in test:
                                errors.append(
                                    f"Missing 'stimulus_message' value in {test_index} element of 'test' block"
                                    f"of {sc_index} element of 'scenarios' block."
                                )
                            else:
                                if not isinstance(test.get("stimulus_message"), dict):
                                    errors.append(
                                        f"Value 'stimulus_message' in {test_index} element of 'test' block"
                                        f"of {sc_index} element of 'scenarios' block SHOULD be dictionary."
                                    )
                                message = test.get("stimulus_message")
                                errors.extend(
                                    cls.__validate_stimulus_or_output(
                                        message,
                                        "stimulus_message",
                                        test_index,
                                        sc_index,
            ...
            for entity in entities_block:
                if not isinstance(entity, dict):
                    ...
                    if "name" not in entity:
                        errors.append(
                            f"Missing 'name' value in {entity_index} 'entity' block."
                        )
                    else:
                        name = entity.get("name")
                        if not isinstance(name, str):
                            errors.append(
                                f"'entities.name' of {entity_index} 'entity' block should be an STR"
                            )

                    if "type" not in entity:
                        errors.append(
                            f"Missing 'type' value in {entity_index} 'entity' block."
                        )
                    else:
                        ent_type = entity.get("type")
                        if not isinstance(ent_type, str):
                            errors.append(
                                f"'entities.type' of {entity_index} 'entity' block should be an STR"
                            )
                        else:
                            if ent_type not in EntityType.list():
                                errors.append(
                                    f"'entities.type' of {entity_index} 'entity' block"
                                    f" should be one of {EntityType.list()}"
                                )

                    if "fqdn" not in entity:
                        errors.append(
                            f"Missing 'fqdn' value in {entity_index} 'entity' block."
                        )
                    else:
                        fqdn = entity.get("fqdn")
                        if not isinstance(fqdn, str):
                            errors.append(
                                f"'entities.fqdn' of {entity_index} 'entity' should be an STR"
                            )
                        else:
                            if len(fqdn) > 0 and not cls.is_valid_fqdn(fqdn):
                                errors.append(
                                    f"'entities.fqdn' of {entity_index} 'entity' block"
                    ...
                    if "certificate_file" not in entity:
                        errors.append(
                            f"Missing 'certificate_file' value in {entity_index} 'entity' block."
                        )
                    else:
                        certificate_file = entity.get("certificate_file")
                        if not isinstance(certificate_file, str):
                            errors.append(
                                f"'entities.certificate_file' of {entity_index} 'entity' block"
                                f" should be an STR"
                            )
                        else:
                            status, f_errors = cls.__validate_file_exists(
                                certificate_file
                            )
                    ...
                    if "certificate_key" not in entity:
                        errors.append(
                            f"Missing 'certificate_key' value in {entity_index} 'entity' block."
                        )
                    else:
                        certificate_key = entity.get("certificate_key")
                        if not isinstance(certificate_key, str):
                            errors.append(
                                f"'entities.certificate_key' of {entity_index} 'entity' block"
                                f" should be an STR"
                            )
                        else:
                            status, f_errors = cls.__validate_file_exists(
                    ...
                    if "interfaces" not in entity:
                        errors.append(
                            f"Missing 'interfaces' value in {entity_index} 'entity' block."
                        )
                    else:
                        ...
                        if not isinstance(interfaces, list):
                            ...
                            for interface in interfaces:
                                if "name" not in interface:
                                    errors.append(
                                        f"Missing 'name' value in {interface_index} 'interface' block"
                                    ...
                                    if not isinstance(name, str):
                                        errors.append(
                                            f"'name' of {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block"
                                            f" should be an STR"
                                        )
                                ...
                                if "fqdn" not in interface:
                                    errors.append(
                                        f"Missing 'fqdn' value in {interface_index} 'interface' block"
                                        f"of {entity_index} 'entity' block."
                                    )
                                else:
                                    ...
                                    if not isinstance(fqdn, str):
                                        errors.append(
                                            f"'interface.fqdn' value in {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block should be an STR"
                                        )
                                    else:
                                        if len(fqdn) > 0 and not cls.is_valid_fqdn(
                                            fqdn
                                        ):
                                            errors.append(
                                                f"'interface.fqdn' value in {interface_index} 'interface' block"
                                ...
                                if "ip" not in interface:
                                    errors.append(
                                        f"Missing 'ip' value in {interfaces} 'interface' block"
                                        f"of {entity_index} 'entity' block."
                                    )
                                else:
                                    ip = interface.get("ip")
                                    if not isinstance(ip, str):
                                        errors.append(
                                            f"'ip' of {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block"
                                            f" should be an STR"
                                        )
                                    else:
                                ...
                                if "mask" not in interface:
                                    errors.append(
                                        f"Missing 'mask' value in {interface_index} 'interface' block"
                                        f"of {entity_index} 'entity' block."
                                    )
                                else:
                                    ...
                                    if not isinstance(mask, str):
                                        errors.append(
                                            f"'mask' of {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block"
                                            f" should be an STR"
                                        )
                                    else:
                                        if len(
                                            mask
                                        ) > 0 and not cls.is_valid_subnet_mask(mask):
                                            errors.append(
                                                f"'mask' of {interface_index} 'interface' block"
                                                f"of {entity_index} 'entity' block"
                                ...
                                if "gateway" not in interface:
                                    errors.append(
                                        f"Missing 'gateway' value in {interface_index} 'interface' block"
                                        f"of {entity_index} 'entity' block."
                                    )
                                else:
                                    ...
                                    if not isinstance(gateway, str):
                                        errors.append(
                                            f"'gateway' of {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block"
                                            f" should be an STR"
                                        )
                                    else:
                                        if len(gateway) > 0 and not cls.is_valid_ip(
                                            gateway
                                        ):
                                            errors.append(
                                                f"'gateway' of {interface_index} 'interface' block"
                                ...
                                if "dns" not in interface:
                                    errors.append(
                                        f"Missing 'dns' value in {interface_index} 'interface' block"
                                        f"of {entity_index} 'entity' block."
                                    )
                                else:
                                    ...
                                    if not isinstance(dns, list):
                                        errors.append(
                                            f"'dns' of {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block"
                                ...
                                if "port_mapping" not in interface:
                                    errors.append(
                                        f"Missing 'port_mapping' value in {interface_index} 'interface' block"
                                        f"of {entity_index} 'entity' block."
                                    )
                                else:
                                    ...
                                    if not isinstance(port_mapping, list):
                                        errors.append(
                                            f"'port_mapping' of {interface_index} 'interface' block"
                                            f"of {entity_index} 'entity' block"
                                            f" should be an lIST"
                                        )
                                    else:
                                        ...
                                        for entry in port_mapping:
                                            if not isinstance(entry, dict):
                                                errors.append(
                                                    f"'{protocol_index} entry in 'port_mapping' list "
                                                    f"of {interface_index} 'interface' block"
                                                    f"of {entity_index} 'entity' block"
                                                    f" should be an dict"
                                                )
                                                ...
                                                if "protocol" not in entry:
                                                    ...
                                                    if (
                                                        entry.get("protocol")
                                                        not in PacketTypeEnum.list()
                                                        and entry.get("protocol")
                                                        not in TransportProtocolEnum.list()
                                                    ):
                                                        errors.append(
                                                            f"'protocol' value in {protocol_index} entry of "
                                                            f"'port_mapping' list "
                                                            f"of {interface_index} 'interface' block"
                                                            f"of {entity_index} 'entity' block"
                                                ...
                                                if "port" not in entry:
                                                    ...
                                                    if (
                                                        ...
                                                        or entry.get("port") < 0
                                                        or entry.get("port") > 65535
                                                    ):
                                                        errors.append(
                                                            f"'port' value in '{protocol_index} entry in "
    ...
    def __validate_lab_config(cls, config) -> tuple[bool, list[str]]:
        ...
        if "lab_config" not in config:
            errors.append(
                "Missing 'lab_config' value. Check your configuration structure."
            )
        else:
            ...
            if "entities" not in lab_config:
                errors.append(
                    "Missing 'entities' value. Check your configuration structure."
                )
            else:
                g_status, g_errors = cls.__validate_entities_block(
                    lab_config.get("entities")
                )
                if not g_status:
                    errors.extend(g_errors)
