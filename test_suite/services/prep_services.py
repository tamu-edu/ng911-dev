...
def get_logevent_id_list_from_json(
    ...
    with open(json_source, "r") as f:
        data = json.load(f)
        if "logEventArray" in data.keys():
            log_event_array_dict = data.get("logEventArray", {})
            if "logEventContainers" in log_event_array_dict.keys():
                log_event_cont_list = log_event_array_dict.get("logEventContainers", [])
                for log_event_container in log_event_cont_list:
                    if "logEvent" in log_event_container.keys() and "logEventId" in log_event_container.keys:
                        _, payload_data = decode_jws(log_event_container.get("logEvent"), key_path, key_password)
