...
from cryptography.hazmat.primitives.asymmetric import ec, ed448, rsa, ed25519
...
from test_suite.enums import PacketTypeEnum
...
def generate_jws(
    ...
    return gjws(
        ...
        disable_eddsa_checks=disable_eddsa_checks,
    )
...
def generate_random_certificate(
    ...
    if algorithm.lower() == "ecdsa":
        ...
        raise ValueError("Unsupported algorithm. Use 'ecdsa', 'ed448', or 'rsa'.")
...
SigningAlgorithm = Literal["Ed448", "Ed25519", "RSA2048", "RSA3072", "RSA4096"]

_RSA_KEY_SIZES: dict[str, int] = {
    "RSA2048": 2048,
    "RSA3072": 3072,
    "RSA4096": 4096,
}
...
def _expected_key_type(algorithm: SigningAlgorithm) -> type:
    if algorithm == "Ed448":
        return ed448.Ed448PrivateKey
    if algorithm == "Ed25519":
        return ed25519.Ed25519PrivateKey
    if algorithm in _RSA_KEY_SIZES:
        return rsa.RSAPrivateKey
    raise ValueError(f"Unsupported algorithm: {algorithm}")


def _generate_leaf_key(algorithm: SigningAlgorithm):
    if algorithm == "Ed448":
    ...
    if algorithm in _RSA_KEY_SIZES:
        return rsa.generate_private_key(
            ...
            key_size=_RSA_KEY_SIZES[algorithm],
        )
    raise ValueError(f"Unsupported algorithm: {algorithm}")


def _signing_hash_algorithm(
    algorithm: SigningAlgorithm,
) -> Optional[hashes.HashAlgorithm]:
    if algorithm in ("Ed448", "Ed25519"):
        return None
    if algorithm in _RSA_KEY_SIZES:
...
def generate_pca_signed_certificate(
    ...
    output_key_file: str,
    algorithm: SigningAlgorithm = "Ed448",
    ...
    """
    Generate an leaf certificate signed by a PCA (issuer) certificate.

    This function:
        1. Loads a PCA certificate (PEM).
        2. Loads a PCA private key (PEM, optionally encrypted).
    ...
    algorithm : str
        Name of algorithm - allowed is one of "Ed448", "Ed25519", "RSA2048", "RSA3072", "RSA4096".
    ...
    try:
        expected_key_type = _expected_key_type(algorithm)
    except ValueError as e:
        if fail_or_empty(str(e), e):
            return
    ...
    if not isinstance(pca_private_key, expected_key_type):
        if fail_or_empty(
            f'PCA key "{pca_cert_key}" does not match algorithm="{algorithm}" '
            f"(expected {expected_key_type.__name__}, got {type(pca_private_key).__name__})"
        ):
            return
    ...
    try:
        ...
        if not isinstance(issuer_pub, pca_private_key.public_key().__class__):
            if fail_or_empty(
                f'PCA certificate "{pca_cert_file}" public key type does not match its private key type.'
            ):
                return
    except Exception as e:
    ...
    try:
        leaf_private_key = _generate_leaf_key(algorithm)
    except Exception as e:
    ...
    try:
        if leaf_private_key:
            ...
            now = datetime.utcnow()
            sign_hash = _signing_hash_algorithm(algorithm)
            ...
            cert = (
                ...
                .sign(private_key=pca_private_key, algorithm=sign_hash)
            )
    except Exception as e:
...
def generate_identifier(identifier_type: str, fqdn: str, set_default: str = "") -> str:
    identifiers = {
    ...
    if identifier_type.lower() not in identifiers.keys():
        print(
            f"❌ [generate_identifier] Unknown type provided: '{identifier_type}', returning None"
        )
        return ""
    if not fqdn or not is_valid_fqdn(fqdn):
        if not set_default:
            print(
                f"❌ [generate_identifier] Incorrect FQDN provided '{fqdn}', returning None"
            )
            return ""
        elif set_default.lower() == "true":
            print(
                f"⚠️ [generate_identifier] Incorrect FQDN provided: '{fqdn}', "
                f"returning default 'default-fqdn.ng911.example'"
            )
            fqdn = "default-fqdn.ng911.example"
...
def get_interface_address(interface_name: str, lab_config: dict) -> str:
    ...
    """
    if len(interface_name.split("_")) != 3 and not interface_name.endswith("_PRS"):
        print(f"❌ Given interface name is incorrect: {interface_name}")
        return ""
    entity = "PROX" if interface_name.endswith("_PRS") else interface_name.split("_")[1]
...
def get_interface_port_number(
    interface_name: str, protocol: str, lab_config: dict
) -> str:
    """
    Gets port number of given interface name and specified protocol
    :interface_name: e.g. IF_OSP_BCF
    :protocol: e.g. SIP
    :return: port number
    """
    if len(interface_name.split("_")) != 3 and not interface_name.endswith("_PRS"):
        print(f"❌ Given interface name is incorrect: {interface_name}")
        return ""
    if protocol not in PacketTypeEnum.list():
        print(f"❌ Given protocol name is incorrect: {protocol}")
        return ""
    entity = "PROX" if interface_name.endswith("_PRS") else interface_name.split("_")[1]
    interfaces = extract_from_config(
        lab_config["lab_config"]["entities"], "interfaces", "name", entity
    )
    port_mapping = extract_from_config(
        interfaces, "port_mapping", "name", interface_name
    )
    port = extract_from_config(port_mapping, "port", "protocol", protocol)
    return str(port)


def encode_base64url(payload: str, lab_config: dict) -> str:
