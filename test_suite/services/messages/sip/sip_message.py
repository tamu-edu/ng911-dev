...
from services.messages.sip.inner_types.refer_to import ReferTo
from services.messages.sip.inner_types.referred_by import ReferredBy
...
class SipMessage(Message):
    ...
    refer_to: list[ReferTo] | None = None
    referred_by: ReferredBy | None = None
    ...
    def from_packet(cls, packet: Any) -> "SipMessage | None":
        ...
        return cls(
            ...
            call_info=list_or_none(
                [
                    CallInfo.parse(part)
                    for v in get_field_values(sip, "call_info")
                    for part in split_header_value_list(v)
                ]
            ),
            resource_priority=list_or_none(get_field_values(sip, "resource_priority")),
            refer_to=list_or_none(
                [ReferTo.parse(v) for v in get_field_values(sip, "refer_to")]
            ),
            referred_by=next(
                iter(
                    [ReferredBy.parse(v) for v in get_field_values(sip, "referred_by")]
                ),
                None,
            ),
            event=next(
