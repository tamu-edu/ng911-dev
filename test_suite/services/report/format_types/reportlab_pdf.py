    ...
    for test in report_data["tests"]:
        ...
        for req in test["reqs"]:
            ...
            for variation in req["variations"]:
                elements.append(Paragraph(f"Variation: {variation['name']}", styles['Heading4']))
                elements.append(Paragraph(f"Description: {variation['description']}", styles['BodyText']))
                elements.append(Paragraph(f"Verdict: {variation['verdict']}", styles['BodyText']))

                data = [["Check", "Verdict"]]
                for check in variation.get("checks", []):
                    data.append([check["name"], check["verdict"]])
