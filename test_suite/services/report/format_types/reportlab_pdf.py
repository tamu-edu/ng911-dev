...
def generate_pdf_with_reportlab(output_path, report_data):
    ...
    elements.append(
        Paragraph(
            f"Conformance Test Report ID: {report_data['info']['id']}", styles["Normal"]
        )
    )
    ...
    for k, v in iut.items():
        elements.append(
            Paragraph(f"{k.replace('_', ' ').capitalize()}: {v}", styles["Normal"])
        )
    elements.append(Spacer(1, 6))
    ...
    if report_data["info"]["comments"]:
        ...
        for c in report_data["info"]["comments"]:
            elements.append(
                Paragraph(f"{c['author']} – {c['comment']}", styles["Normal"])
            )
    ...
    for test in report_data["tests"]:
        ...
        for req in test["reqs"]:
            elements.append(
                Paragraph(f"Requirement: {req['name']}", styles["Heading3"])
            )
            ...
            if report_data.get("detailed_view", False):
                for check in req.get("checks", []):
                    elements.append(
                        Paragraph(
                            f"Check: {check['name']} – {check['verdict']}",
                            styles["Heading4"],
                        )
                    )
