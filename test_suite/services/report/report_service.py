...
class ReportService:
    ...
    def _generate_req_vars_results(self, variation_names: list, req, var_descriptions):
        ...
        if req_schema.get("test_id"):
            check_names = importlib.import_module(req_schema.get("test_id")).get_test_names()
                            ...
                            if var_name == scenario.name:
                                for verdict in scenario.intermediate_verdicts:
                                    if verdict.test_name == check_name:
                                        variation_dict["verdict"] = verdict.test_verdict.value
                                        variation_dict["error"] = verdict.error
                                        check_dict["variations"].append(variation_dict)
                    check_dict["verdict"] = self._calculate_check_verdict(check_dict.get("variations"))
                    if check_dict["verdict"]:
    ...
    def _generate_xml(self, output_path):
        ...
        props_el = ET.SubElement(testsuites_el, "properties")

        def add_prop(name, value):
            ET.SubElement(props_el, "property", name=name, value=str(value if value is not None else ""))
        ...
        add_prop("ctr.general_verdict", "PASSED" if status.get("general_verdict") else "FAILED")
        ...
        for test in rd.get("tests", []):
            ...
            suite_el = ET.SubElement(testsuites_el, "testsuite", name=str(test.get("name", "")))

            # Optional: suite-level properties (could include test-level metadata)
            suite_props = ET.SubElement(suite_el, "properties")
            ET.SubElement(suite_props, "property", name="ctr.section",
            ...
            for req in test.get("reqs", []):
                ...
                if not checks:
                    ...
                    if rver.upper() == "FAILED":
                        suite_failures += 1
                        fail = ET.SubElement(tc, "failure", message=f"Requirement verdict: {rver}")
                        fail.text = "Requirement failed."
                    elif rver.upper() in {"INC", "INCONCLUSIVE"}:
                        suite_skipped += 1
                        sk = ET.SubElement(tc, "skipped", message="Inconclusive")
                ...
                for check in checks:
                    ...
                    for variation in check.get("variations", []):
                        ...
                        if var_ver == "FAILED":
                            suite_failures += 1
                            fail = ET.SubElement(tc, "failure", message=f"{var_name}: FAILED")
    ...
    def _generate_csv(self, output_path):
        ...
        with open(output_path, mode="w", newline="") as csv_file:
            ...
            writer.writerow([
                ...
                "Requirement Verdict", "Check", "Check Verdict",
                "Variation", "Variation Description", "Variation Verdict",
            ])
    ...
    def _calculate_req_verdict(req_dict) -> str:
        ...
        for check in req_dict.get("checks"):
            ...
            if check.get("verdict") == VerdictType.INC.value:
                return VerdictType.INC.value
            if check.get("verdict") == VerdictType.NOT_RUN.value:
                return VerdictType.NOT_RUN.value
