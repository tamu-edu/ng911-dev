...
from .format_types.html_report import generate_html_report
from .diagrams import DiagramService
...
IO_DOCX_TEMPLATE = "test_suite/services/report/templates/io_doc_template.docx"
PDF_TEMPLATE = "test_suite/services/report/templates/pdf_template.html"
HTML_TEMPLATE = "test_suite/services/report/templates/html_template.html"
XML_ESCAPE_MAP = {
    '"': "&quot;",
    "'": "&apos;",
}
...
class ReportService:
    ...
    def __init__(
        ...
        if test_oracle:
            self._io_mode = test_oracle.get_io_mode()
        ...
        if self._io_mode:
            if report_format.lower() == ReportType.HTML.value:
                ...
                self._generate_html(
                    os.path.join(
                        html_report_directory,
                        filename + "." + report_format.lower(),
                    )
                )
            else:
                raise ValueError(
                    f"Unsupported report format for Interop mode. Currently supported: {ReportType.HTML}"
                )
        else:
            if report_format.lower() == ReportType.PDF.value:
                ...
                raise ValueError(
                    f"Unsupported report format. Use one of {ReportType.not_io_list()}"
                )
            ...
            for test in run_config.tests:
                for var in test.variations:
                    if var.name == var_name:
                        return var.description or ""
        return ""

    def _sanitize_xml(self, value):
        if isinstance(value, dict):
            return {k: self._sanitize_xml(v) for k, v in value.items()}

        elif isinstance(value, list):
            return [self._sanitize_xml(v) for v in value]
            ...
            # escape() already does: &, <, >
            return escape(value, XML_ESCAPE_MAP)
    ...
    def _generate_req_vars_results(
        self,
        variation_names,
        req,
        var_descriptions,
            ...
            for check_name in check_names:
                if check_name in subtests or len(subtests) == 0:
                    ...
                    for var_name in variation_names:
                        ...
                        if diagram_service:
                            artifacts = diagram_service.get_variation_artifacts(
                                requirement_name=req.name,
                                variation_name=var_name,
                            )

                            if artifacts:
                                variation_dict["artifacts"] = artifacts

                        for scenario in self.test_oracle.variations:
                            if var_name == scenario.name:
                                for verdict in scenario.intermediate_verdicts:
                                    if (
                                        verdict.test_name == check_name
                                        or "[Error]" in verdict.test_name
                                    ):
    ...
    def _generate_report_data(self):
        ...
        if self._io_mode:
            diagram_service = DiagramService(
                output_directory=run_config.output_folder,
            )
        ...
        if diagram_service:
            sequence_data = diagram_service.get_sequence_data()

            if sequence_data:
                self.report_data["sequence_diagrams"] = sequence_data
    ...
    def _generate_html(self, output_path):
        """Generate an HTML report from the canonical report data."""
    ...
    def _generate_docx(self, output_path):
        ...
        _sanitized_data = self._sanitize_xml(self.report_data)

        tpl.render({"add_run_style": add_run_style, **_sanitized_data})
