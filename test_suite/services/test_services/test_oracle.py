...
class TestOracle(metaclass=LoggingMeta):
    ...
    def _prepare_oracle_test_var_results(self, run_test: RunTest, variation: RunVariation,
                                         capture=None, pcap_file: str = None, ssl_keys_file_path: str = None):

        if capture:
            variation_pcap_service = PcapCaptureService(capture=capture)
        elif pcap_file:
            variation_pcap_service = PcapCaptureService(pcap_file=pcap_file, ssl_keys_file_path=ssl_keys_file_path)
        else:
    ...
    def _make_variation_not_run(self, run_variation):
        self.variations.append(
            OracleTestVariation.from_not_run_var(
                name=run_variation.name,
                description=run_variation.description
            )
        )
    ...
    def _prepare_variation_results(self):
        ...
        for run_test in self._run_config.tests:
            ...
            _not_run_variations = []
            print(f"[DEBUG] Starting preamble")
            ...
            for run_variation in run_test.variations:
                ...
                if run_variation.mode.lower() == ScenarioMode.PCAP.value:
                    ...
                    if pcap_ssl_keys_file_path:
                        self.__copy_ssl_keys_file_path_to_output_folder(pcap_ssl_keys_file_path,
                ...
                if pcap_path:
                    _var, variation_pcap_service = self._prepare_oracle_test_var_results(
                        pcap_file=pcap_path,
    ...
    def calculate_general_verdict(self):
        ...
        for scenario in self.variations:
            ...
            if scenario.get_scenario_verdict().test_verdict in [VerdictType.NOT_RUN, VerdictType.INC]:
