...
from services.interoperability.psm_service import (
    ProxyServerManagementService as PSMService,
)
...
from services.cleanup_registry import register_cleanup, unregister_cleanup
...
class TestOracle(metaclass=LoggingMeta):
    ...
    _io_mode: bool = False
    _forward_conduit_config: ForwardConduit | None = None
    ...
    def __init__(
        ...
        test_id: str,
        fwd_conduit_config: ForwardConduit | None = None,
    ):
        self._test_id = test_id
        self._lab_config = lab_config
        self._run_config = run_config
        self.tests: List[Any] = []
        self._forward_conduit_config = fwd_conduit_config
        self.general_verdict = None

        if self._run_config.global_config.type == "conformance":
            self._io_mode = False
        elif self._run_config.global_config.type == "interoperability":
            self._io_mode = True
            ...
            try:
                tas_list.append(
                    TestAssessmentService(
                        ...
                        tests_list=importlib.import_module(
                            test.get("test_id")
                        ).get_test_list(
                            pcap_service,
                            self._lab_config,
                ...
                print(f"[TEST ORACLE] VariationNotFoundError was raised -> {e}")
                tas_list.append(
                    TestAssessmentService(
                        name=f"{variation.name}_{test.get('test_id').split('_', 1)[0]}",
                        test_id=test.get("test_id"),
    ...
    def _prepare_oracle_test_var_results(
        ...
        if capture:
            variation_pcap_service = PcapCaptureService(
                ...
                forward_conduit_config=self._forward_conduit_config,
            )
        elif pcap_file:
            variation_pcap_service = PcapCaptureService(
                pcap_file=pcap_file,
                io_mode=self._io_mode,
                ssl_keys_file_path=ssl_keys_file_path,
                lab_config=self._lab_config,
                forward_conduit_config=self._forward_conduit_config,
            )
        else:
        ...
        if self._io_mode:
            html_path = variation_pcap_service.save_sequence_diagram_html(
                variation_name=variation.name,
                output_dir=self._run_config.output_folder,
            )
            print(f"Sequence diagram saved to: {html_path}")
    ...
    def prepare_variation_results(self):
            ...
            for run_variation in run_test.variations:
                ...
                if run_variation.mode.lower() == ScenarioMode.PCAP.value:
                    ...
                    if not self._io_mode:
                        ...
                        _session_id = f"{self._test_id}_{run_variation.name}"

                        # set up Proxy server
                        proxy_service = PSMService(
                        ...
                        # strat PS session
                        proxy_service.start_session()
                        proxy_service.wait_until_running()

                        # start Variation - without local capturing
                        live_var_service = LiveVariationService(
                            output_folder=self._run_config.output_folder,
                            lab_config=self._lab_config,
                            run_variation=run_variation,
                        ...
                        # After TS have done everything related to Var
                        # Start polling PS to see if artifacts are ready for extraction

                        # TODO - remove and replce with smth MORE
                        timeout = 15
                        while timeout > 0:
                            time.sleep(1)
                            timeout -= 1

                        proxy_service.stop_session()
                        _artifacts = proxy_service.wait_until_retrieve_ready()

                        local_paths = proxy_service.retrieve_artifacts(
                            artifacts=_artifacts,
                            dst_dir=self._run_config.output_folder,
                            session_id=_session_id,
                        )

                        pcap_path = local_paths.get("pcap_file")
                        pcap_ssl_keys_file_path = local_paths.get("tls_keys")

                        proxy_service.reset()
                        proxy_service.wait_until_reset()
                if pcap_path:
