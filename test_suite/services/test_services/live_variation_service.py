...
from services.config.types.run_config import RunVariation
...
class LiveVariationService(metaclass=LoggingMeta):
    ...
    def __init__(
        ...
        output_folder: str,
        disable_capturing: bool = False,
    ):
        self._monitor_shared_state: list = []
        self._shared_ip_aliases: list = []
        self._shared_routes: list = []
        self._test_id = test_id
        self._lab_config = lab_config
        self._run_variation = run_variation
        self._output_folder = output_folder or tempfile.mkdtemp(prefix="pcaps_")
        self._captured_files: List[Any] = []
        self._shared_scenarios: List[Any] = []
        ...
        self._enable_capturing = not disable_capturing
        ...
        while retry_count < max_retries:
            ...
            while p.is_alive():
                if parent_conn.poll():  # non-blocking check
                    ...
                    if (
                        isinstance(msg, dict)
                        and msg.get("type") == "WAIT_FOR_MANUAL_ACTION"
                    ):
                        prompt = msg.get("prompt")
                        input(f"👉 {prompt}")
                        parent_conn.send("CONTINUE")

                    if isinstance(msg, dict) and msg.get("type") == "MON_ADDED":
                        self._monitor_shared_state.append(msg.get("data"))

                    if isinstance(msg, dict) and msg.get("type") == "IP_ALIAS_ADD":
                        self._shared_ip_aliases.append(msg.get("data"))

                    if isinstance(msg, dict) and msg.get("type") == "IP_ALIAS_REMOVE":
                        data = msg.get("data")
                        if data in self._shared_ip_aliases:
                            self._shared_ip_aliases.remove(data)

                    if isinstance(msg, dict) and msg.get("type") == "ROUTE_ADD":
                        self._shared_routes.append(msg.get("data"))

                    if isinstance(msg, dict) and msg.get("type") == "ROUTE_REMOVE":
                        data = msg.get("data")
                        if data in self._shared_routes:
                            self._shared_routes.remove(data)
            ...
            if p.exitcode == 0:
                print(
                    f"✅ Variation {self._run_variation.name} finished without error."
                )
                if self._enable_capturing:
                    self._pcap_path = shared_scenarios[0]
                break  # success
            else:
                print(
                    f"❌ Variation {self._run_variation.name} failed with exit code {p.exitcode}. Retrying..."
                )
                retry_count += 1
                self.var_cleanup()
    ...
    def _is_manual_action_first(self) -> tuple[bool, bool]:
        """
        Generates manual actions based on variation description
        :param variation: RunVariation object
        :return: list
    ...
    def _get_config_part_value(self, part: str):
        config_part = None
        ...
        for key in keys:
            if isinstance(config_part, dict):
                ...
                raise ValueError(
                    f"Cannot go deeper at key '{key}', current is not a dict"
                )
    ...
    def _if_port_valid_for_action(port: PortMapping, message_type: str) -> bool:
        if port.protocol.upper() == message_type.upper():
            return True
        if port.protocol.lower() in [
            StubServerProtocol.HTTP.value,
            StubServerProtocol.HTTPS.value,
        ] and message_type.upper() in [
            StubServerProtocol.HTTP.value,
            StubServerProtocol.HTTPS.value,
        ]:
            return True
    ...
    def _prepare_sipp_scenario_kwargs(self, context: dict, kwargs: dict) -> dict:
        ...
        for key, value in kwargs.items():
            ...
            if v_type == "var":
                ...
                prepared_kwargs[key] = self._prepare_sipp_scenario_kwargs(
                    context, i_value
                )
            else:
        ...
        for message in self._run_variation.params["messages"]:
            if (
                ...
                and (
                    ...
                    (not next_action_id)
                    or (next_action_id and message.get("id") == next_action_id)
                )
                and (
                    # check IF name
                    message["if_name"]
                ...
                and (
                    ...
                    or (not message["if_port_name"])
                )
            ):
                context: dict[str, Any] = (
                    {}
                )  # This is where we save results (e.g., {"var.jws_body": "..."})
                ...
                if prep_steps:
                    for step in prep_steps:
                        ...
                        for key, value in kwargs.items():
                            i_value, _, v_type = check_value_for_file_var_mode(
                                value, True
                            )
                            if v_type == "var":
                                ...
                                for l_value in i_value:
                                    li_value, _, lv_type = (
                                        check_value_for_file_var_mode(l_value, True)
                                    )
                                    if lv_type == "var":
                                        validated_list.append(context.get(li_value))
                                    else:
                                ...
                                kwargs[key] = i_value
                        save_key, _ = check_value_for_file_var_mode(
                            step.get("save_result_as")
                        )
                        ...
                        if method_name == "extract_from_config":
                            if isinstance(kwargs.get("config_part"), str):
                                kwargs["config_part"] = self._get_config_part_value(
                                    kwargs.get("config_part")
                                )

                        if method_name == "get_interface_address":
                            kwargs["lab_config"] = copy.deepcopy(
                                self._lab_config.to_dict()
                            )

                        if method_name == "get_interface_port_number":
                            kwargs["lab_config"] = copy.deepcopy(
                                self._lab_config.to_dict()
                            )
                ...
                if (
                    message["type"] == StubServerProtocol.HTTP.value
                    or message["type"] == StubServerProtocol.HTTPS.value
                ):
                    request_body, is_file = check_value_for_file_var_mode(
                        message.get("body")
                    )

                    # TODO if there would be different file types (formats)
                    if is_file:
                        # leave as it is as HTTP_SERVER expect files as well
                        request_body = f"file.{request_body}"
                    elif isinstance(request_body, list):
                        _new_r_body: List[Any] = []
                        for _rb in request_body:
                            if _rb in context.keys():
                                _new_r_body.append(context.get(_rb))
                            else:
                                _new_r_body.append(_rb)
                        request_body = _new_r_body
                    else:
                        if request_body in context.keys():
                            request_body = context.get(request_body)

                    http_url, _, http_url_type = check_value_for_file_var_mode(
                        message.get("http_url"), True
                    )
                    ...
                    action_data[f"http_{entity.name.lower()}"] = {
                        ...
                        "ssl_config_file": message.get("ssl_config_file"),
                        "openssl_config_file": message.get("openssl_config_file"),
                    ...
                    if "/SIPp_scenarios/sip_service/" in sipp_scenario_file:
                        destination_dir = os.path.join(
                            "services", "stub_server", "sip", "scenarios", "sip_service"
                        )
                    else:
                        destination_dir = os.path.join(
                            "services", "stub_server", "sip", "scenarios"
                        )
                    destination_path = os.path.join(destination_dir, filename)

                    sipp_scenario_dependencies = sipp_scenario.get("dependencies")

                    if sipp_scenario_dependencies:
                        for dependency in sipp_scenario_dependencies:
                            _d, is_file = check_value_for_file_var_mode(dependency)

                            if is_file:
                                _f = os.path.basename(_d)
                                _d_path = os.path.join(destination_dir, _f)
                                shutil.copy(_d, _d_path)
                            else:
                                print(
                                    "[ACTION PREPARATION ERROR] sipp scenario dependency "
                                    "is not a file or file. prefix is missing"
                                )

                    # Ensure destination directory exists
                    os.makedirs(destination_dir, exist_ok=True)

                    shutil.copy(sipp_scenario_file, destination_path)
                    ...
                    action_data[f"sipp_{entity.name.lower()}_scenario"] = {
                        ...
                        "openssl_config_file": message.get("openssl_config_file"),
                        "next_action": message.get("next_action"),
                        "rtp_bind_port": sipp_scenario.get("rtp_bind_port"),
                        "rtp_bind_ip": sipp_scenario.get("rtp_bind_ip"),
                    }
                    return action_data
        return None
    ...
    def _launch_stub_server(
        ...
        action_data: dict,
        conn,
    ) -> tuple[StubServerService | None, str | None]:
        if port.protocol == StubServerProtocol.SIP.value:
            _sip_scenario = action_data.get(f"sipp_{entity.name.lower()}_scenario", {})
            try:
                stub_service = StubServerService(
                    ...
                    ss_role=_sip_scenario.get("role"),
                    docker_service=None,
                    sipp_kwargs=_sip_scenario.get("sipp_kwargs"),
                    rtp_bind_port=_sip_scenario.get("rtp_bind_port"),
                    rtp_bind_ip=_sip_scenario.get("rtp_bind_ip"),
                    scenario_file=_sip_scenario.get("path"),
                    ssl_config_file=_sip_scenario.get("ssl_config_file"),
                    openssl_config_file=_sip_scenario.get("openssl_config_file"),
                ...
                return stub_service, _sip_scenario.get("next_action")
            except Exception as e:
                print(e)
                print(
                    f"Failed to run stub server for {entity} -> {port.protocol}: {e}, Skipping this part"
                )
                return None, None
        elif (
            port.protocol == StubServerProtocol.HTTP.value
            or port.protocol == StubServerProtocol.HTTPS.value
        ):
            _http_data = action_data.get(f"http_{entity.name.lower()}", {})
            try:
                stub_service = StubServerService(
                    entity_name=entity.name,
                    lab_config=self._lab_config,
                    ...
                    port=port,
                    current_if=interface,
                    ss_role=_http_data.get("role"),
                    docker_service=None,
                    request_data=_http_data,
                    ssl_config_file=_http_data.get("ssl_config_file"),
                    openssl_config_file=_http_data.get("openssl_config_file"),
                    ssl_keylog_file=self._variation_ssl_keylog_file,
                    run_in_background=True,
                    conn=conn,
                )
                stub_service.launch_stub_server()
                self._active_stub_services.append(stub_service)
                return stub_service, _http_data.get("next_action")
            except Exception as e:
                print(
                    f"Failed to run stub server for {entity} -> {port.protocol}: {e}, Skipping this part"
                )
    ...
    def _prepare_and_launch_stub_server(
        ...
        action_data = self._prepare_action_data_for_ss_launch(
            entity.role, interface, port, entity, next_action
        )
    ...
    def __if_if_port_name_in_var_if_port_names(
        self, if_name: str, if_port_name: str
    ) -> bool:
        _interface = None
        for _if in self._run_variation.interfaces:
            if if_name == _if.name:
    ...
    def _sort_entities_sender_last(self, params: dict):
        ...
        return sorted(
            ...
            key=lambda e: (
                1
                if (
                    StubServerRole.SENDER.value in e.role
                    and any(
                        iface.name == message.get("if_name")
                        and message.get("action") == "send"
    ...
    def _run_variation_live(self, shared_scenario, conn):
        ...
        monitoring_capture_file: Optional[str] = None

        if self._enable_capturing:
            monitor, monitoring_capture_file, tshark_pid = self._launch_live_capturing(
                self._run_variation
            )
            conn.send(
                {"type": "MON_ADD", "data": [tshark_pid, monitoring_capture_file]}
            )
            time.sleep(3)

        # structure for further management of docker containers and docker services
        sss_list: List[Any] = []
        sss_senders_list: List[Any] = []
        ...
        is_manual_first, is_first_manual_has_next_action = (
            self._is_manual_action_first()
        )
        ...
        if is_manual_first:
            if not is_first_manual_has_next_action:
                ...
                if _timeout and isinstance(_timeout, int):
                    ...
                    conn.send(
                        {
                            "type": "WAIT_FOR_MANUAL_ACTION",
                            "prompt": _action.get("prompt"),
                        }
                    )
                    conn.recv()
            else:
                    ...
                    for interface in entity.interfaces:
                        if self.__if_if_name_in_var_ifs(interface.name):
                            for port in interface.port_mapping:
                                if self.__if_if_port_name_in_var_if_port_names(
                                    interface.name, port.name
                                ):
                                    _service, _next_action = (
                                        self._prepare_and_launch_stub_server(
                                            interface=interface,
                                            port=port,
                                            entity=entity,
                                            conn=conn,
                                    ...
                                    if _service is not None:
                                        if StubServerRole.SENDER.value in entity.role:
                                            while _next_action:
                                                ...
                                                for _index, _action in enumerate(
                                                    manual_actions
                                                ):
                                                    if (
                                                        _action.get("id")
                                                        ...
                                                        manual_actions.pop(_index)

                                                        _timeout = _action.get(
                                                            "timeout"
                                                        )
                                                        if _timeout and isinstance(
                                                            _timeout, int
                                                        ):
                                                            print(
                                                                "⚠️ REQUIRED TIMEOUT ⚠️"
                                                            )
                                                            if _action.get("prompt"):
                                                                print(
                                                                    _action.get(
                                                        ...
                                                        if _action.get("next_action"):
                                                            _next_action = _action.get(
                                                                "next_action"
                                                            )
        ...
        for action in manual_actions:
            ...
            if _timeout and isinstance(_timeout, int):
                ...
                print("⚠️ MANUAL STEP REQUIRED ⚠️")
                conn.send(
                    {"type": "WAIT_FOR_MANUAL_ACTION", "prompt": action.get("prompt")}
                )
                conn.recv()
            ...
            if next_action:
                for _index, _action in enumerate(manual_actions):
                    if _action.get("id") == next_action:
                        manual_actions.pop(_index)
                        ...
                        if _action.get("next_action"):
                            next_action = _action.get("next_action")

                for entity in sorted_entities:
                    if (
                        entity.mode.upper() == EntityMode.STUB.value
                        and get_ss_role_by_action_type(next_action.split("_")[0])
                        in entity.role
                    ):
        ...
        if self._enable_capturing and monitor and monitoring_capture_file:
            ...
            print(
                f"🧪 Prepared capture contains {len(list(monitor.get_capture()))} packets."
            )
            time.sleep(2)
    ...
    def ip_alias_cleanup(self):
        for ip, prefix, interface in self._shared_ip_aliases:
            subprocess.run(
    ...
    def routing_cleanup(self):
        print("[cleanup] Routing cleanup started")

        sudo = get_sudo()

        # --- 1. tracked cleanup (fast path)
        for source_ip, target_ip, table_id in self._shared_routes:
            try:
                subprocess.run(
                    [
                        *([sudo] if sudo else []),
                ...
                print(f"[cleanup] tracked cleanup failed: {e}")

        # --- 2. fallback cleanup (CRITICAL)
        try:
            ...
            for line in result.stdout.splitlines():
                if "lookup" in line:
                    ...
                    # your namespace: 10000–60000
                    if 10000 <= table_id <= 60000:
                        print(f"[cleanup] removing orphan rule: {line}")
            ...
            print(f"[cleanup] fallback routing cleanup failed: {e}")
