...
class LiveVariationService(metaclass=LoggingMeta):
        ...
        for message in self._run_variation.params["messages"]:
            if (
                ...
                context: dict[str, Any] = (
                    {}
                )  # This is where you save results (e.g., {"var.jws_body": "..."})
    ...
    def _launch_stub_server(
        self, interface: Interface, port: PortMapping, entity: Entity, action_data: dict
    ...
    def _prepare_and_launch_stub_server(
        self, entity: Entity, port: PortMapping, interface: Interface, next_action=None
    ) -> tuple[StubServerService | None, str | None]:
        ...
        if action_data:
            return self._launch_stub_server(
                interface=interface, port=port, entity=entity, action_data=action_data
            )
        else:
            return None, None
    ...
    def _run_variation_live(
        self, shared_scenario, conn, shared_ip_aliases, shared_monitor
    ):
