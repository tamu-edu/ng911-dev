...
class WorkflowNotificationService:
    ...
    def on_test_finish(self, test_id: str, verdict: str, reason: Optional[str] = None):
        self.sender.notify_test_status(
            test_id, status="done", verdict=verdict, reason=reason
        )
    ...
    @skip_if_disabled
    def on_verification_request(
        self, message: str, options: List[str], timelimit: int = 30
    ) -> str:
        verification_id = str(uuid.uuid4())
        self.sender.send_verification(
            verification_id, message, options, timelimit=timelimit
        )
