...
class OracleTestVariation:
    ...
    @classmethod
    def from_not_run_var(cls, name: str, description: str | None = None):
        return cls(
            name=name,
    ...
    def calculate_scenario_verdict(self):
        ...
        self.scenario_verdict = TestVerdict(
            test_name=self.name, test_verdict=VerdictType.PASSED
        )
    ...
    def print_scenario_verdict(self):
        print(
            f"Variation: {self.name}, Test verdict: {self.scenario_verdict.test_verdict.value}"
        )
        print(f"Description: {self.description}")
        print("-" * 50)
        for _verdict in self.intermediate_verdicts:
            print(
                f"{_verdict.test_verdict.value} - Test: {_verdict.test_name} -> "
