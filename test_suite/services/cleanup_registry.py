...
def register_cleanup(name: str, fn):
    with _lock:
        ...
        print(f"[cleanup] → Added {name} cleanup to registry")


def unregister_cleanup(name: str | None = None, fn=None):
    with _lock:
        global _cleanup_hooks
        _cleanup_hooks = [
            (n, f)
            for (n, f) in _cleanup_hooks
            if not ((name and n == name) or (fn and f == fn))
        ]
