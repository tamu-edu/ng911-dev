...
class ProxyServerManagementService(metaclass=LoggingMeta):
    ...
    def _set_proxy_server_config(self):
        ...
        ms_if = self._lab_config.get_if_by_name("IF_PROX_TS")
            ...
            conduit_list.append(
                {
                    ...
                    "transport": {
                        ...
                        "tls": self._get_tls_details(proxy_port, from_if, to_if),
                    },
                }
            )
        return conduit_list

    def _get_tls_details(
        self,
        port: PortMapping,
        from_if,
        to_if,
        ...
        if not pca_cert:
            raise RuntimeError(
                "[PSM Service] ⚠️ TLS enabled but PCA certificate not configured"
            )

        from_entity = self._lab_config.get_entity_by_if_name(from_if.name)
        to_entity = self._lab_config.get_entity_by_if_name(to_if.name)

        if from_entity is None:
            raise RuntimeError(
                f"[PSM Service] Source entity for {from_if.name} NOT FOUND"
            )

        if to_entity is None:
            raise RuntimeError(
                f"[PSM Service] Destination entity for {to_if.name} NOT FOUND"
            )

        if not to_entity.certificate_file or not to_entity.certificate_key:
            raise RuntimeError(
                f"[PSM Service] Destination entity {to_entity.name} "
                f"certificate/key not configured"
            )

        if not from_entity.certificate_file or not from_entity.certificate_key:
            raise RuntimeError(
                f"[PSM Service] Source entity {from_entity.name} "
                f"certificate/key not configured"
            )
        ...
        return {
            ...
            "mode": "terminate",
            "server_side": {
                "cert": to_entity.certificate_file,
                "key": to_entity.certificate_key,
                "ca": pca_cert,
            },
            "client_side": {
                "cert": from_entity.certificate_file,
                "key": from_entity.certificate_key,
                "ca": pca_cert,
            },
        }
    ...
    def start_session(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"[PSM Service] 🌐 -> session with {self._session_id} id accepted startup request."
                )
                return  # success
    ...
    def wait_until_running(self) -> None:
        ...
        while True:
            ...
            if state == State.RUNNING.value:
                print(
                    f"[PSM Service] 🌐 -> Session with id {self._session_id} "
                    f"has successfully been started."
                )
                return

            if state == State.STARTING.value:
                print(
                    "[PSM Service] ⏳ -> PROXY session is STARTING. "
                    "Waiting for all proxy workers to become ready."
                )
                time.sleep(self._POLL_INTERVAL)
                continue
            ...
            raise RuntimeError(
                f"[PSM Service] Unexpected state during startup: {state}"
            )
    ...
    @staticmethod
    def _get_artifact_prefix(artifact_type: str) -> str:
        if artifact_type == "pcap_file":
            return "pcaps"

        if "log" in artifact_type:
            return "io_logs"

        return "strange_artifact"

    def retrieve_artifacts(
        self, artifacts: dict, dst_dir: str, session_id: str
    ) -> dict:
        """
        ...
        for name, path in artifacts.items():
            ...
            _dst_dir = f"{dst_dir}/{self._get_artifact_prefix(name)}/{session_id}"
            os.makedirs(_dst_dir, exist_ok=True)

            dst_path = os.path.join(_dst_dir, filename)
