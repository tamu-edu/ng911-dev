    ...
    if launch_config.global_config.type == "conformance":
        for test in launch_config.tests:
            ...
            if use_rc:
                ...
                run_config_service.get_run_config().output_folder = (
                    launch_config.output_folder
                )

                is_all_data_present, comment = (
                    run_config_service.verify_interface_data_presence(
        ...
        print(
            "We are sorry, but the interoperability mode is in the development right now and we cannot use it"
        )
        return
    else:
        print("Unknown mode")
